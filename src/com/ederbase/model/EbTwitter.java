...
public class EbTwitter {
	...
	String getTwitterDetail(String stTwitterId) {
		String stReturn = "<table>";
		...
		try {
			 ...
			 * +"</td><td>" + status.getText() + "</td>"; }
			 */
			stReturn += "</tr><tr><td colspan=2><h1>getDirectMessages</h1></td>";
	...
	public String processResult(int iType) {
		...
		try {
			...
			if (rs != null) {
				rs.last();
				int iMax = rs.getRow();
				stReturn += "<tr><th>#</th><th>User</th><th>ID</th><th>Batches</th><th>Searches</th><th>Proc</th><th>Fing</th><th>Fers</td><th>%</td><th>Sec.</td></tr>";
				for (int iU = 1; iU <= iMax; iU++) {
					rs.absolute(iU);
					nmLoginId = rs.getInt("nmMyTwitterId");
				...
				if (iTotFollowing > 0)
					...
					fPct = 0;
				stReturn += "<tr>";
				stReturn += "<td align=right colspan=5>Totals:</td>";

				stReturn += "<td align=right>" + iProc + "</td>";
				stReturn += "<td align=right>" + iTotFollowing + "</td>";
				stReturn += "<td align=right>" + iTotFollowers + "</td>";
				stReturn += "<td align=right>" + fPct + "%</td>";
				stReturn += "</tr>";
			}
	...
	public String searchAndFollow(String stValue) {
		String stReturn = "";
		int iRpp = 0;
		int iLoop = 0;
	...
	public String addTwUserAndFollow(String stTwUser, String stType,
		...
		stReturn += "<td align=left>" + stTwUser + "</td>";
		stReturn += "<td align=left>" + stDate + "</td>";
		...
		if (nmCount <= 0) {
			...
			try {
				...
				stReturn += "<td align=right>ok " + iFollow + "</td>";
			} catch (twitter4j.TwitterException e) {
	...
	public String sendTweet(String stId, String stMessage) {
		String stReturn = "";
		try {
	...
	/*
	 * OK //OK twitter.sendDirectMessage("roberteder2",
	 * "working on the final release for monday"); List<Status> statuses =
	 * twitter.getFriendsTimeline();
	 * System.out.println("Showing friends timeline."); for (Status status :
