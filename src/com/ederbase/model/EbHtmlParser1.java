...
public class EbHtmlParser1 {
	...
	public String logWebSpider(EbDatabase dbEnterprise, int nmCompanyId,
		...
		if (nmSpiderId <= 0) {
			...
			stSql = "update X25WebSpider set dtLast=now(),nmStatus=" + nmStatus
					+ ",nmLength= " + this.cbOut.getLength() + " ,nmCompanyId="
	...
	public String[] doParse(EbDatabase dbEntertprise, int nmCompanyId,
			String stUrl) throws Exception {
		this.stUrl = stUrl;
		...
		try {
			...
			if (iCount == 0) {
				...
				int tmp219_218 = 0;
				String[] tmp219_216 = aReturn;
				tmp219_216[tmp219_218] = (tmp219_216[tmp219_218] + "<hr>DONE parsing ");
				int tmp243_242 = 0;
				String[] tmp243_240 = aReturn;
				tmp243_240[tmp243_242] = (tmp243_240[tmp243_242]
						+ "<hr><font size=1>" + this.cbOut.getString());

				logWebSpider(dbEntertprise, nmCompanyId, 1, "ok", theURLconn
						.getContent().toString(), theURLconn.getContentLength());
			} else {
				int tmp303_302 = 0;
				String[] tmp303_300 = aReturn;
				tmp303_300[tmp303_302] = (tmp303_300[tmp303_302] + "<hr>Already parsed<hr>");
			}
		} catch (InterruptedIOException e) {
			System.out.println("<br>ERROR: doParse: timeout " + e);
			int tmp358_357 = 0;
			String[] tmp358_355 = aReturn;
			tmp358_355[tmp358_357] = (tmp358_355[tmp358_357]
					+ "<br>ERROR: doParse: timeout " + e);
			logWebSpider(dbEntertprise, nmCompanyId, 2, e.toString(), "", 0);
		} catch (Exception e) {
			System.out.println("<br>ERROR: doParse: " + e);
			int tmp434_433 = 0;
			String[] tmp434_431 = aReturn;
			tmp434_431[tmp434_433] = (tmp434_431[tmp434_433]
					+ "<br>ERROR: doParse: " + e);
			aReturn[1] = this.cbOut.getHrefList();
			logWebSpider(dbEntertprise, nmCompanyId, 2, e.toString(), "", 0);
		}
	...
	public void handleText(char[] text, int position) {
		...
		if (this.iInHref > 0) {
			...
			if (this.stLastHref.indexOf("PersonID=") >= 0) {
				this.stAll = (this.stAll + "AGENT\t" + this.stLastHref + "\t"
						+ String.valueOf(text).trim() + "\n");
				this.stOut = (this.stOut + "<br>AGENT\t" + this.stLastHref
						+ "\t<b>" + String.valueOf(text).trim() + "</b>\n");
			} else if ((iPos = this.stLastHref.indexOf("AgentEmailAddress=")) >= 0) {
				int iPos2 = this.stLastHref.indexOf("&", iPos);
				if (iPos2 > iPos) {
					this.stAll = (this.stAll + "EMAIL\t"
							+ this.stLastHref.substring(iPos + 18, iPos2) + "\t\n");
					this.stOut = (this.stOut + "<br>EMAIL\t"
							+ this.stLastHref.substring(iPos + 18, iPos2) + "\t\n");
				}
				...
				int iPos2 = this.stLastHref.indexOf("&", iPos);
				if (iPos2 > iPos) {
					this.stAll = (this.stAll + "ADDRESS\t"
							+ this.stLastHref.substring(iPos + 4, iPos2) + "\t\n");
					this.stOut = (this.stOut + "<br>ADDRESS\t"
							+ this.stLastHref.substring(iPos + 4, iPos2) + "\t\n");
				}
			}
			if (this.stLastClass.equals("ao_office_name_container")) {
				if ((iPos = this.stLastHref.indexOf("OfficeID=")) > 0) {
					this.stAll = (this.stAll + "OFFICE\t"
							+ String.valueOf(text).trim() + "\t\n");
					this.stOut = (this.stOut + "<br>OFFICE\t"
							+ String.valueOf(text).trim() + "\t\n");
				} else {
					this.stAll = (this.stAll + "OFFICE\t"
							+ String.valueOf(text).trim() + "\t"
		...
		if (this.stLastClass.equals("ao_page_controls_text")) {
			if (String.valueOf(text).trim().equals(">")) {
				...
				this.stOut = (this.stOut + "<br>NEXT: yes <b>"
						+ this.stLastHref + "</b>");
			}
		...
		if (this.iStart > 0) {
			if ((this.stLastClass.equals("fn org")) || (this.iStart == 1)) {
				...
				this.stOut = (this.stOut + "<hR><b>COMPANY: "
						+ String.valueOf(text) + "</b>");
				this.iStart += 1;
			...
			} else if ((this.stLastClass.equals("web"))
					|| (this.stLastClass.equals("main_web_site"))) {
				this.stAll = (this.stAll + "WEB\t" + this.stLastHref + "\t\n");
				this.stOut = (this.stOut + "<BR>Web: " + this.stLastHref);
			} else {
				this.stAll = (this.stAll + this.stLastClass + "\t"
						+ String.valueOf(text) + "\t\n");
				this.stOut = (this.stOut + "<br>[" + this.stLastClass + " "
						+ this.iStart + "] " + String.valueOf(text));
			}
		}
		if (this.stLastClass.equals("next")) {
			this.stAll = (this.stAll + "NEXT\t" + this.stLastHref + "\t\n");
			this.stOut = (this.stOut + "<br>NEXT: " + String.valueOf(text)
					+ " <b>" + this.stLastHref + "</b>");
		}
	}
