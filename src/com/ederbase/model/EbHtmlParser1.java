	...
	public String[] doParse(EbDatabase dbEntertprise, int nmCompanyId,
		...
		try {
			...
			if (iCount == 0) {
				...
				tmp243_240[tmp243_242] = (tmp243_240[tmp243_242] + "<hr><font size=1>" + this.cbOut
				    .getString());
			...
			tmp434_431[tmp434_433] = (tmp434_431[tmp434_433] + "<br>ERROR: doParse: " + e);
			aReturn[1] = this.cbOut.getHrefList();
			logWebSpider(dbEntertprise, nmCompanyId, 2, e.toString(), "", 0);
		}
		return aReturn;
	...
	public void handleText(char[] text, int position) {
		...
		if (this.iInHref > 0) {
			...
			if (this.stLastHref.indexOf("PersonID=") >= 0) {
				...
				this.stOut = (this.stOut + "<br>AGENT\t" + this.stLastHref + "\t<b>"
				    + String.valueOf(text).trim() + "</b>\n");
			...
			if (this.stLastClass.equals("ao_office_name_container")) {
				if ((iPos = this.stLastHref.indexOf("OfficeID=")) > 0) {
					...
					this.stAll = (this.stAll + "OFFICE\t" + String.valueOf(text).trim()
					    + "\t" + this.stLastHref + "\n");
					this.stOut = (this.stOut + "<br>OFFICE\t"
					    + String.valueOf(text).trim() + "\t" + this.stLastHref + "\n");
				}
			}
		}
		...
		if (this.stLastClass.equals("next")) {
			this.stAll = (this.stAll + "NEXT\t" + this.stLastHref + "\t\n");
			this.stOut = (this.stOut + "<br>NEXT: " + String.valueOf(text) + " <b>"
			    + this.stLastHref + "</b>");
		}
	}
