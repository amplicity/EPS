...
public class EbAdmin {
	...
	public String getDbManagement(ResultSet rsA, HttpServletRequest request) {
		...
		try {
			...
			default:
				stReturn = stReturn
						+ "<BR>ERROR getDbManagement: invalid state/process: "
						+ iState;
			}
		} catch (Exception e) {
			...
			if (rsF != null) {
				...
				for (int iF = 1; iF <= iMaxF; iF++) {
					...
					eb = new EbDatabase(rsF.getInt("nmDbType"),
							rsF.getString("stServerIp"),
							rsF.getString("stUser"),
							rsF.getString("stPassword"),
							rsF.getString("stDbName"), "");
				...
				for (int iU = 1; iU <= iMaxRow; iU++) {
					...
					this.ebEnt.dbEnterprise
							.ExecuteUpdate("update X25User set stEMail="
									+ this.ebEnt.dbEnterprise
											.fmtDbString(stEmail)
									+ " where RecId=" + rs.getString("RecId"));
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			for (int iRS = 1; iRS <= iMaxServer; iRS++) {
				...
				for (int iRD = 1; iRD <= iMaxDatabases; iRD++) {
					...
					case 1:
						...
						if ((!stDbName.equals("model"))
								&& (!stDbName.equals("master"))
								&& (!stDbName.equals("msdb"))
								&& (!stDbName.equals("pubs"))
					...
					EbDatabase ebD = new EbDatabase(rsS.getInt("nmDbType"),
							rsS.getString("stServerIp"),
							rsS.getString("stUser"),
					...
					for (int iRT = 1; iRT <= iMaxTables; iRT++) {
						...
						for (int iRF = 1; iRF <= iMaxFields; iRF++) {
							...
							this.ebEnt.dbEb
									.ExecuteUpdate("update t_fields set stFieldType='"
											+ stFieldType
											+ "',stDefault='"
											+ stDefault
											+ "',stExtra='"
											...
											+ ",nmTableOrder="
											+ iRF
											+ ",nmFieldFlag = ( (nmFieldFlag |"
											+ nmFieldFlag
											+ " )& ~0x40000000) where nmFieldId="
											+ iFieldId);
						}
					}
					ebD.ebClose();
					stReturn = stReturn + "<br>DB " + iRD + " of: "
							+ iMaxDatabases;
				}
			...
			if (rsS1 != null) {
				...
				for (int iS = 1; iS <= iMaxS1; iS++) {
					...
					EbDatabase ebTemp = new EbDatabase(rsS1.getInt("nmDbType"),
							rsS1.getString("stServerIp"),
							rsS1.getString("stUser"),
			...
			for (int iR = 1; iR <= iMaxRow; iR++) {
				...
				if ((iTableId != iLastTable) && (iLastTable > 0)) {
					this.ebEnt.dbEb.ExecuteUpdate("update t_tables set stPk='"
							+ stPk + "', stPkFids='" + stPkFids
							+ "' where nmTableId=" + iLastTable);
					stPk = stPkFids = "";
				}
				if (!stPk.equals("")) {
					stPk = stPk + "|";
	...
	public String fixCommHistory() {
		...
		try {
			...
			stReturn = stReturn + "<br> " + iMax
					+ " a) mulitple emails from gmail ";
			...
			stReturn = stReturn + "<br> " + iMax4
					+ " c) add NEW COMM to HISTORY ";
			...
			for (int iC5 = 1; iC5 <= iMax5; iC5++) {
				...
				this.ebEnt.ebNorm.mergePerson(rs5.getString("stFirstName"),
						rs5.getString("stMiddleName"),
						rs5.getString("stLastName"));
			}
			stReturn = stReturn
					+ "<br> "
					+ iMax5
	...
	public String getImportCompanies(ResultSet rsA, HttpServletRequest request) {
		...
		try {
			...
			case 2:
				stReturn = stReturn
						+ "<table><tr><th colspan=2>Importing Companies from "
						+ stDbName + "</th></tr>";
				...
				if (rs != null) {
					...
					int iMax = rs.getRow();
					stReturn = stReturn
							+ "<tr><td>By Email reference: </td><td>" + iMax
							+ "</td>";
					for (int iT = 1; iT <= iMax; iT++) {
						...
						for (int iI = 1; iI <= iMaxI; iI++) {
							...
							int nmCompanyId = this.ebEnt.ebNorm.getCompanyId(
									rsImport.getString("stCompanyName"),
				...
				if (rs != null) {
					...
					int iMax = rs.getRow();
					stReturn = stReturn
							+ "<tr><td>By Phone reference: </td><td>" + iMax
							+ "</td>";
					for (int iT = 1; iT <= iMax; iT++) {
						rs.absolute(iT);
	...
	public String getImportEnterprise(ResultSet rsA, HttpServletRequest request) {
		...
		try {
			...
			case 2:
				...
				ResultSet rsImport = db.ExecuteSql(stSql);
				stReturn = stReturn
						+ "<table><tr><th colspan=2>Importing from " + stDbName
						+ "</th></tr>";
				if (rsImport != null) {
					rsImport.last();
					...
					for (int iI = 1; iI <= iMax; iI++) {
						...
						String stMiddleName = rsImport
								.getString("stMiddleName");
						...
						nmPersonId = this.ebEnt.ebNorm.getPersonId(stFirstName,
								stMiddleName, stLastName, stPhone, "US",
								stEMail);
						...
						stReturn = stReturn + "<tr><td align=right>"
								+ nmPersonId + "</td>" + "<td>" + stFirstName
								+ "</td>" + "<td>" + stLastName + "</td>"
								+ "<td>" + stPhone + "</td>" + "<td>" + stEMail
								+ "</td>" + "<td " + stBg + ">"
								+ rsImport.getString("RecId") + "</td>"
								+ "<td>"
								+ rsImport.getString("dtEventStartTime")
								+ "</td>" + "<td>"
								+ rsImport.getString("stComment") + "</td>"
								+ "</tr>";
		...
		try {
			...
			if ((stTn == null) || (stTn.equals(""))) {
				...
				if ((s1 != null) && (s1.equals("y"))) {
					stSql = stSql
							+ " and d.stDbName in ('ederbase','ebenterprise') ";
				}
				stSql = stSql
						+ " order by t.stTableName, d.stDbName, s.stServerName";
				rs = this.ebEnt.dbEb.ExecuteSql(stSql);
				rs.last();
				iRowMax = rs.getRow();
				stReturn = stReturn
				...
				for (iF = 1; iF <= iRowMax; iF++) {
					...
					stReturn = stReturn + "<td align=right>"
							+ rs.getString("nmRows") + "</td>";
					stReturn = stReturn + "<td><a href='./?a=28&tn="
							+ rs.getString("nmTableId") + "'><b>"
							+ rs.getString("stTableName") + "</b></a></td>";
					stReturn = stReturn + "<td>" + rs.getString("stDbName")
							+ "</td>";
					stReturn = stReturn + "<td>" + rs.getString("stServerName")
							+ "</td>";
					stReturn = stReturn + "<td>" + rs.getString("nmServerId")
							+ "</td>";
					stReturn = stReturn + "</tr>";
				}
				...
				dbTemp = new EbDatabase(rsDb.getInt("nmDbType"),
						...
						rsDb.getString("stPassword"),
						rsDb.getString("stDbName"),
						rsDb.getString("stConnectString"));
				String stSqlList = "";
				if (rsDb.getInt("nmDbType") == 1) {
					stSqlList = "select top 1000 * from "
							+ rsDb.getString("stTableName");
				} else {
					stSqlList = "select * from "
							+ rsDb.getString("stTableName") + " limit 1000 ";
				}
				ResultSet rsF = this.ebEnt.dbEb
						.ExecuteSql("select * from t_fields where nmTableId="
								+ stTn + " order by stDbFieldName");
				rsF.last();
				...
				if ((stTid != null) && (!stTid.equals(""))) {
					...
					if ((stTemp != null) && (stTemp.equals("y"))) {
						stSql = "delete from " + rsDb.getString("stTableName")
								+ " where " + stPk;
						...
						if ((stTemp != null)
							...
							if (((aPkValue[0].length() >= 2) && (aPkValue[0]
									.substring(0, 2).equals("-2")))
									|| (stTemp.equals("Insert as new"))) {
								String s1 = "";
								String s2 = "";
								...
								for (; iF <= iFieldMax; iF++) {
									rsF.absolute(iF);
									stTemp = request.getParameter("g"
											+ rsF.getInt("nmFieldId") + "|"
											+ stTid);
				...
				if ((stTid == null) || (stTid.trim().equals(""))) {
					...
					for (iD = 1; iD <= iRowMax; iD++) {
						...
						for (iF = 1; iF <= iFieldMax; iF++) {
							...
							stReturn1 = stReturn1 + "<td valign=top>" + stValue
									+ "</td>";
						}
					...
					for (iD = 1; iD <= iRowMax; iD++) {
						...
						for (iF = 1; iF <= iFieldMax; iF++) {
							...
							stReturn = stReturn + "<td align=right valign=top "
									+ stBg + ">"
									+ rsF.getString("stDbFieldName") + "</th>";
							...
							if (stValue != null)
								stValue = stValue.replace("&", "&amp;");
							stReturn = stReturn
									+ dbTemp.fmtInput(1, rsF, stValue, stTid);
							stReturn = stReturn + "<tr>";
						}
					}
					stReturn = stReturn
	...
	int[] parseFieldType(String stFieldType) {
		...
		if (stTemp.equals("bigint")) {
			...
			aF[0] = 3;
				...
				|| (stTemp.equals("mediumblob"))
				|| (stTemp.equals("mediumtext")) || (stTemp.equals("nchar"))
				|| (stTemp.equals("ntext")) || (stTemp.equals("text"))
				|| (stTemp.equals("tinyblob"))) {
			aF[0] = 4;
		...
		if ((stValue != null) && (stValue.length() > 0)) {
			...
			for (int iFound = 0; iL < aLines.length; iL++) {
				...
				if (iFound > 0) {
					...
					stReturn = stReturn + "<br>Removing: " + stLine
							+ " nmErrorCount: " + nmErrorCount;
					iFound = 0;
	...
	public String processHourly(int iInterval) {
		...
		try {
			if (iInterval > 0) {
				...
				this.ebEnt.dbEnterprise
						.ExecuteUpdate("truncate table MyHourlyTable");
			}
			ResultSet rs = this.ebEnt.dbEnterprise.ExecuteSql(stSql);
			if (rs != null) {
				rs.last();
			...
			if (rs != null) {
				...
				for (int iH = 1; iH <= iMax; iH++) {
					...
					if (i <= 0) {
						...
						stSql = "update MyHourlyTable set nmMoodCount="
								+ rs.getString(1) + ",nmMoodSum="
								+ rs.getString(6) + " where nmYear="
								+ rs.getString(2) + " and nmMonth="
								+ rs.getString(3) + " and " + "nmDay="
								+ rs.getString(4) + " and nmHour="
								+ rs.getString(5);
					}
			...
			if (rs != null) {
				...
				for (int iH = 1; iH <= iMax; iH++) {
					...
					if (i <= 0) {
						...
						stSql = "update MyHourlyTable set nmPetCount="
								+ rs.getString(1) + ",nmPetSum="
								+ rs.getString(6) + "," + "stPetDetail = "
								+ this.ebEnt.dbEnterprise.fmtDbString(stDetail)
								+ " where nmYear=" + rs.getString(2)
								+ " and nmMonth=" + rs.getString(3) + " and "
								+ "nmDay=" + rs.getString(4) + " and nmHour="
								+ rs.getString(5);
					}
	...
	public String processTwitter(HttpServletRequest request) {
		...
		if ((stValue != null) && (stValue.length() > 0)) {
			stReturn = stReturn
					+ "<h1>Processing Twitter: search and follow</h1>";
			EbTwitter ebTwitter = new EbTwitter(this.ebEnt);
	...
	public String fixInsert() {
		...
		if (stValue != null && stValue.length() > 0) {
			 ...
			 * ,dtDate,nmOrigSize,stTitle,stContent
			 * ,stHeader,stForeignRefId,stHtml) values
			 * (41575,42,5518,512,now(),9118
			 ...
			 * "") Exception: com.mysql.jdbc.exceptions.jdbc4.
			...
			if (rs != null) {
				...
				for (int iC = 1; iC <= iMax; iC++) {
					...
					if (!stLast.equals(stEmail)) {
						...
						stReturn = stReturn
								+ "<td><input type=checkbox name=usrs value='"
						...
						if (iPos1 >= 0) {
							iPos2 = rs.getString("stContent").indexOf("\n",
									iPos1);
							if (iPos2 > iPos1) {
								stUrl = rs.getString("stContent").substring(
										iPos1, iPos2);
							} else {
								stUrl = rs.getString("stContent").substring(
										iPos1);
		...
		try {
			...
			if (rs != null) {
				...
				for (int iR = 1; iR <= iMax; iR++) {
					rs.absolute(iR);
					stReturn = stReturn
							+ markEmail(rs.getString("stContent"),
									rs.getInt("RecId"));
				}
				rs.close();
			}
			...
			this.stError = (this.stError + "<BR>ERROR fixUndeliverable: ["
					+ stSql + "] " + e);
		}
		return stReturn;
	}
	...
	public String markEmail(String stContent, int iRecId) {
		...
		if (stContent != null) {
			...
			for (int iL = 0; iL < aLines.length; iL++) {
				...
				stReturn = stReturn + "<br>Marking: " + stEmail + " count: "
						+ i;
				this.ebEnt.dbEnterprise
			...
			if (iCount == 0) {
				...
				if (iPos1 >= 0) {
					...
					if ((iPos2 > iPos1) && (iPos3 > iPos2)
						...
						if (EbStatic.isEmail(stEmail)) {
							iCount++;
							int i = incEmailError(stEmail);
							stReturn = stReturn + "<br>Marking: " + stEmail
									+ " count: " + i;
							this.ebEnt.dbEnterprise
									.ExecuteUpdate("update X25Communications set nmFlags=0x100 where RecId="
											+ iRecId);
				...
				if (iCount == 0) {
					...
					stReturn = stReturn
							+ "<br>ERROR: Cannot find email for com.RecId="
							+ iRecId + " content: "
							+ stContent.replace("<", "&lt;");
					this.ebEnt.dbEnterprise
							.ExecuteUpdate("update X25Communications set nmFlags=0x100 where RecId="
			...
			if (rs != null) {
				...
				if ((stValue != null) && (stValue.length() > 0)) {
					...
					for (; iT <= iMax; iT++) {
						...
						stReturn = stReturn + "<td colspan=2><b>" + stMessage
								+ "</b> ";
				...
				for (; iT <= iMax; iT++) {
					...
					stReturn = stReturn + "<td>" + rs.getString("nmFollowers")
							+ "</td>";
					...
					try {
						...
						if (nmForeignId <= 0) {
							this.stError = (this.stError
									+ "<BR>NOT FOUND stResponseStructure: ["
									+ aV[(iV + 1)] + "] " + stNewField);
						}
					} catch (Exception e) {
					}
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				for (int iV = 2; iV < aV.length; iV += 2) {
					...
					try {
						...
						stNew = stNew + stNewField;
						if (nmForeignId <= 0) {
							this.stError = (this.stError + "<BR>NOT FOUND: ["
									+ aV[(iV + 1)] + "] " + stNewField);
						}
					} catch (Exception e) {
					}
	...
	public static int[] getEpsDataType(String stDt) {
		...
		if ((stDt.equals("Drop-Down List")) || (stDt.equals("Drop Down List"))
			...
			aiReturn[0] = 31;
		} else if ((stDt.equals("Radio Button"))
				|| (stDt.equals("Radio Buttons"))) {
			aiReturn[0] = 37;
