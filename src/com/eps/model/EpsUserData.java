					...
					if (iRecId > 0) {
						...
						if (iRecId == -5) {
							...
							makeTask(5, "E-Mail/Password not found "
									+ this.ebEnt.ebUd.aLogin[2]); // 5 1 Failed
																	// Login
																	// Enabled
																	// Yes
							makeMessage("All",
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					double InitialEstimatedCost = this.ebEnt.dbDyn
							.ExecuteSql1n("select sum(SchCost) from Schedule"
									...
									+ iIniticalBaseline + " and SchLevel=0");
					double CurrentEstimatedCost = this.ebEnt.dbDyn
							.ExecuteSql1n("select sum(SchCost) from Schedule"
									+ " where nmProjectId="
									+ rs.getInt("RecId") + " and nmBaseline="
									+ rs.getInt("CurrentBaseline")
									+ " and SchLevel=0");
						...
						try {
							...
							if (iM > 0) {
								...
								if (rsSch.getString("SchDone") != null
										&& rsSch.getString("SchDone").equals(
												"Y")) {
		...
		try {
			...
			if (rsTable.getInt("nmTableId") == 9) // Users
			{
				...
				stTable2 += "<tr><td class='' valign=top>Labor Categories: </td><td align=left >"
						+ makeList(97, 1, rsU.getInt("RecId"))
						+ "</td></tr>"
						+ "</table>";
