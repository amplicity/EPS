		...
		try {
			...
			epsud.ebEnt.dbDyn
			    .ExecuteUpdate("DELETE FROM teb_refinventory WHERE nmProjectId NOT IN ("
			        + pids + ")");
			epsud.ebEnt.dbDyn
			    .ExecuteUpdate("DELETE FROM test WHERE nmProjectId NOT IN (" + pids
			        + ")");
			...
			epsud.ebEnt.dbEnterprise
			    .ExecuteUpdate("DELETE FROM x25audittrail WHERE nmUserId NOT IN ("
			        + uids + ")");
			epsud.ebEnt.dbEnterprise
			    .ExecuteUpdate("DELETE FROM x25user WHERE RecId NOT IN (" + uids
			        + ")");
			epsud.ebEnt.dbEnterprise
...
public class EpsUserData {
	...
	public void processSubmit(HttpServletRequest request,
		...
		try {
			...
			if (stLogout != null && stLogout.equals("Logout")) {
				...
				Set<Integer> ids = (Set<Integer>) this.ebEnt.ebUd.request.getSession()
				    .getServletContext().getAttribute("LOG_IN_USERS");
				Object userId = this.ebEnt.ebUd.request.getSession().getAttribute(
				    "userId");
						...
						for (iL = 0; iL < aLines.length; iL++) {
							...
							if (stTemp.contains(" of ") || stTemp.contains(" in ")) {
								...
								if (stTemp.contains("3rd Monday of January")) {
									...
									iDay += 14;
								} else if (stTemp.contains("3rd Monday of February")
								    || stTemp.contains("Third Monday in February")) {
									iMonth = 2;
									calendar.set(Calendar.MONTH, iMonth - 1);
									calendar.set(Calendar.DAY_OF_MONTH, 1);
									iWeekday = calendar.get(Calendar.DAY_OF_WEEK);
								    ...
								    || stTemp.contains("Second Monday in October")) {
									iMonth = 10;
									calendar.set(Calendar.MONTH, iMonth - 1);
									calendar.set(Calendar.DAY_OF_MONTH, 1);
									iWeekday = calendar.get(Calendar.DAY_OF_WEEK);
									...
									iDay += 7;
								} else if (stTemp.contains("first Monday of September")
								    || stTemp.contains("First Monday in September")) {
									iMonth = 9;
									calendar.set(Calendar.MONTH, iMonth - 1);
									calendar.set(Calendar.DAY_OF_MONTH, 1);
									iWeekday = calendar.get(Calendar.DAY_OF_WEEK);
		...
		try {
			...
			if (stAction != null && stAction.equals("schema")) {
					...
					if (stValue == null || stValue.trim().length() <= 0) {
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update teb_table set stDbTableName = \""
						        + rs.getString("stTableName").trim().replace(" ", "_")
						        + "\" where nmTableId=" + rs.getString("nmTableId"));
					}
				}
					...
					if (!stValue.equals(stValueOrig)) {
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update teb_fields set stDbFieldName = \""
						        + stValue + "\" where stDbFieldName = \"" + stValueOrig
						        + "\" ");
					}
				}
				...
				for (iR = 1; iR <= iMax; iR++) {
					...
					if (rs.getInt("nmTableType") > 0) {
						...
						case 2: // Labor Category, etc. Simple tables
							...
							break;
						case 3: // from table only. use this after making manuall changes to
							      // schema.
							break;
						default:
							this.stError += "<BR>ERROR: Rebuilding Schema nmTableType="
							    + rs.getInt("nmTableType") + " not implemented. ";
							break;
						}
						this.ebEnt.dbDyn.ExecuteUpdate("update teb_table set stTop=\""
						    + stTop + "\",stPk=\"" + stPk + "\",stBottom=\"" + stBottom
						    + "\",stFields=\"" + stFields + "\" where nmTableId="
				...
				for (iR = 1; iR <= iMax; iR++) {
					...
					/* TODO: Insert */
					stReturn += "<tr>";
					stReturn += "<td align=right>" + rs.getString("nmForeignId")
					    + "</td>";
					if (stSave != null && stSave.length() > 0) {
						stSql = "update teb_fields set " + "nmDataType=\""
						    + this.ebEnt.ebUd.request.getParameter("nmDataType_" + iRecId)
						    + "\","
						    + "nmTabId=\""
						    + this.ebEnt.ebUd.request.getParameter("nmTabId_" + iRecId)
						    + "\","
						    + "nmFlags="
						    + this.ebEnt.ebUd.request.getParameter("nmFlags_" + iRecId)
						    + ","
						    + "stDbFieldName=\""
						    + this.ebEnt.ebUd.request.getParameter("stDbFieldName_"
						        + iRecId)
						    + "\","
						    + "stLabel=\""
						    + this.ebEnt.ebUd.request.getParameter("stLabel_" + iRecId)
						    + "\","
						    + "nmMinBytes=\""
						    + this.ebEnt.ebUd.request.getParameter("nmMinBytes_" + iRecId)
						    + "\","
						    + "nmMaxBytes=\""
						    + this.ebEnt.ebUd.request.getParameter("nmMaxBytes_" + iRecId)
						    + "\","
						    + "nmRows=\""
						    + this.ebEnt.ebUd.request.getParameter("nmRows_" + iRecId)
						    + "\","
						    + "nmCols=\""
						    + this.ebEnt.ebUd.request.getParameter("nmCols_" + iRecId)
						    + "\","
						    + "stValidation=\""
						    ...
						    + "\","
						    + "nmHeaderOrder=\""
						    + this.ebEnt.ebUd.request.getParameter("nmHeaderOrder_"
						        + iRecId)
						    + "\","
						    + "stDefaultValue=\""
						    + this.ebEnt.ebUd.request.getParameter("stDefaultValue_"
						        + iRecId) + "\"," + "nmOrder2=\""
						    + this.ebEnt.ebUd.request.getParameter("nmOrder2_" + iRecId)
						    + "\"" + " where nmForeignId=" + iRecId2;
						stReturn += "<td align=left class=small>" + stSql + "</td>";
						this.ebEnt.dbDyn.ExecuteUpdate(stSql);
						stSql = "update teb_epsfields set "
						    + "nmOrderDisplay=\""
						    + this.ebEnt.ebUd.request.getParameter("nmOrderDisplay_"
						        + iRecId) + "\"," + "stHandler=\""
						    + this.ebEnt.ebUd.request.getParameter("stHandler_" + iRecId)
						    + "\"," + "nmPriv="
						    + this.ebEnt.ebUd.request.getParameter("nmPriv_" + iRecId) + ""
						    + " where nmForeignId=" + iRecId2;
						stReturn += "<td align=left class=small>" + stSql + "</td>";
						this.ebEnt.dbDyn.ExecuteUpdate(stSql);
					} else {
						...
						stReturn += "<td align=right><input class=small style='width:30px' type=text name=nmFlags_"
						    + rs.getString("nmForeignId")
						    + " value=\"0x"
						    + Long.toHexString(rs.getInt("nmFlags")) + "\"></td>";
						...
						stReturn += "<td align=right><input class=small style='width:30px' type=text name=nmPriv_"
						    + rs.getString("nmForeignId")
						    + " value=\"0x"
						    + Long.toHexString(rs.getInt("nmPriv")) + "\"></td>";
				...
				for (iR = 1; iR <= iMax; iR++) {
					...
					for (int iRF = 1; iRF <= iMaxF; iRF++) {
						...
						stReturn += "<td align=right>0x"
						    + Long.toHexString(rsF.getInt("nmPriv")) + "</td>";
						...
						if (nmOrderDisplay != rsF.getInt("nmOrderDisplay")) // only update
						                                                    // if different
						{
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update teb_epsfields set nmOrderDisplay="
							        + nmOrderDisplay + " where nmForeignId="
							        + rsF.getString("nmForeignId"));
						}
						stReturn += "<tr>";
					}
				...
				if (stTa == null || stTa.trim().length() <= 0) {
					...
					for (int iL = 1; iL < aLines.length; iL++) {
						...
						if (aFields.length > 10) {
							...
							if (nmFieldId > 0) {
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update teb_epsfields set nmPriv=" + nmPriv
								        + " where nmForeignId=" + nmFieldId);
							}
						}
					}
				}
				...
				if (stTa == null || stTa.trim().length() <= 0) {
					...
					/*
					 * 6.29 Report--Budget (D29) The purpose of the Budget Report is to
					 * provide management a capsule view of how their projects are
					 * performing financially. Reqâ€™t ID Title Level Description 1
					 * Access 0 2 Project Manager 1 Project Managers shall be able to
					 * access the â€œBudget Reportâ€� for their project.
					 */
					...
					for (int iL = 1; iL < aLines.length; iL++) {
						...
						if (iState == 6 && aLines[iL].trim().endsWith(")")) {
							try {
								...
								nmDeliveryId = -1;
								this.stError += "<BR>ERROR: from teb_reqspec line " + iL + " "
								    + e;
							}
							continue;
						}
						...
						if (aFields.length > 2) {
							if (iState == 4
							    && aFields[0].trim().toUpperCase().startsWith("D")
							...
							if (iState == 6
							    && !aFields[0].trim().toUpperCase().subSequence(0, 1)
							        .equals("R")) {
								try {
									stDescription = "";
									if (aFields.length > 3) {
					...
					if (stValue.startsWith("Access")) {
						...
						if (nmTableId > 0 && stFieldList.length() > 0) {
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update teb_table set nmReportPriv ="
							        + nmPriviledge + ", stReportFieldList="
							...
							if (nmFieldId <= 0) {
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("insert into teb_fields "
								        ...
								        + "values(" + nmFieldId + ",\"" + stDbFieldName
								        + "\",\"" + stValue.trim() + "\",3,1,0,255,64,"
								        + rs.getInt("nmTableId") + ") ");
								this.ebEnt.dbDyn
								    .ExecuteUpdate("insert into teb_epsfields (nmForeignId) values("
								        + nmFieldId + ")");
						...
						if (iState == 1) {
							if (stValue.equals("Administrator")
								...
								nmPriviledge |= 0x40;
							} else if (stValue.equals("Project Portfolio Manager")
							    || stValue.equals("Project Portfolio Managers")) {
								nmPriviledge |= 0x20;
				...
				if (nmTableId > 0 && stFieldList.length() > 0) {
					this.ebEnt.dbDyn.ExecuteUpdate("update teb_table set nmReportPriv ="
					    + nmPriviledge + ", stReportFieldList="
					    + this.ebEnt.dbDyn.fmtDbString(stFieldList) + " where nmTableId="
					    + rs.getString("nmTableId"));
				}
			}
	...
	private void updateScheduleTask(String param) {
		try {
			...
			String stEstimatedHours = this.ebEnt.ebUd.request.getParameter(param
			    + "Estimated-" + stPrjId + "_" + stBaseline + "_" + stSchId);
			String stExpendedHours = this.ebEnt.ebUd.request.getParameter(param
			    + "Expended-" + stPrjId + "_" + stBaseline + "_" + stSchId);
			String stStatus = this.ebEnt.ebUd.request.getParameter(param + "Status-"
			    + stPrjId + "_" + stBaseline + "_" + stSchId);
			...
			if (rsCheck.next()) {
				...
				if (lowlvl > 0
				    && newEffort > this.rsMyDiv.getDouble("MaximumTaskHours")) {
					...
					makeMessage(
					    ...
					    this.ebEnt.ebUd.getLoginId() + ","
					        + rsCheck.getString("ProjectManagerAssignment") + ","
					        + rsCheck.getString("ProjectPortfolioManagerAssignment"),
					    "Schedule Warning",
			...
			if (nmWfStatus == 0) {
				this.ebEnt.dbDyn.ExecuteUpdate("REPLACE teb_workflow VALUE (" + stPrjId
				    + "," + stBaseline + "," + stSchId + "," + stEstimatedHours + ","
				    + stExpendedHours + "," + stStatus + ","
				    + this.ebEnt.ebUd.getLoginId() + "," + stWfStatus + ")");
				...
				if (rsP.next()) {
					String msg = "Task "
					    ...
					    + stExpendedHours
					    + "\", Status \""
					    + (nmStatus == 0 ? "Wait" : (nmStatus == 1 ? "Progress" : "Done"))
					    + "\"";
					makeMessage(
				...
				String nmUserId = this.ebEnt.dbDyn
				    .ExecuteSql1("SELECT nmUserId FROM teb_workflow WHERE nmProjectId="
				        + stPrjId + " AND nmBaseline=" + stBaseline + " AND nmSchId="
				        + stSchId);
				ResultSet rsP = this.ebEnt.dbDyn
				    .ExecuteSql("SELECT * FROM Projects Where RecId=" + stPrjId);
				if (rsP.next()) {
				...
				this.ebEnt.dbDyn.ExecuteUpdate("UPDATE teb_workflow SET nmStatus="
				    + stWfStatus + " WHERE nmProjectId=" + stPrjId + " AND nmBaseline="
				    + stBaseline + " AND nmSchId=" + stSchId);
				String stWhere = " WHERE RecId=" + stSchId + " AND nmProjectId="
				    + stPrjId + " AND nmBaseLine=" + stBaseline;
				...
				makeTask(
				    15,
				    this.epsEf.getScheduleName("" + stSchId, stPrjId,
				        Integer.parseInt(stBaseline)));
			}
		} catch (Exception e) {
		...
		try {
					...
					if (paramValues.length == 1) {
						...
						if (paramName.startsWith("actual_")) {
							try {
								...
								stSql = "update teb_allocateprj set nmActual='" + paramValue
								    + "' where " + "dtDatePrj='" + aV[1] + "' and nmUserId='"
								    + aV[2] + "' and nmPrjId='" + aV[3] + "' and nmTaskId='"
								    + aV[4] + "'";
								this.ebEnt.dbDyn.ExecuteUpdate(stSql);
							} catch (Exception e) {
								this.ebEnt.ebUd
							...
							try {
								...
								stSql = "update teb_allocateprj set nmActualApproved='"
								    + paramValue + "' where " + "dtDatePrj='" + aV[1]
								    + "' and nmUserId='" + aV[2] + "' and nmPrjId='" + aV[3]
								    + "' and nmTaskId='" + aV[4] + "'";
								this.ebEnt.dbDyn.ExecuteUpdate(stSql);
							} catch (Exception e) {
								this.ebEnt.ebUd
			...
			if (rsP != null) {
				...
				for (int iP = 1; iP <= iMax; iP++) {
					rsP.absolute(iP);
					stProjects += this.ebEnt.ebUd.addOption2(
					    rsP.getString("ProjectName"), rsP.getString("RecId"), stPrj);
				}
			}
			if (iMax == 0) {
			...
			if (iMaxActual > 0) {
				...
				stReturn += "</tr>";
				for (int iA = 1; iA <= iMaxActual; iA++) { // addOption(String stLabel,
					                                         // String stId, String
					                                         // stCurrent)
					...
					stReturn += "<td class=l1td>" + rsActual.getString("FirstName") + " "
					    + rsActual.getString("LastName") + "</td>";
					stReturn += "<td class=l1td>"
					    + this.ebEnt.ebUd.fmtDateFromDb(rsActual.getString("dtDatePrj"))
					    + "</td>";
					stReturn += "<td class=l1td align=right>"
					    + rsActual.getString("nmAllocated") + "</td>";
					stReturn += "<td class=l1td><input type=text" + " name='actual_"
					    + rsActual.getString("dtDatePrj").substring(0, 10) + "_"
					...
					if ((this.ebEnt.ebUd.getLoginPersonFlags() & 0x20) != 0) {
						stReturn += "<td class=l1td><input type=text" + " name='approve_"
						    ...
						    + rsActual.getString("nmTaskId") + "'"
						    + " size=2 style='text-align:right;' value='"
						    + rsActual.getString("nmActualApproved") + "'></td>";
					}
					stReturn += "</tr>";
				}
			...
			iCount = 0;
			if (iMax > 0) {
				for (int iR = 1; iR <= iMax; iR++) { // addOption(String stLabel, String
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					double InitialEstimatedCost = this.ebEnt.dbDyn
					    .ExecuteSql1n("select sum(SchCost) from Schedule"
					        + " where nmProjectId=" + rs.getInt("RecId")
					        + " and nmBaseline=" + iIniticalBaseline + " and SchLevel=0");
					double CurrentEstimatedCost = this.ebEnt.dbDyn
					    .ExecuteSql1n("select sum(SchCost) from Schedule"
					        + " where nmProjectId=" + rs.getInt("RecId")
					        + " and nmBaseline=" + rs.getInt("CurrentBaseline")
					        + " and SchLevel=0");
					...
					stReturn += "<td class=l1td>"
					    + (fixedStartDate != null ? dateFormatter.format(fixedStartDate)
					        : "") + "</td>";
					stReturn += "<td class=l1td>"
					    + (fixedEndDate != null ? dateFormatter.format(fixedEndDate) : "")
					    + "</td>";
					stReturn += "<td class=l1td align=right>"
					...
					double BCWP = this.ebEnt.dbDyn
					    .ExecuteSql1n("select sum(nmExpenditureToDate) from Schedule"
					        + " where nmProjectId=" + rs.getInt("RecId")
					        + " and nmBaseline=" + iIniticalBaseline
					...
					double PV = this.ebEnt.dbDyn
					    .ExecuteSql1n("select sum(SchCost) from Schedule"
					        + " where nmProjectId=" + rs.getInt("RecId")
					        + " and nmBaseline=" + iIniticalBaseline + " and lowlvl=1");
					if (PV != 0) {
				...
				if (stTemp != null && stTemp.equals("taskdetail")) {
					...
					if (rsSch.next()) {
						...
						try {
							...
							if (iM > 0) {
								stDesc = "";
								if (rsSch.getString("SchDone") != null
								    && rsSch.getString("SchDone").equals("Y")) {
									stDesc += "Status: Done<br>";
									stDesc += "Estimated Hours: "
									    + rsSch.getString("SchEstimatedEffort") + "<br>"
						...
						stReturn += "<table border='1'>"
						    ...
						    + "<tr>" + "	<td>" + rsSch.getString("ProjectName") + "</td>"
						    + "	<td>" + rsSch.getString("RecId") + "</td>" + "	<td>"
						    + rsSch.getString("SchLevel") + "</td>" + "	<td>"
						    + rsSch.getString("stDivisionName") + "</td>" + "	<td>"
						    + (aDate == null ? "&nbsp;" : formatter.format(aDate))
						    + "</td>" + "	<td>" + stDesc + "</td>" + "</tr>" + "</table>";
					}
				} else {
		...
		try {
			...
			if (this.nmTableId == 12 && stPk != null) // Projects
			{
					...
					if (stUnlock != null && stUnlock.length() > 0) {
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Projects set nmLockFlags=0 where RecId="
						        + stPk);
						stReturn += "<br>Project <b>" + rsP.getString("ProjectName")
						    + "</b> has been unlocked";
						return stReturn; // -------------------------------?
						...
						stReturn += "<td><a href=\"mailto:" + rsUser.getString("stEMail")
				...
				this.ebEnt.dbDyn.ExecuteUpdate("update Projects set nmLockFlags = 1,"
				    + "nmLockUserId= " + this.ebEnt.ebUd.getLoginId()
			                                                              ...
			                                                              // NEW USER:
			{
				...
				nmUserId++;
				this.ebEnt.dbEnterprise
				    .ExecuteUpdate("replace into X25User (RecId) values(" + nmUserId
				        + ") ");
				this.ebEnt.dbDyn
			...
			if (stPk != null && stPk.length() > 0) // primarily Login has no data
			{
				if (stChild != null && (stChild.equals("21") || stChild.equals("19"))) {
					...
					stSql = "SELECT * FROM " + rsTable.getString("stDbTableName")
					    + " where nmProjectId=" + stTemp + " " + "and nmBaseline="
					    + stBaseline + " and " + rsTable.getString("stPk") + " = \""
					    + stPk + "\"";
				} else {
					stSql = "SELECT * FROM " + rsTable.getString("stDbTableName")
			...
			if (rsTable.getInt("nmTableId") == 9) // Users
			{
				...
				stTable2 += "<tr><td class='' valign=top>User Type: </td><td align=left >"
				    + editUserType(rsU.getInt("nmPriviledge"), 1) + "</td></tr>";
				...
				stTable2 += "<tr><td class='' valign=top>Labor Categories: </td><td align=left >"
				    + makeList(97, 1, rsU.getInt("RecId")) + "</td></tr>" + "</table>";
			...
			for (int iF = 1; iF <= iMax; iF++) {
				...
				if (this.ebEnt.ebUd.getLoginId() > 0) {
					int nmPriv = rsF.getInt("nmPriv"); // only can check for logged in
					                                   // users
					iValue = (this.ebEnt.ebUd.getLoginPersonFlags() & nmPriv);
					if (iValue == 0) {
						continue; // --------------------> Not allowed, so ignore this
						          // field.
					}
				} else {
					iValue = 1;
				...
				if (rsF.getString("stSpecial") != null
					...
					stReturn += "<td colspan=2 class=\""
					    + stClass
					    + "\">"
					    + this.epsEf.editField(rsMyDiv, rsTable, rsF, rsD, null, iEnable,
					        stLabel) + "</td>";
					stReturn += "</tr>";
				} else {
					stReturn += "<tr>";
			...
			if (rsTable.getString("stChildren").trim().length() > 0) { // show
				...
				for (int i = 0; i < aV.length; i++) {
					ResultSet rsC = this.ebEnt.dbDyn
					    .ExecuteSql("select * from  teb_table where nmTableId =" + aV[i]);
					if (rsC != null) {
						rsC.absolute(1);
		...
		try {
			...
			for (int iD = 1; iD <= iMaxDiv; iD++) {
				...
				stReturn += this.ebEnt.ebUd.addOption4(
				    rsDiv.getString("stDivisionName"), rsDiv.getString("nmDivision"),
				    stDivision, i);
			}
			stReturn += "</select>";
		} catch (Exception e) {
		...
		try {
				...
				if (stDo.equals("specialday")) {
					...
					if (stSave != null || stCancel != null) { // DO SAVEDATA()
						...
						{ // Must Uncheck Lock Flag
							if (stPk != null
							    && ((stSave != null && stSave.equals("Save")) || stCancel != null)) {
								ResultSet rsP = this.ebEnt.dbDyn
								    .ExecuteSql("select * from Projects where RecId=" + stPk);
								rsP.absolute(1);
								if (rsP.getInt("nmLockFlags") != 0) {
							...
							if (rsTable.getString("stDbTableName").equals("Users")) {
									...
									for (int i = 0; i < aV.length; i++) {
										if (aV[i] != null && aV[i].trim().length() > 0) {
											if (stPrimary.length() <= 0) {
												stPrimary = aV[i]; // No primary set, make 1st one the
												                   // primary.
											}
											this.ebEnt.dbDyn
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update Users set WeeklyWorkHours = \""
								        + stWeeklyWorkHours + "\" where nmUserId=" + stPk);
								stWeeklyWorkHours = "";
						...
						if (stDo.equals("xls") || this.stChild.equals("19")
						    || this.stChild.equals("21") || this.stChild.equals("34")
						    || this.stChild.equals("46") || this.stChild.equals("26")) {
							if (!stDo.equals("xls")) {
								this.ebEnt.ebUd
								    .setRedirect("./?stAction=projects&t=12&do=xls&pk=" + stPk
						...
						if (this.stChild.length() > 0) {
							...
							if ((nmTableFlags & 0x100) != 0) {
								this.ebEnt.ebUd.setRedirect("./?stAction=projects&t="
								    + rsTable.getInt("nmRedirectToParent") + "&do=xls&pk="
								    + stPk + "&parent=" + rsTable.getInt("nmTableId")
								    + "&child=" + stChild);
								return ""; // ----------------------------------------->
							} else {
						...
						if (rsTable.getInt("nmRedirectToParent") > 0) {
							this.ebEnt.ebUd.setRedirect("./?stAction=admin&t="
							    + rsTable.getInt("nmRedirectToParent") + "&do=edit&pk="
							    + stPk);
							return ""; // ----------------------------------------->
						} else {
							stError += this.ebEnt.dbDyn.getError(); // just to see if there
							                                        // was an error
			...
			if (iMaxS > 0) {
				...
				for (int iS = 1; iS <= iMaxS; iS++) {
					...
					String stWhere = this.ebEnt.ebUd.request.getParameter("where_"
					    + rsS.getString("nmForeignId"));
					...
					case 891:
						...
						stReturn += "<td class=l1td><input type=hidden name='where_"
						    + rsS.getString("nmForeignId") + "' value='beg'>&nbsp;</td>";
						break;
					case 893:
						...
						if (iSearchByDivision == 0) {
							...
							stSearch += rsS.getString("stDbFieldName") + " in ("
							    + this.stUsersDivision + ") ";
						}
						break;
					default:
						stReturn += "<td class=l1td>" + rsS.getString("stLabel")
						    + ": </td>";
						stReturn += "<td class=l1td><input type=text name=search_"
						    + rsS.getString("nmForeignId") + " value=''></td>";
						stReturn += "<td class=l1td>"
						    + getMatchWhere("where_" + rsS.getString("nmForeignId"), "")
						    + "</td>";
						break;
					}
					stReturn += "</tr>";
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if ((nmTableFlags & 0x18) != 0) {
					...
					if ((nmTableFlags & 0x8) != 0) {
						stReturn += "<a title='Edit' href='" + stLink2
						    + "&do=edit'><img src='./common/b_edit.png'></a>";
					}
				...
				for (int iF = 1; iF <= iMaxF; iF++) {
					...
					if (rsF.getString("stHandler").contains("center")) {
					    ...
					    || rsF.getInt("nmDataType") == 1 || rsF.getInt("nmDataType") == 5
					...
					if (rsF.getInt("nmDataType") == 9) {
						...
						stValue = this.rsMyDiv.getString("stMoneySymbol") + " " + stValue
						    + " <font class=small>" + this.rsMyDiv.getString("stCurrency")
						    + "</font>";
					}
		...
		try {
			if (stFilter == null) // comes from Reports, form is already open
				stReturn = "</form><form method=post id='form"
				    + rsTable.getString("nmTableId") + "' name='form"
				    + rsTable.getString("nmTableId")
			...
			if (stFilter != null) {
				if (stFilter.length() > 5 && !stFilter.equals("Save")) {
					// makeUserSql( nmType,stLcList, stDivList, stSearchType, stSearch,
					// stWhere );
			...
			if (stLc != null && stLc.length() > 0) {
				...
				for (int iL = 1; iL <= iMaxLc; iL++) {
					rsLc.absolute(iL);
					stReturn += this.ebEnt.ebUd.addOption3(
					    rsLc.getString("LaborCategory"), "," + rsLc.getString("nmLcId")
					        + ",", stLcList);
				}
				stReturn += "</select>";
			}
			...
			stReturn += this.ebEnt.ebUd.addOption2("Search By Full Name:", "full",
			    stSearchType);
			...
			stReturn += this.ebEnt.ebUd.addOption2("Search By Email:", "stEMail",
			    stSearchType);
			stReturn += this.ebEnt.ebUd.addOption2("Search By Phone:", "Telephone",
			    stSearchType);
			stReturn += "</select></td>";
			...
			if (stFilter != null && !stFilter.equals("Save"))
				return stReturn + "</table>"; // -------------------> for REPORTS --
			...
			if (stFilter != null && stFilter.equals("Save"))
				return nmType + "^" + stLcList + "^" + stDivList + "^" + stSearchType
				    + "^" + stSearch + "^" + stWhere; // ------>
					...
					for (int iR = 1; iR <= iMax; iR++) {
						...
						if (stLookup != null && stLookup.length() > 0
							...
							stReturn1 += "<input type=submit name=userslect"
							    + rs.getString("nmUserId") + " value=Select class=small"
							    + " onClick='sendBack(\"" + rs.getString("FirstName") + " "
							...
							if ((this.ebEnt.ebUd.getLoginPersonFlags() & rsTable
							    .getInt("nmEditPriv")) != 0) {
								stReturn1 += "<a title='Edit' href='" + stLink2
								    + "&do=edit&pk=" + rs.getString("nmUserId")
						...
						stReturn1 += "<td " + stClass + ">" + rs.getString("FirstName")
						    + "</td>";
						stReturn1 += "<td " + stClass + ">" + rs.getString("LastName")
						    + "</td>";
						stReturn1 += "<td " + stClass + ">" + rs.getString("stEMail")
						    + "</td>";
						stReturn2 += "\n<option value=\"" + rs.getString("nmUserId")
	...
	public String makeList(int iF, int iType, int iUserId) {
		...
		try {
			...
			stEdit += "<table border=0><tr><td valign=top align=center>"
			    ...
			    + iF + "_list' id='f" + iF
			    + "_list' onDblClick=\"moveOptions(document.form" + this.nmTableId
			    + ".f" + iF + "_list, document.form" + this.nmTableId + ".f" + iF
			    + "_selected);\">";
			String stChecked = " ";
			for (int i = 1; i <= iMax; i++) {
				rs.absolute(i);
				stEdit += "\n<option value=\"" + rs.getString("Id") + "\" " + stChecked
				    + ">" + rs.getString("value") + "</option>";
			}
			stEdit += "</select>";
			String stDblClick = "";
			if (iType == 2) {
			...
			for (int i = 1; i <= iMaxCurrent; i++) {
				...
				stEdit += "\n<option value=\"" + rsCurrent.getString("Id") + "\" "
				    + stChecked + stClass + ">" + rsCurrent.getString("value")
				    + "</option>";
			}
			stEdit += "</select>";
			if (iType == 2) {
				stEdit += "<br/><font class=small>(Double click for primary)</font>";
				stEdit += "\n<input type=hidden name='f" + iF + "_dcvalue' id='f" + iF
				    + "_dcvalue' value=\"" + stPrimary + "\">";
				// stEdit += "<input type=hidden name='f" + iF + "_dcfield' id='f" + iF
	...
	public String getLoginPage() {
		...
		try {
			...
			// What is your favorite pet?~What is your favorite sports team?~What is
			// your favorite color?
	...
	public String getPageTitle() {
		if (this.stPageTitle.length() > 0) {
			...
			this.stPageTitle = this.stPageTitle.replace("Administration:  Projects",
			    "Project: Projects");
	...
	public String getMatchWhere(String stLabel, String stWhere) {
		...
		stReturn += this.ebEnt.ebUd.addOption2("Match beginning of word", "beg",
		    stWhere);
	...
	private String specialDay(ResultSet rsTable) {
		...
		try {
			...
			ResultSet rsHoliday = this.ebEnt.dbDyn
			    .ExecuteSql("SELECT * FROM Calendar where dtDay >= '" + iYear
			        + "-01-01' and dtDay <= '" + iYear + "-12-31' and ( nmDivision="
			        + this.rsMyDiv.getString("nmDivision") + " or nmUser = " + stPk
			        + " ) order by dtDay");
			stReturn = "</div></div></form><form method=post name=formsd id=formsd><center><h2>Select Special Day</h2>";
			stReturn += "<table><tr>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 1, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 2, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 3, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 4, rsHoliday)
			    + "</td>";
			stReturn += "</tr><tr>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 5, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 6, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 7, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 8, rsHoliday)
			    + "</td>";
			stReturn += "</tr><tr>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 9, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 10, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 11, rsHoliday)
			    + "</td>";
			stReturn += "<td valign=top>" + getCalendarMonth(iYear, 12, rsHoliday)
			    + "</td>";
			stReturn += "</tr>";
			stReturn += "</table>";
			...
			for (int iD = 1; iD <= iMax; iD++, iPos++) {
				...
				if (aWorkDays[(iPos % 7)] == 0) {
					...
					if (iFound > 0) {
						...
						stEdit += "<td>";
						stEdit += "\n<a href='#' onClick=\"selectSpecialDay2( '" + iMonth
						    + "/" + iD + "/" + iYear + "');\">" + iD + "</a></td>";
					}
				}
			}
	...
	private void saveSpecialDays(int nmFieldId, String stValue, String stPk) {
		try {
			...
			if (stDel != null && stDel.trim().length() > 1) {
				...
				for (int i = 0; i < aV.length; i++) {
					if (aV[i].length() > 0) {
						this.ebEnt.dbDyn.ExecuteUpdate("delete from Calendar where RecId="
						    + aV[i]);
					}
				}
			}
				...
				if (aSave[iL].length() > 0) {
					...
					int iRecId = this.ebEnt.dbDyn
					    .ExecuteSql1n("select max(RecId) from Calendar where dtDay = \""
					        + this.ebEnt.ebUd.fmtDateToDb(aV[3]) + "\" and nmUser="
					        + stPk);
					if (iRecId <= 0) {
		...
		try {
			...
			if (stSave != null && stSave.length() > 0) {
				if (!stSave.contains("Cancel")) {
					for (int iR = 1; iR <= iMax; iR++) {
						rs.absolute(iR);
						String stFlags = this.ebEnt.ebUd.request.getParameter("approve"
						    + rs.getString("RecId"));
						if (stFlags != null && stFlags.length() > 0) {
							if (stFlags.equals("8")) // Declined
							{
								...
								this.ebEnt.dbDyn.ExecuteUpdate("update Calendar set nmFlags='"
								    + stFlags + "' where RecId=" + rs.getString("RecId"));
							}
						}
					}
				}
			...
			if (iMax > 0) {
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					stReturn += "<td>"
					    + this.ebEnt.ebUd.fmtDateFromDb(rs.getString("dtDay")) + "</td>";
	...
	public String validateD50(ResultSet rsTable, ResultSet rsF, String stPk,
		...
		try {
			...
			if (!this.ebEnt.ebUd.request.getParameter("stAction").equals("runeob")) {
				stSql = "SELECT * FROM requirements where nmProjectId=" + stProject
				    + " and nmBaseline=" + nmBaseline + " order by RecId desc";
				...
				for (int i = 1; i <= iSch; i++) {
					...
					if (iLastLevel == rs1.getInt("ReqLevel") && stValue.length() <= 0
					    && stId.equals(rs1.getString("RecId"))) {
						// same level as before, therefore we are LAST
						this.ebEnt.ebUd.setPopupMessage("[ID:" + rs1.getString("RecId")
					...
					/*
					 * else if(iLastLevel != rs1.getInt("ReqLevel") && stValue.length() >
					 * 0 && stId.equals(rs1.getString("RecId"))){ //parent
					 * this.ebEnt.ebUd.setPopupMessage("[ID:" + rs1.getString("RecId") +
					 * "] " + rsF.getString("stLabel") +
					 * ": Parent tasks cannot have a description."); makeMessage("All",
					 * rs.getString("ProjectPortfolioManagerAssignment")+","+rs.getString(
					 * "ProjectManagerAssignment"
					 * )+","+rs.getString("BusinessAnalystAssignment"),
					 * rs1.getString("ReqTitle"),
					 * rs.getString("ProjectName")+": Parent tasks cannot have a descriptions"
					 * , dateEnd); err = 1; break; }
					 */
			...
			if (err <= 0) {
				if (stValue.length() > 0) {
					...
					if (iCount > 1) {
						iD50Flags |= 0x1;
						this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
						    + rsF.getString("stLabel") + ": contains " + iCount
						    + " Sentences. ");
					}
					...
					if (iCount > 0) {
						iD50Flags |= 0x2;
						this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
						    + rsF.getString("stLabel") + ": contains lists or clauses.");
					} else {
						iCount = EpsStatic.countIndexOf(stValue, ":");
						if (iCount > 0) {
							iD50Flags |= 0x4;
							this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
							    + rsF.getString("stLabel") + ": contains clauses.");
						}
					}
				} else
					iWordCount = 0;
				if (iWordCount < this.rsMyDiv.getInt("ReqtsSpecificationMinimumWords")
					...
					this.ebEnt.ebUd.setPopupMessage(rsF.getString("stLabel")
					    + ": contains " + iWordCount + " words. "
					    + " Minumum words required: "
					    + this.rsMyDiv.getInt("ReqtsSpecificationMinimumWords"));
				}
				if (iWordCount > this.rsMyDiv.getInt("ReqtsSpecificationMaximumWords")) {
					iD50Flags |= 0x10;
					this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
					    + rsF.getString("stLabel") + ": contains too many (" + iWordCount
					    + ") words. " + " Maximum words: "
					    + this.rsMyDiv.getInt("ReqtsSpecificationMaximumWords"));
				}
				if (iWordCount > 0 && stWords.trim().length() > 0) {
					...
					for (int iW = 1; iW <= iMaxW; iW++) {
						...
						if (rsType.getString("stKeywordType").equals("C")) {
							iD50Flags |= 0x40;
							this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
							    + rsF.getString("stLabel") + ": contains conjunctions ");
							// this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] " +
							// rsF.getString("stLabel") + ": contains " +
		...
		try {
			...
			if (stValue.length() > 0) {
				...
				if (iVerb <= 0) {
					iD53Flags |= 0x1;
					this.ebEnt.ebUd.setPopupMessage("[ID:" + stId + "] "
					    + rsF.getString("stLabel") + ": does not begin with a verb.");
				}
			}
		} catch (Exception e) {
			...
			if (stValue.length() > 0) {
				...
				int i = this.ebEnt.dbDyn
				    .ExecuteSql1n("SELECT count(*) FROM Requirements r,Projects p"
				        ...
				        + " and r.nmProjectId="
				        + this.ebEnt.ebUd.request.getParameter("pk") + " and r.RecId="
				        + stValue);
				if (i <= 0)
					this.ebEnt.ebUd.setPopupMessage("[ID:" + stPk + "] "
					    + rsF.getString("stLabel") + ": is not a valid low level ID.");
			} else
				this.ebEnt.ebUd.setPopupMessage("[ID:" + stPk + "] "
	...
	public String validateD22(ResultSet rsTable, ResultSet rsF, String stPk,
		...
		try {
			...
			/*
			 * iDup = this.ebEnt.dbDyn.ExecuteSql1n(stSql); if (iDup > 0) { stReturn
			 * += " - Copy" + stPk;
	...
	public String validateD22(String stParam, ResultSet rs2, String stName,
		...
		try {
			...
			if (stPopupError.length() <= 0) {
				// Check UNIQE in DB
				int iDup = 0;
				ResultSet rsDup = this.ebEnt.dbDyn.ExecuteSql("select count(*) as cnt,"
				    + stPkField + " from " + stTable + " where " + stName + " = \""
				    + stValue + "\" ");
				if (rsDup != null) {
					rsDup.last();
	...
	public String validateEffort(ResultSet rsF, String stPk, String stValue,
		...
		try {
			...
			// + stProject + " and nmBaseline=" + nmBaseline + " and RecId=" + stPk);
	...
	public String runEOB() {
		...
		try {
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				// "Please assign: " + rs.getInt("cnt") + " CRITERIA for Division: " +
				// rs.getString("stDivisionName"), "7/1/2011");
				makeMessage("All",
				    getUsers(rs.getInt("nmDivision"), getPriviledge("ppm")),
				    "Criteria", "Please assign: " + rs.getInt("cnt")
			...
			if (iMax > 0) {
				for (int iR = 1; iR <= iMax; iR++) {
					...
					if (!rs.getString("FirstName").isEmpty()
					    && !rs.getString("LastName").isEmpty())
						makeMessage("All", getAllUsers(getPriviledge("ppm")),
						    "Missing User Labor Category", rs.getString("FirstName") + " "
			...
			if (iMax > 0) {
				for (int iR = 1; iR <= iMax; iR++) {
					...
					if (!rs.getString("FirstName").isEmpty()
					    && !rs.getString("LastName").isEmpty())
						makeMessage("All", getAllUsers(getPriviledge("ppm")),
						    "Missing User Work Telephone Number", rs.getString("FirstName")
						        + " " + rs.getString("LastName"), dateEnd);
				}
			}
			rs.close();
			...
			if (iMax > 0) {
				for (int iR = 1; iR <= iMax; iR++) {
					...
					if (!rs.getString("FirstName").isEmpty()
					    && !rs.getString("LastName").isEmpty())
						makeMessage("All", getAllUsers(getPriviledge("ppm")),
						    "Missing User Email Address", rs.getString("FirstName") + " "
						        + rs.getString("LastName"), dateEnd);
				}
			}
			rs.close();
					...
					if (iWordCount > this.rsMyDiv
						...
						makeMessage(
						    ...
						    rs.getString("ProjectPortfolioManagerAssignment") + ","
						        + rs.getString("ProjectManagerAssignment") + ","
						        + rs.getString("BusinessAnalystAssignment"),
						    "Requirement Warning", "Requirements cannot exceed "
						        + this.rsMyDiv.getInt("ReqtsSpecificationMaximumWords")
						        + " words", dateEnd);
					}
						...
						if (this.ebEnt.dbDyn
							...
							makeMessage(
							    ...
							    rs.getString("ProjectPortfolioManagerAssignment") + ","
							        ...
							        + rs.getString("BusinessAnalystAssignment"),
							    "Requirement Warning",
							    "Project " + rs.getString("ProjectName") + " requirement ID "
							        + rs.getString("RecId") + " is a compound statement",
							    dateEnd);
						}
					}
					iWordCount = 0;
				...
				if (this.ebEnt.dbDyn.ExecuteSql1n(stSql) == 0) {
					makeMessage(
					    ...
					    rs.getString("ProjectPortfolioManagerAssignment") + ","
					        + rs.getString("ProjectManagerAssignment") + ","
					        + rs.getString("BusinessAnalystAssignment"),
					    "Requirement Warning", "Project " + rs.getString("ProjectName")
			...
			c.add(Calendar.DATE, -1); // set calendar to date range we are checking
							...
							if (iWordCount > 0) {
								if (this.ebEnt.dbDyn
								    .ExecuteSql1n("SELECT count(*) FROM teb_dictionary where stKeyWord='"
								        + iWords[0] + "' AND stKeywordType!='V'") > 0) {
	...
	public String loadingPage() {
		...
		if (stURL != null && stURL.length() > 0) {
			stReturn = stReturn.replaceAll("~~Redirect~~", "window.location = '"
			    + stURL + "';");
		} else {
	...
	public String makeTask(int iTriggerId, String stTaskDetail) {
		...
		try {
			ResultSet rsTrigger = this.ebEnt.dbDyn
			    .ExecuteSql("select * from Triggers" + " where RecId=" + iTriggerId);
			...
			if (rsTrigger.getString("TriggerEvent").equals("Enabled")
				...
				int iTaskId = this.ebEnt.dbEnterprise
				    .ExecuteSql1n("select max(RecId) from X25Task" + " where stTitle="
				        ...
				        + " and nmTaskFlag=" + rsTrigger.getString("nmTasktype"));
				if (iTaskId <= 0) {
				...
				this.ebEnt.dbEnterprise
				    .ExecuteUpdate("delete from X25RefTask where nmTaskId=" + iTaskId
				        + " and nmRefType=42 ");
	 ...
	 * always show
	 */
	public String makeMessage(String projectName, String stUsers, String stTask,
	...
	public String getUsers(int iDivision, int nmPriv) {
		...
		try {
			...
			String stSql = "SELECT distinct u.RecId FROM "
			    ...
			    + "rd.nmDivision=" + iDivision + " and ( u.nmPriviledge & " + nmPriv
			    + " ) != 0";
			ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs.last();
			int iMax = rs.getRow();
						...
						if (stTemp != null) {
							...
							if (!stTemp.equals(nmScore)) {
								...
								this.ebEnt.dbDyn.ExecuteUpdate("update teb_project set "
								    + "stValue=" + this.ebEnt.dbDyn.fmtDbString(stTemp)
								    + ",iValue=" + iValue + ",nmValue=" + dValue
								    + " ,dtLastChanged=now(),nmUserId="
								    + this.ebEnt.ebUd.getLoginId() + " where nmProjectId="
								    + stPrj + " and nmBaseline="
								    + rs.getString("CurrentBaseline") + " and nmFieldId="
								    + rs.getString("nmForeignId"));
							}
						}
					}
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					if (rsCriteria != null && rsCriteria.getRow() > 0) {
						...
						if (stResponsibilities.contains("," + this.ebEnt.ebUd.getLoginId()
							...
							if (iWeight != 0) {
								stReturn += "<tr>";
								stReturn += "<td class=l1td>" + rs.getString("stLabel")
								    + "</td>";
								stReturn += "<td class=l1td>"
								    + rsCriteria.getString("stDivisionName") + "</td>";
								...
								for (int ii = -1; ii <= 10; ii++) {
									...
									stReturn += this.ebEnt.ebUd.addOption2(stTemp, "" + ii,
									    nmScore);
								}
								stReturn += "</select></td>";
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				this.ebEnt.dbDyn
				    .ExecuteUpdate("insert into teb_fields "
				        ...
				        + "values(" + nmFieldId + ",0x10000001,\"" + stDbFieldName
				        + "\",\"" + stValue.trim() + "\",3,0,255,64,300) ");
				this.ebEnt.dbDyn
				    .ExecuteUpdate("insert into teb_epsfields (nmForeignId) values("
				        + nmFieldId + ")");
			}
		...
		try {
			...
			for (int iF = 1; iF <= iMaxF; iF++) {
				...
				if (rsF.getInt("nmForeignId") == 352
				    && (this.ebEnt.dbDyn.ExecuteSql1n("select nmFlags from "
				        + rsTable.getString("stDbTableName") + " where "
				...
				if ((iFlags & 1) != 0) // must be editable field to SAVE IT
				{
					...
					if (rsF.getInt("nmDataType") == 21) {
						...
						stValue = "";
						String[] paramValues = this.ebEnt.ebUd.request
						    .getParameterValues("f" + rsF.getString("nmForeignId"));
					...
					if (stTemp != null && stTemp.length() > 0
						...
						stValue = validateD22(rsTable, rsF, stPk, stValue, stProject,
						    nmBaseline);
					} else if (stTemp != null && stTemp.length() > 0
					    && stTemp.toLowerCase().equals("d50")) {
						stValue = validateD50(rsTable, rsF, stPk, stValue, stProject,
						    nmBaseline);
					} else if (stTemp != null && stTemp.length() > 0
					    && stTemp.toLowerCase().equals("d53")) {
						stValue = validateD53(rsTable, rsF, stPk, stValue, stProject,
						    nmBaseline);
					} else if (stTemp != null && stTemp.length() > 0
					    && stTemp.toLowerCase().equals("rid")) {
						stValue = validateRID(rsTable, rsF, stPk, stValue, stProject,
						    nmBaseline);
					}
					if (stValue.trim().length() <= 0) {
						if (rsF.getInt("nmDataType") == 1 || rsF.getInt("nmDataType") == 5
						    || rsF.getInt("nmDataType") == 31) {
							stValue = "0";
					...
					if (rsF.getString("stDbFieldName").equals("SchFixedFinishDate")
						...
						String sql = "delete from teb_link where nmProjectId="
						    + this.ebEnt.ebUd.request.getParameter("pk")
						    + " and nmBaseline=" + bline + " and nmLinkFlags=2 "
						    + "and nmToId=" + stPk
					...
					} else if (rsF.getString("stDbFieldName")
				...
				case 47: // Indicators
					...
					iCount++;
					stSql += rsF.getString("stDbFieldName") + "= (( "
					    + rsF.getString("stDbFieldName") + " & ~(0xF000 )) | " + iMask
					    + ") ";
					break;
				}
			}
			...
			if (iCount > 0) {
				if (iPk > 0) {
					...
					case 19:
					case 21:
						stWhere += " and nmProjectId=" + stProject + " and nmBaseline="
						    + nmBaseline;
						break;
					}
					...
					case 21:
						...
						ResultSet rsDate = this.ebEnt.dbDyn
						    .ExecuteSql("select SchStartDate from  Schedule" + stWhere);
						rsDate.first();
					...
					this.epsEf.addAuditTrail(rsTable, rsOld, rsNew, stPk, stProject,
					    nmBaseline);
					switch (rsTable.getInt("nmTableId")) {
					case 9: // Users
		...
		try {
			...
			for (int iR = 1; iR <= iReqMax; iR++) {
				...
				validateD50(rsTable, rsF, rsReq.getString("RecId"),
				    rsReq.getString("ReqDescription"), rsReq.getString("nmProjectId"),
				    rsReq.getInt("nmBaseline"));
				this.ebEnt.dbDyn.ExecuteUpdate("update Requirements set nmD50Flags = "
				    + iD50Flags + " where RecId=" + rsReq.getString("RecId")
			...
			for (int iS = 1; iS <= iSchMax; iS++) {
				...
				validateD53(rsTable, rsF, rsSch.getString("RecId"),
				    rsSch.getString("SchDescription"), rsSch.getString("nmProjectId"),
				    rsSch.getInt("nmBaseline"));
				this.ebEnt.dbDyn.ExecuteUpdate("update Schedule set nmD53Flags = "
				    + iD53Flags + " where RecId=" + rsSch.getString("RecId")
