...
public class EpsReport {
	...
	public String doReport(ResultSet rsTable, EpsUserData epsUd) {
		...
		try {
			...
			String stValue = epsUd.ebEnt.ebUd.request
					.getParameter("customreport");

			String stReportId = epsUd.ebEnt.ebUd.request
			...
			stReturn += "<center><form method=post><h1>"
					+ rsTable.getString("stTableName") + "</h1><table>";
			ResultSet rsR = epsUd.ebEnt.dbDyn
					.ExecuteSql("SELECT * FROM teb_customreport where stReportType = '"
							+ rsTable.getString("nmTableId")
							+ "' order by nmDefaultReport DESC,stReportName ");
			rsR.last();
			int iMaxR = rsR.getRow();
			if (iMaxR <= 0) {
				// create default Report
				int i = epsUd.ebEnt.dbDyn
						.ExecuteSql1n("select max(RecId) from teb_customreport");
				i++;
				epsUd.ebEnt.dbDyn
						.ExecuteUpdate("replace into teb_customreport (RecId,stReportType,stReportName,nmReportFlags)"
								+ " values( "
								+ i
								+ ",'"
								+ rsTable.getString("nmTableId")
								+ "',\""
								+ rsTable.getString("stTableName") + "\",1)");
				rsR = epsUd.ebEnt.dbDyn
						.ExecuteSql("SELECT * FROM teb_customreport where stReportType = '"
								+ rsTable.getString("nmTableId")
								+ "' order by nmDefaultReport DESC,stReportName ");
				rsR.last();
			...
			if (iMaxR <= 0) {
				...
				for (int iR = 1; iR <= iMaxR; iR++) {
					rsR.absolute(iR);
					stReturn += epsUd.ebEnt.ebUd.addOption2(
							rsR.getString("stReportName"),
							rsR.getString("RecId"), "");
				}
				stReturn += epsUd.ebEnt.ebUd.addOption2(
						"--Create New Custom Report--", "-1", "");
	...
	public String epsShowSavedReports(ResultSet rsTable) {
		...
		// String stValue =
		// epsUd.ebEnt.ebUd.request.getParameter("customreport");
		String stValue = "0";
		int iValue = -1;
		...
		try {
			...
			// String stSql =
			// "select r.*,cr.stReportType,cr.stReportName from teb_reports r, teb_customreport cr where r.nmCustomReportId=cr.RecId order by r.RecId Desc";
			String stSql = "select r.*,cr.stReportType,cr.stReportName from teb_reports r, teb_customreport cr where r.nmCustomReportId=cr.RecId"
					+ (iValue > 0 ? " and r.nmCustomReportId=" + iValue : "")
					+ " and cr.stReportType="
			...
			if (rs != null) {
				...
				// String stClass =
				// "style='background-color:yellow;font-size:12px;padding:0px 10px 0px 10px;'";
				// <th " + stClass + ">Report Type</th>
				// stReturn +=
				// "<table border=0 bgcolor=white cellspacing=1 cellpadding=2>"
				stReturn += "<table class=l1tableb>" + "<tr>" + "<th "
						+ stClass + ">Report Title</th><th " + stClass
						+ ">Run Time</th>" + "<th " + stClass
						+ " colspan=2>View Report Format</th><th " + stClass
						+ ">&nbsp;</th></tr>";
				// <th " + stClass + " colspan=2>Download Report</th>
				// stClass =
				// "style='background-color:white;font-size:10px;padding:0px 10px 0px 10px;'";
				stClass = "class=l1td";
				for (int iR = 1; iR <= iMax; iR++) {
					rs.absolute(iR);
					stReturn += "<tr>";
					// stReturn += "<td " + stClass + ">" +
					// rs.getString("stReportType") +
					// "</td>";
					stReturn += "<td " + stClass + ">"
							+ rs.getString("stReportName") + "</td>";
					stReturn += "<td " + stClass + ">" + rs.getString("dtRun")
							+ "</td>";
					stReturn += "<td " + stClass
							+ "><a target=_blank href='./?stAction=reports&t="
							+ stT + "&reportid=" + rs.getString("RecId")
							+ "&format=html'>HTML</a></td>";
					stReturn += "<td " + stClass
							+ "><a href='./?stAction=reports&customreport="
							+ stValue + "&t=" + stT + "&reportid="
							+ rs.getString("RecId")
							+ "&format=excel'>Excel</a></td>";
					stReturn += "<td "
							+ stClass
							+ "><a href='./?stAction=reports&customreport="
							+ stValue
							...
							+ "&reportid="
							+ rs.getString("RecId")
							+ "&format=delete'  onclick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
					stReturn += "</tr>";
				}
				stReturn += "</table>";
			}
	...
	public String epsFormatSavedReports(String stReportId, EbEnterprise ebEnt) {
		...
		try {
			...
			String stCustomReportId = ebEnt.ebUd.request
					.getParameter("customreport");

			if (stTemp != null && stTemp.equals("delete")) {
				ebEnt.dbDyn
						.ExecuteUpdate("delete from teb_reports where RecId="
								+ stReportId);
				ebEnt.ebUd.setRedirect("./?stAction=reports&customreport="
						+ stCustomReportId + "&t=" + stT + "&submit2=View");
				return ""; // ----------------------------->
			}

			String stSql = "select * from teb_reports r, teb_customreport cr where r.nmCustomReportId=cr.RecId and r.RecId= "
					+ stReportId;
			...
			if (rs != null) {
				...
				/*
				 * //project name //get filtered projects String pIDs = "";
				 * String prjName = ""; String[] prjArr = null; prjArr =
				 * rs.getString("prjFilter").split(","); if(prjArr != null &&
				 * prjArr.length > 0){ for(int i=0; i<prjArr.length; i++){
				 * if(!prjArr[i].equals("")) pIDs += "RecId = " + prjArr[i] +
				 * " or "; } if(!pIDs.equals("")){ pIDs = " where " +
				 * pIDs.substring(0, pIDs.length()-4); } } ResultSet rsPID =
				 * ebEnt.dbDyn.ExecuteSql("select ProjectName from projects" +
				 * pIDs); while(rsPID.next()){ prjName +=
				 * rsPID.getString("ProjectName") + ", "; }
				 * if(!prjName.equals("")){ prjName = prjName.substring(0,
				 * prjName.length()-2); }
				 */

				// stPageTitle = rs.getString("stReportType") +
				// " Report &nbsp;&nbsp;&nbsp; Title: <b>" +
				// rs.getString("stReportName") + "</b> Run: " +
				// rs.getString("dtRun");
				stPageTitle = "<b>" + rs.getString("stReportName")
						+ "</b> &nbsp;&nbsp;<i>Run: " + rs.getString("dtRun")
						+ "</i>";
				stReturn += "<center><table border=1>";
				String[] aTdClass = null;
				ResultSet rs2 = ebEnt.dbDyn
						.ExecuteSql("select * from teb_reportcolumns rc,teb_fields f, teb_epsfields ef"
								+ " where rc.nmFieldId=f.nmForeignId and f.nmForeignId=ef.nmForeignId"
								+ " and stShow='Y' and nmCustomReportId="
								+ rs.getString("nmCustomReportId")
								+ " order by rc.nmOrder");
				rs2.last();
				int i2 = rs2.getRow();
				for (int iL = 0; iL < aLines.length; iL++) {
					...
					if (iL == 0) {
						...
						for (int iF = 1; iF < aFields.length && iF <= i2; iF++) {
							aTdClass[iF] = "";
							rs2.absolute(iF);
							if (rs2.getString("stClass").length() > 0
									&& !rs2.getString("stClass").equals("null"))
								aTdClass[iF] += " class=\""
										+ rs2.getString("stClass") + "\" ";
							if (rs2.getString("stCustom").length() > 0
									&& !rs2.getString("stCustom")
											.equals("null"))
								aTdClass[iF] += " style=\""
										+ rs2.getString("stCustom") + "\" ";
							if (aTdClass[iF].length() <= 0) {
								...
								/*
								 * if (rs2.getInt("nmDataType") == 1 ||
								 * rs2.getInt("nmDataType") == 5 ||
								 * rs2.getInt("nmDataType") == 31) { if
								 * (rs2.getString("stHandler").length() <= 0)
								 * aTdClass[iF] = " align=right "; }
								 */
								if (rs2.getInt("nmDataType") == 1
										|| rs2.getInt("nmDataType") == 5
										|| rs2.getInt("nmDataType") == 31) {
						...
						/* AS -- 19Oct2011 -- Issue # 55 */
						// stReturn += "<tr><th colspan=" + (aFields.length - 1)
						// +
						// " align=center>" + stPageTitle + "</th></tr>";
						stReturn += "<tr><th colspan=" + (aFields.length - 1)
								+ " align=center class=l1th>" + stPageTitle
								+ "</th></tr>";

						/*
						 * //display project filters if any projects are
						 * filtered if(rs.getString("prjFilter") != ""){
						 * stReturn += "<tr><th colspan=" + (aFields.length - 1)
						 * + " align='left'>Project " +
						 * rs.getString("stReportName") + " Report for project "
						 * + prjName + "</th></tr>"; }
						 */
					}

					// add project name immediately before project info from
					// project
					// filter #105
					// TODO: TanPD remove group line
					// if(!"89".equals(stT) &&
					// !aFields[aFields.length-1].equals(currPrj)
					// && iL > 0 && iL < (aLines.length-1)){
					// stReturn += "<tr><td colspan=" + (aFields.length - 1) +
					// "><strong>"
					// + aFields[aFields.length-1].replace("`", "<br>") +
					// "</strong></td></tr>";
					// currPrj = aFields[aFields.length-1];
					// }

					stReturn += "<tr>";
					// ~Project Name^123~Synergy With Organization^1088~Synergy
					// With Other
					// Projects^1089~Total Ranking Score^998|~EPS Office
					// Building~27~24~3370|~EPPORA Training
					// Sessions~21~21~3179|~EPPORA
					// SQL Server Conversion Suite~18~18~2950|~EPPORA Source
					// Analysis~15~15~2749|~EPPORA Resource
					// Modeling~12~12~2546|~EPPORA
					// Project Templates~9~9~2365|~EPPORA Project Manager
					// Scheduling
					// Enhancer~24~27~2192|~EPPORA Primavera Conversion
					// Suite~6~6~2022|~EPPORA Performance
					// Enhancer~3~3~1871|~EPPORA Oracle
					// Conversion Suite~6~6~1833|~EPPORA Online
					// Training~9~9~1761|~EPPORA
					// Niku Workbench Conversion Suite~12~12~1655|~EPPORA MySQL
					// Conversion
					// Suite~15~15~1511|~EPPORA Microsoft Project Conversion
					// Suite~30~0~1337|~EPPORA Implementation~27~27~1148|~EPPORA
					// Estimation Analyzer~18~18~852|~EPPORA DB2 Conversion
					// Suite~21~21~650|~EPPORA Comprehensive Project Test
					// Tool~24~24~418|~EPPORA Automated Test Suite~18~21~156|
					for (int iF = 1; iF < aFields.length && iF <= i2; iF++) {
						if (iL == 0) {
							String[] aV = aFields[iF].trim().split("\\^", -1);
							/* AS -- 19Oct2011 -- Issue # 55 */
							// stReturn += "<th>" + aV[0] + "</td>";
							stReturn += "<th STYLE='font-size: 12px; background-color:yellow;'>"
									+ aV[0] + "</td>";
						} else {
							stReturn += "<td "
									+ aTdClass[iF]
									+ ">"
									+ (aFields[iF].replace("`", "<br>").equals(
											"") ? "&nbsp;" : aFields[iF]
											.replace("`", "<br>")) + "</td>";
						}
					}
				...
				stOut = "<html><head><title>" + stPageTitle
	...
	public String customReportDesigner(ResultSet rsTable) {
		...
		try {
			...
			String stReportName = epsUd.ebEnt.ebUd.request
					.getParameter("stReportName");
			if (stTemp != null && stTemp.length() > 0)
				giSubmitId = Integer.parseInt(stTemp);
			boolean isChecked = false;
			boolean isDefaultReport = false;
			String stCanEdit = "";
			...
			do {
				if (nmCustomReportId <= 0) {
					...
					nmCustomReportId = epsUd.ebEnt.dbDyn
							.ExecuteSql1n("select max(RecId) from teb_customreport");
					nmCustomReportId++;
					epsUd.ebEnt.dbDyn
							.ExecuteUpdate("replace into teb_customreport (RecId,stReportName,stReportType, prjFilter) values "
									+ "("
									+ nmCustomReportId
									+ ","
									+ epsUd.ebEnt.dbDyn.fmtDbString((stReportName != null
											&& stReportName.length() > 0 ? stReportName
											: "New "
													+ rsTable
															.getString("stTableName")
													+ " - " + nmCustomReportId))
									+ ",\""
									+ rsTable.getString("nmTableId")
									+ "\", \"\") ");
				}
				rsCr = epsUd.ebEnt.dbDyn
						.ExecuteSql("SELECT * FROM teb_customreport where RecId="
								+ nmCustomReportId);
				rsCr.absolute(1);
				isChecked = true;

				isDefaultReport = (rsCr.getInt("nmDefaultReport") > 0);
				stCanEdit = epsUd.ebEnt.ebUd.request.getParameter("canedit");
				isCanEdit = (stCanEdit != null && stCanEdit.length() > 0);
				if (isDefaultReport && nmCustomReportId > 0
						&& "Save".equals(stSubmit)
						&& !rsCr.getString("stReportName").equals(stReportName)) {
					nmCustomReportId = 0;
					isChecked = false;
				}
			} while (!isChecked);
			...
			case 72: // Projects
				stSql = "select * from teb_fields f, teb_epsfields ef left join teb_reportcolumns rc"
						+ " on ef.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = "
						+ nmCustomReportId
			...
			case 56: // Division
				...
				/*
				 * stSql =
				 * "select * from teb_fields f, teb_epsfields ef left join teb_reportcolumns rc"
				 * +
				 * " on ef.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = "
				 * + nmCustomReportId +
			...
			case 71: // Project Ranking
				stSql = "select * from teb_fields f left join teb_reportcolumns rc"
						+ " on f.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = "
						+ nmCustomReportId
						+ " where  ( f.nmTabId="
						+ rsTable.getString("nmTableId")
						+ " or "
			...
			for (iF = 1; iF <= iMaxF; iF++) {
				rsF.absolute(iF);
				epsUd.ebEnt.dbDyn
						.ExecuteUpdate("replace into teb_reportcolumns (nmCustomReportId,nmFieldId,nmWidth,nmOrder) "
								+ "values("
								+ nmCustomReportId
								+ ",\""
								+ rsF.getString("nmForeignId")
								+ "\","
								+ rsF.getString("nmCols") + "," + iF + ") ");
			}
			if (giSubmitId > 0 && giSubmitId < 2000) {
				// Need to reorder.
				int iOrig = epsUd.ebEnt.dbDyn
						.ExecuteSql1n("select nmOrder from teb_reportcolumns "
								+ "where nmCustomReportId=" + nmCustomReportId
								+ " and nmFieldId=" + giSubmitId);
				stTemp = epsUd.ebEnt.ebUd.request.getParameter("order_"
						+ giSubmitId);
				int iNew = Integer.parseInt(stTemp);
				if (iNew < iOrig) { // 1 2 3 4 5 -> 3 1 2 4 5
					epsUd.ebEnt.dbDyn
							.ExecuteUpdate("update teb_reportcolumns set nmOrder = ( nmOrder + 1 )"
									+ " where nmOrder >= "
									+ iNew
									+ " and nmOrder <= "
									+ iOrig
									+ " and nmCustomReportId="
									+ nmCustomReportId);
				} else {
					epsUd.ebEnt.dbDyn
							.ExecuteUpdate("update teb_reportcolumns set nmOrder = ( nmOrder - 1 )"
				...
				epsUd.ebEnt.dbDyn
						.ExecuteUpdate("update teb_reportcolumns set nmOrder = "
								+ iNew
								+ " where nmFieldId = "
								+ giSubmitId
								+ " and nmCustomReportId=" + nmCustomReportId);
			}

			switch (rsTable.getInt("nmTableId")) {
			case 76: // project requirements omit project name
				rsF = epsUd.ebEnt.dbDyn
						.ExecuteSql("select rc.*,f.stLabel from teb_reportcolumns rc, teb_fields f "
								+ " where rc.nmFieldId=f.nmForeignId"
								+ " and rc.nmCustomReportId="
								+ nmCustomReportId
								+ " and rc.nmFieldId <> 116 order by rc.stShow desc, rc.nmOrder asc ");
				break;
			...
			default:
				rsF = epsUd.ebEnt.dbDyn
						.ExecuteSql("select rc.*,f.stLabel from teb_reportcolumns rc, teb_fields f "
								...
								+ " and rc.nmCustomReportId="
								+ nmCustomReportId
								+ " order by rc.stShow desc, rc.nmOrder asc ");
			}
			...
			if ((stSubmit != null && stSubmit.length() > 0)
					|| (giSubmitId > 0 && giSubmitId < 2000)) {
				for (iF = 1; iF <= iMaxF; iF++) {
					rsF.absolute(iF);
					int nmFieldId = rsF.getInt("nmFieldId");
					String stShow = epsUd.ebEnt.ebUd.request
							.getParameter("show_" + nmFieldId);
					if (stShow == null)
						stShow = "N";
					else
						stShow = "Y";

					stSql = "update teb_reportcolumns set stShow = \""
							+ stShow
							+ "\""
							+ ",stClass = \""
							+ epsUd.ebEnt.ebUd.request.getParameter("class_"
									+ nmFieldId)
							+ "\""
							+ ",stCustom = \""
							+ epsUd.ebEnt.ebUd.request.getParameter("custom_"
									+ nmFieldId)
							+ "\""
							+ ",stShort = \""
							+ epsUd.ebEnt.ebUd.request.getParameter("short_"
									+ nmFieldId) + "\"";
					if (epsUd.ebEnt.ebUd.request.getParameter("width_"
							+ nmFieldId) != null
							&& epsUd.ebEnt.ebUd.request.getParameter(
									"width_" + nmFieldId).length() > 0)
						stSql += ",nmWidth = "
								+ epsUd.ebEnt.ebUd.request
										.getParameter("width_" + nmFieldId);
					stSql += " where nmFieldId = " + nmFieldId
							+ " and nmCustomReportId=" + nmCustomReportId;
				...
				if (stSubmit != null) {
					if (stSubmit.equals("Save")) {
						if (nmCustomReportId > 3) {
							stPopupError += epsUd.validateD22("stReportName",
									rsCr, "stReportName", "teb_customreport",
									"RecId");
							if (stPopupError.length() <= 0) {
								String stFilter = this.epsUd.userSearch(
										rsTable, "Save");

								// update project filters
								String[] aV = epsUd.ebEnt.ebUd.request
										.getParameterValues("fprojects_selected");
								String prjFilter = "";
								if (aV != null && aV.length > 0) {
									for (int i = 0; i < aV.length; i++) {
										prjFilter += aV[i] + ",";
									}
									prjFilter = prjFilter.substring(0,
											prjFilter.length() - 1);
								}
								epsUd.ebEnt.dbDyn
										.ExecuteUpdate("update teb_customreport set "
												+ " stReportName = \""
												+ stReportName
												+ "\""
												+ ",stFilter = "
												+ epsUd.ebEnt.dbDyn
														.fmtDbString(stFilter)
												+ ",stReportCss = "
												+ epsUd.ebEnt.dbDyn
														.fmtDbString(epsUd.ebEnt.ebUd.request
																.getParameter("stReportCss"))
												+ ",prjFilter = "
												+ epsUd.ebEnt.dbDyn
														.fmtDbString(prjFilter)
												+ " where RecId = "
												+ nmCustomReportId);
								epsUd.ebEnt.ebUd
										.setRedirect("./?stAction=reports&t="
												+ rsTable
														.getString("nmTableId"));
								return ""; // ----------------------------->
							}
						...
						epsUd.ebEnt.ebUd.setRedirect("./?stAction=reports&t="
								+ rsTable.getString("nmTableId"));
						return "";
					}
				}
			...
			stReturn = "\n</form><form method=post name='form"
					...
					+ epsUd.ebEnt.ebUd.request.getParameter("submit2")
					+ "'>"
					+ "<input type=hidden name=customreport id=customreport value='"
					+ nmCustomReportId + "'>";
			if (nmCustomReportId > 3) {
				stReturn += "<h2 style=\"font-size: 14pt;\">Report Content</h2>"
						+ "Report Title: <input type=text name=stReportName "
						+ (!isCanEdit && isDefaultReport ? "readonly " : "")
						+ "value=\""
						+ epsUd.ebEnt.ebUd.getMyDb("stReportName", rsCr,
								"stReportName") + "\" size=64>";
			}
			stReturn += "<br/>&nbsp;"
					+ "<font color=red>"
					+ stPopupError
					+ "</font><br/><table border=1 style=\"background-color:white\">";
			...
			for (iF = 1; iF <= iMaxF; iF++) {
				...
				stSql = "update teb_reportcolumns set nmOrder= " + iF
						+ " where nmCustomReportId=" + nmCustomReportId
						+ " and nmFieldId=\"" + nmFieldId + "\"";
				epsUd.ebEnt.dbDyn.ExecuteUpdate(stSql); // Just for safety
														// sakes. Should
														// be ok though.
				...
				stReturn += "\n<td align=center>"
						+ this.orderSelect("order_" + nmFieldId, iF, iMaxF,
								rsTable.getInt("nmTableId"), nmFieldId)
						+ "</td>";
				stReturn += "<td align=left>" + rsF.getString("stLabel")
						+ "</td>";
				...
				stReturn += "<td align=center><input type=checkbox id=show_"
						+ nmFieldId + " name=show_" + nmFieldId + " value="
						+ nmFieldId + " " + stChecked + "></td>";
				/* AS -- 19Oct2011 -- Issue # 64 */
				vecshow.add(nmFieldId);
				if (nmCustomReportId <= 3)
					stReturn += "<td align=left><input type=text name=width_"
							+ nmFieldId
							+ " value=\""
							+ epsUd.ebEnt.ebUd.getMyDb("width_" + nmFieldId,
									rsF, "nmWidth")
							+ "\" size=3 style=\"text-align:right\" ></td>";
				// stReturn += "<td align=left>" +
				// epsUd.ebEnt.ebUd.getClass(nmFieldId,
				// epsUd.ebEnt.ebUd.getMyDb("class_" + nmFieldId, rsF,
				// "stClass")) +
				// "</td>";
				else
					stReturn += "<td align=left><input type=text name=custom_"
							+ nmFieldId
							+ " value=\""
							+ epsUd.ebEnt.ebUd.getMyDb("custom_" + nmFieldId,
									rsF, "stCustom") + "\" size=40 ></td>";
				stReturn += "<td align=left><input type=text name=short_"
						+ nmFieldId
						+ " value=\""
						+ epsUd.ebEnt.ebUd.getMyDb("short_" + nmFieldId, rsF,
								"stShort") + "\" size=15 ></td>";
				stReturn += "</tr>";
			}
			...
			for (int checkcount = 0; checkcount < vecshow.size(); checkcount++) {
				strjsbuilder += "document.getElementById('show_"
						+ vecshow.get(checkcount) + "').checked = true;";
			}
			strjsbuilder += "} else{";
			for (int checkcount = 0; checkcount < vecshow.size(); checkcount++) {
				strjsbuilder += "document.getElementById('show_"
						+ vecshow.get(checkcount) + "').checked = false;";
			}
			strjsbuilder += "}}";
			strjsbuilder += "</script>";
			...
			String stFilter = epsUd.ebEnt.dbDyn
					.ExecuteSql1("select stFilter from teb_customreport where RecId="
							+ nmCustomReportId);
			...
			case 55:
				stReturn += "<br>&nbsp;<br>"
						+ this.epsUd.divisionFilter(rsTable, stFilter);
				break;
			...
			case 69: // Prooductivity Report
				stReturn += "<br>&nbsp;<br>"
						+ this.epsUd.userSearch(rsTable, stFilter);
				break;
			...
			case 81: // Project Schedule users
				stReturn += this.epsUd.epsEf.selectProjects(rsTable,
						nmCustomReportId);
				stReturn += "<br>&nbsp;<br>"
						+ this.epsUd.userSearch(rsTable, stFilter);
				break;
			...
			String stCss = epsUd.ebEnt.dbDyn
					.ExecuteSql1("select stReportCss from teb_customreport where RecId="
							+ nmCustomReportId);
			if (stCss == null || stCss.trim().length() <= 0
					&& !stCss.equals("null"))
			...
			if (rsTable.getInt("nmTableId") == 76) { // project requirement
														// submit
				stReturn += "<tr><td align=center colspan=2>"
						...
						+ "</td></tr></table>";
			} else if (rsTable.getInt("nmTableId") == 81) { // project schedule
															// submit
				stReturn += "<tr><td align=center colspan=2>"
						+ "<input type=submit name=savedata value='Save' onclick='selectAllOptions(document.getElementById(\"fprojects_selected\"))'>&nbsp;&nbsp;&nbsp;"
						+ "<input type=submit name=savedata value='Cancel'>"
						+ "</td></tr></table>";
			} else if (rsTable.getInt("nmTableId") == 72) { // projects report
															// submit
				stReturn += "<tr><td align=center colspan=2>"
						+ "<input type=submit name=savedata value='Save' onclick='selectAllOptions(document.getElementById(\"fprojects_selected\"))'>&nbsp;&nbsp;&nbsp;"
						+ "<input type=submit name=savedata value='Cancel'>"
						+ "</td></tr></table>";
			} else if (rsTable.getInt("nmTableId") == 78) { // project
															// requirement
															// analysis report
															// submit
				stReturn += "<tr><td align=center colspan=2>"
						...
						+ "<input type=submit name=savedata value='Cancel'>"
						+ "</td></tr></table>";
			} else if (rsTable.getInt("nmTableId") == 83) { // projects schedule
															// analysis submit
				stReturn += "<tr><td align=center colspan=2>"
						+ "<input type=submit name=savedata value='Save' onclick='selectAllOptions(document.getElementById(\"fprojects_selected\"))'>&nbsp;&nbsp;&nbsp;"
						+ "<input type=submit name=savedata value='Cancel'>"
	...
	public String orderSelect(String stName, int iC, int iLength,
			int nmTableId, int iField) {
		String stReturn = "<select name=\"" + stName + "\" id=\"" + stName
				+ "\" onChange=\"setSubmitId(" + iField + ");document.form"
				+ nmTableId + ".submit();\">";
		String stChecked = "";
		for (int i = 1; i <= iLength; i++) {
			...
			stReturn += "<option value=\"" + i + "\" " + stChecked
					+ " size=3 style=\"text-align:right\" >" + i + "</option>";
		}
		stReturn += "</select>";
		return stReturn;
	}
	...
	private String runReport(ResultSet rsTable) {
		...
		if (nmCustomReportId > 0) {
			try {
				ResultSet rsR = null;
				int iMaxR = 0;
				ResultSet rsF = epsUd.ebEnt.dbDyn
						.ExecuteSql("select rc.*,f.*,ef.stHandler"
				...
				String stFilter = epsUd.ebEnt.dbDyn
						.ExecuteSql1("select stFilter from teb_customreport where RecId="
								+ nmCustomReportId);
				String prjFilter = epsUd.ebEnt.dbDyn
						.ExecuteSql1("select prjFilter from teb_customreport where RecId="
								+ nmCustomReportId);
				String stReportName = epsUd.ebEnt.dbDyn
						.ExecuteSql1("select stReportName from teb_customreport where RecId="
								+ nmCustomReportId);
				String[] prjArr = null;
				String pIDs = "";
				String stDivList = "";
				...
				case 76: // Project Requirement
					...
					if (prjArr != null && prjArr.length > 0) {
						...
						if (!pIDs.equals("")) {
							pIDs = " and ("
									+ pIDs.substring(0, pIDs.length() - 4)
									+ ")";
						}
					}
				...
				case 81: // Project Schedule
					/*
					 * stSql =
					 * "select p.ProjectName,s.*, count(*) cnt, sum(l.nmPercent) nmPercent"
					...
					/*
					 * stSql =
					 * "select p.ProjectName,s.* from Projects p, Schedule s" +
					 * " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline"
					 * +
					 * " group by p.ProjectName,s.RecId order by p.ProjectName,s.SchId"
					 * ; rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					 * rsR.last(); iMaxR = rsR.getRow(); for (int iR = 1; iR <=
					 * iMaxR; iR++) { rsR.absolute(iR); stReport +=
					 * getReportFields(rsR, rsF, iMaxF) + "|"; }
					 */
					...
					if (prjFilter != null && prjFilter.length() > 0) {
						...
						if (prjArr != null && prjArr.length > 0) {
							for (int i = 0; i < prjArr.length; i++) {
								if (!prjArr[i].equals(""))
									pIDs += "p.RecId = " + prjArr[i] + " or ";
								// pIDs += "nmPrjId = " + prjArr[i] + " OR ";
							}
							if (!pIDs.equals("")) {
								pIDs = "AND ("
										+ pIDs.substring(0, pIDs.length() - 4)
										+ ")";
								// pIDs = "(" + pIDs.substring(0,
								// pIDs.length()-4) + ")";
							}
						}
					}
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						String[] aV = stFilter.split("\\^", -1);
						int nmType = Integer.parseInt(aV[0]);
						stSql = this.epsUd.makeUserSql(nmType, aV[1], aV[2],
								aV[3], aV[4], aV[5]);
					} else {
						stSql = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,",
								"full", "", "beg"); // Do
													// all
					}
					...
					for (int iR = 1; iR <= iMaxR; iR++) {
						...
						// stusrs += "nmUserId=" + rsR.getInt("nmUserId") +
						// " OR ";
					}

					/*
					 * //create user and project filter for query
					 * if(stusrs.length() > 0 && pIDs.length() > 0) filters =
					 * " WHERE (" + stusrs.substring(0, stusrs.length()-4) +
					 * ") AND " + pIDs; else if(stusrs.length() > 0 &&
					 * pIDs.length() <= 0) filters = " WHERE " +
					 * stusrs.substring(0, stusrs.length()-4); else
					 * if(stusrs.length() <= 0 && pIDs.length() > 0) filters =
					 * " WHERE " + pIDs;
					 * 
					 * //Look through allocated tasks an display based on
					 * filters #103 stSql =
					 * "SELECT DISTINCT nmUserId, nmTaskId, nmPrjId FROM teb_allocateprj"
					 * + filters; ResultSet rsA =
					 * this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql); rsA.last(); int
					 * iMaxA = rsA.getRow(); for(int i=1; i<= iMaxA; i++){
					 * rsA.absolute(i); stSql =
					 * "select p.ProjectName,s.* from Projects p, Schedule s" +
					 * " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline "
					 * + "and p.RecId=" + rsA.getInt("nmPrjId") +
					 * " and s.RecId=" + rsA.getInt("nmTaskId") +
					 * " group by p.ProjectName,s.RecId order by p.ProjectName,s.SchId"
					 * ; rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					 * rsR.absolute(1); stReport += getReportFields(rsR, rsF,
					 * iMaxF) + "|"; }
					 */
					...
					for (int iR = 1; iR <= iMaxR; iR++) {
						...
						if (!stlb.equals("")) {
							...
							if (!aFields[3].equals("")) {
								...
								stSql = "select count(*) from users u, teb_reflaborcategory rlc where rlc.nmRefType=42 and rlc.nmRefId=u.nmUserId and rlc.nmLaborCategoryId="
										+ aFields[0]
										+ " and rlc.nmRefId IN ("
										+ stUsers + ")";
				...
				case 69: // Productivity
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						String[] aV = stFilter.split("\\^", -1);
						int nmType = Integer.parseInt(aV[0]);
						stSqlUser1 = this.epsUd.makeUserSql(nmType, aV[1],
								aV[2], aV[3], aV[4], aV[5]);
					} else {
						stSqlUser1 = this.epsUd.makeUserSql(0xFFFF, ",0",
								",0,", "full", "", "beg"); // Do
															// all
					}
					stSqlUser1 = stSqlUser1.replace(
							"u.*,xu.stEMail,xu.nmPriviledge,xu.RecId",
							"u.nmUserId");
				...
				case 72: // Projets
					...
					getBudgetReport();
					stSql += "SELECT * FROM Projects p" + pIDs
				...
				case 55: // Criteria
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						String[] aV = stFilter.split("\\^", -1);
						int nmType = Integer.parseInt(aV[0]);
						// stSqlUser = this.epsUd.makeUserSql(nmType, aV[1],
						// aV[2], aV[3],
						// aV[4], aV[5]);
						stDivList = aV[2];
						if (!stDivList.contains(",0,")) {
							stDivList = stDivList.replace(",,", ",");
							stDivList = stDivList.substring(1,
									stDivList.length() - 1);
					...
					// stSql +=
					// "SELECT * FROM Criteria c, teb_division d where c.nmDivision = d.nmDivision order by stDivisionName,c.CriteriaName";
					stSql += "select c.RecId,c.nmDivision,c.nmFlags,c.CriteriaName,c.WeightImportance,IFNULL(d.stDivisionName,'') AS stDivisionName, IFNULL(GROUP_CONCAT(DISTINCT CONCAT(u.FirstName, ' ', u.LastName) SEPARATOR '<br>'),'') as Responsibilities"
							...
							+ (stDivList.length() > 0 ? " and d.nmDivision in ("
									+ stDivList + ")"
									: "")
				...
				case 56: // Division
					...
					stSql += "SELECT * FROM teb_division"
							+ (stDivList.length() > 0 ? " where nmDivision in ("
									+ stDivList + ")"
				...
				case 53: // Cost Effectiveness
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSqlUser = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,",
								"full", "", "beg"); // Do
													// all
					}
					stSqlUser = stSqlUser.replace(
							"u.*,xu.stEMail,xu.nmPriviledge,xu.RecId",
							"u.nmUserId");
					...
					for (int iR = 1; iR <= iMaxR; iR++) {
						...
						for (int i = 1; i < fieldArr.length; i++) {
							...
							// int divid =
							// this.epsUd.ebEnt.dbDyn.ExecuteSql1n("select nmDivision from teb_refdivision where nmRefType=42 and nmRefId="
							// + rsR.getString("nmUserId"));
							// ResultSet rs2 =
							// this.epsUd.ebEnt.dbDyn.ExecuteSql("select stCurrency, stMoneySymbol from teb_division where nmDivision="
							// + divid);
							// rs2.last();
							//
							...
							// String rate =
							// getExchangeRate("http://finance.yahoo.com/d/quotes.csv?e=.csv&f=sl1d1t1&s=USD"+rs2.getString("stCurrency")+"=X");
							// String[] resp = rate.split(",");
							//
							// DecimalFormat df = new
							// DecimalFormat(" #,###,###,##0.00");
							// Double vald =
							// Double.parseDouble(fieldArr[i].substring(2)) *
							// Double.parseDouble(resp[1]);
							// fieldArr[i] = rs2.getString("stMoneySymbol") +
							// df.format(vald);
				...
				case 71: // PROJECT Ranking
					...
					break;
				default:
					stReturn += "<BR>Internal ERROR during report run for "
							+ rsTable.getString("stTableName");
					break;
				}
				// Save Report DATA
				int iId = this.epsUd.ebEnt.dbDyn
						.ExecuteSql1n("select max(RecId) from teb_reports");
				iId++;
				this.epsUd.ebEnt.dbDyn
						.ExecuteUpdate("replace into teb_reports (RecId,nmCustomReportId,stReportName,dtRun,stReportRaw)"
								+ "values( "
								+ iId
								+ ","
								+ nmCustomReportId
								...
								+ stReportName
								+ "',now(),"
								+ this.epsUd.ebEnt.dbDyn.fmtDbString(getFields(
										rsF, iMaxF) + "|\n" + stReport) + ")");
				return viewReport(rsTable);
			} catch (Exception e) {
			...
			return customReportDesigner(rsTable); // Not a valid id, go design
													// it
													// first.
	}
			...
			for (int iF = 1; iF <= iMaxF; iF++) {
				...
				default:
					...
					if ((rsF.getInt("nmFlags") & 0x10000000) != 0) {
						stTemp = this.epsUd.ebEnt.dbDyn
								.ExecuteSql1("select (iValue * WeightImportance) as val"
										+ " from teb_project p, Criteria c where c.nmDivision="
						...
						if (this.epsUd.ebEnt.ebUd.getLoginId() == 1)
							this.stError += "<br>Field: "
									+ rsF.getInt("nmForeignId") + " "
	...
	private String getReportFields(ResultSet rsR, ResultSet rsF, int iMaxF) {
		...
		try {
			for (iF = 1; iF <= iMaxF; iF++) {
				rsF.absolute(iF);
				if (rsF.getInt("nmFieldId") != 765
						&& rsF.getInt("nmFieldId") != 766) { // these
																// are
																// not
																// in
																// db.
				...
				case 41: // LaborCateogries
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeLaborCategories(rsF,
							stValue, 2));
					break;
				case 48: // LaborCateogries
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeLaborCategories(rsF,
							stValue, rsR));
					break;
				case 42: // Dependencies
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeDependencies(rsF, 2,
							rsR.getString("nmProjectId"),
							rsR.getString("RecId")));
					break;
				case 45: // Successors
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeSuccessors(rsF, 2,
							rsR.getString("nmProjectId"),
							rsR.getString("RecId")));
					break;
				case 43: // Inventory
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeInventory(rsF,
							stValue, 2));
					break;
				case 44: // Other Resources
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeOtherResources(rsF,
							stValue, 2));
					break;
				case 47: // Indicators
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeIndicators(rsF,
							stValue, 2));
					break;
				...
				case 56: // mapping task ids
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeProjectReqMap(rsF,
							stValue, rsR));
					break;
				...
				case 49:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeCostEffectiveness(rsF,
							stValue, rsR));
					break;
				case 50:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeProductivity(rsF,
							stValue, rsR));
					break;
				case 51:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeEstimatedHours(rsF,
							stValue, rsR));
					break;
				case 52:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeActualHours(rsF,
							stValue, rsR));
					break;
				case 53:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeDivision(rsF, stValue,
							rsR));
					break;
				default:
					switch (rsF.getInt("nmForeignId")) {
					...
					case 29:
						abReturn.append("~");
						abReturn.append(this.epsUd.ebEnt.dbDyn
					...
					case 16: // Lc - for user;
						abReturn.append("~");
						ResultSet rsLc = this.epsUd.ebEnt.dbDyn
								.ExecuteSql("select distinct LaborCategory from LaborCategory l,"
										+ " teb_reflaborcategory rl where rl.nmRefType=42 and rl.nmRefId="
										+ rsR.getString("RecId")
					...
					case 15: // nmPriviledge
						abReturn.append("~");
						abReturn.append(this.epsUd.epsEf.getPriviledgeTypes(rsR
								.getInt(rsF.getString("stDbFieldName"))));
						break;
					...
					case 765: // baseline cost difference
						...
						stSql = "select sum(SchCost) as costSum FROM Schedule where (SchFlags & 0x10 ) != 0 and nmProjectId="
								+ rsR.getString("RecId")
								+ " and nmBaseline="
								+ rsR.getString("CurrentBaseline");
						rs = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
						if (rs.first()) {
							rs.absolute(1);

							if (rs.getString("costSum") != null)
								currCost = Double.parseDouble(rs
										.getString("costSum"));
						}

						// look up saved baseline
						stSql = "select nmCost from teb_baseline where nmProjectId="
								+ rsR.getString("RecId")
								+ " and stType='Approve' ORDER BY nmBaseline DESC";
						rs = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
						if (rs.first()) {
							rs.absolute(1);
							appCost = Double
									.parseDouble(rs.getString("nmCost"));
						}
						abReturn.append("~");
					...
					case 766: // baseline effort difference
						...
						Double appBL = 0.00;
						// lookup current baseline
						stSql = "SELECT sum(SchEstimatedEffort) as effSum FROM Schedule where (SchFlags & 0x10 ) != 0 and nmProjectId="
								+ rsR.getString("RecId")
								+ " and nmBaseline="
								+ rsR.getString("CurrentBaseline");
						...
						if (rs.first()) {
							...
							if (rs.getString("effSum") != null)
								currBL = Double.parseDouble(rs
										.getString("effSum"));
						}

						// look up saved baseline
						stSql = "select nmEffort from teb_baseline where nmProjectId="
								+ rsR.getString("RecId")
								+ " and stType='Approve' ORDER BY nmBaseline DESC";
						rs = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
						if (rs.first()) {
							rs.absolute(1);
							appBL = Double
									.parseDouble(rs.getString("nmEffort"));
						}
						abReturn.append("~");
						// if baseline was not approved, no difference
						if (appBL > 0.00)
			...
			// Append project name #105
			boolean hPrjId = false;
			boolean hPrj = false;
			for (int i = 1; i <= rsR.getMetaData().getColumnCount(); i++) {
				if ("nmProjectId".equals(rsR.getMetaData().getColumnName(i)))
					hPrjId = true;
				if ("nmProject".equals(rsR.getMetaData().getColumnName(i)))
					hPrj = true;
			}
			String prjId = "";
			if (hPrjId) {
				prjId = rsR.getString("nmProjectId");
			} else {
				if (hPrj) {
					int t = rsR.getInt("nmProject");
					if (t > 0)
						prjId = "" + t;
				}
			}
		...
		try {
			if (stValue != null && stValue.trim().length() > 0) {
				if (rsF.getString("stHandler").contains("selectuser-29")) {
					ResultSet rs2 = this.epsUd.ebEnt.dbDyn
	...
	public void doProjectRanking(EpsUserData epsUd) {
		try {
			this.epsUd = epsUd;
			this.epsUd.ebEnt.dbDyn
					.ExecuteUpdate("update Projects set TotalRankingScore=0");

			ResultSet rs = this.epsUd.ebEnt.dbDyn
					.ExecuteSql("select * from teb_project p, teb_fields f, Projects pr, Criteria c"
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if (iLast > 0 && iLast != rs.getInt("nmProjectId")) {
					this.epsUd.ebEnt.dbDyn.ExecuteUpdate("update Projects set"
							+ " TotalRankingScore=" + dTotal + " where RecId="
							+ iLast);
					dTotal = 0;
				}
				iLast = rs.getInt("nmProjectId");
				if (rs.getString("WeightImportance") != null
						&& rs.getString("iValue") != null
						&& rs.getInt("iValue") > 0) {
					try {
						double dValue = rs.getInt("WeightImportance")
								* rs.getInt("iValue");
						dTotal += dValue;
					} catch (Exception e) {
					}
	...
	public void getBudgetReport() {
		...
		try {
			...
			ResultSet rsP = this.epsUd.ebEnt.dbDyn
					.ExecuteSql("select * from Projects");
			rsP.last();
			int iMaxP = rsP.getRow();
			for (int iP = 1; iP <= iMaxP; iP++) {
				rsP.absolute(iP);
				EarnedValue = this.epsUd.ebEnt.dbDyn
						.ExecuteSql1n("SELECT sum(SchCost)"
								+ " FROM Projects p, Schedule s where s.nmProjectId=p.RecId"
								+ " and p.CurrentBaseline=s.nmBaseline and p.RecId="
								+ rsP.getString("RecId")
				...
				SPI = 0;
				EarnedValue = 0;
				int iFirstApprove = this.epsUd.ebEnt.dbDyn
						.ExecuteSql1n("SELECT min(nmBaseline)"
								+ " FROM teb_baseline where nmProjectId="
								+ rsP.getString("RecId")
								+ " and stType='Approve'");
				int iIniticalBaseline = this.epsUd.ebEnt.dbDyn
				...
				if (iIniticalBaseline < iFirstApprove) {
					...
					InitialEstimatedCost = getScheduleCost(rsP.getInt("RecId"),
							iIniticalBaseline);
					CurrentEstimatedCost = getScheduleCost(rsP.getInt("RecId"),
							rsP.getInt("CurrentBaseline"));
					DifferenceInitCurrCost = CurrentEstimatedCost
				...
				stSql = "INSERT INTO Budget_Report "
						...
						+ ",CPI,SPI,EarnedValue) values("
						+ rsP.getString("RecId") + "," + DifferenceInitCurrCost
						+ "," + CurrentEstimatedCost + ","
						+ DollarsExpendedtoDate + "," + InitialEstimatedCost
						+ "," + CPI + "," + SPI + "," + EarnedValue + ")";
	...
	public double getScheduleCost(int nmProjectId, int nmBaseline) {
		double dReturn = 0;
		try {
			dReturn = this.epsUd.ebEnt.dbDyn
					.ExecuteSql1n("select sum(SchCost) from Schedule"
							+ " where nmProjectId=" + nmProjectId
		...
		try {
			...
			if (iMaxC > 0) {
				for (int iC = 1; iC <= iMaxC; iC++) {
					rsC.absolute(iC);
					/*
					 * Cost Effective formula is: (Hourly Salary * Productivity
					 * Factor) / (Average Labor Category Salary)
					 */
					double nmCostEffectiveness = 0;
					double nmProductiviyFactor = 0.5;
					if (rsC.getDouble("approved") > 0)
						nmProductiviyFactor = rsC.getDouble("allocated")
								/ rsC.getDouble("approved");
					if (rsC.getDouble("AverageHourlySalary") > 0)
						nmCostEffectiveness = rsC.getDouble("HourlyRate")
								* nmProductiviyFactor
								/ rsC.getDouble("AverageHourlySalary");
					stSql = "update teb_reflaborcategory set"
							...
							+ ",nmEstimatedHours = "
							+ rsC.getString("allocated")
							+ " where nmLaborCategoryId = "
							+ rsC.getString("nmLc")
							+ " and nmRefType=42 and nmRefId = "
							+ rsC.getString("nmUserId");
					this.epsUd.ebEnt.dbDyn.ExecuteUpdate(stSql);
				}
			}
