        ...
        switch (rsTable.getInt("nmTableId"))
        {
          ...
          case 81: // Project Schedule
        	...
        	*/
            ...
            if(stusrs.length() > 0 && pIDs.length() > 0)
            	filters = " WHERE (" + stusrs.substring(0, stusrs.length()-4) + ") AND " + pIDs;
            else if(stusrs.length() > 0 && pIDs.length() <= 0)
            	filters = " WHERE " + stusrs.substring(0, stusrs.length()-4);
        	else if(stusrs.length() <= 0 && pIDs.length() > 0)
        		filters = " WHERE " + pIDs;
            
            //Look through allocated tasks an display based on filters #103
            stSql = "SELECT DISTINCT nmUserId, nmTaskId, nmPrjId FROM teb_allocateprj" + filters;
            ResultSet rsA = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
            rsA.last();
            int iMaxA = rsA.getRow();
            for(int i=1; i<= iMaxA; i++){
            	rsA.absolute(i);
            	stSql = "select p.ProjectName,s.* from Projects p, Schedule s"
                    + " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline "
                    + "and p.RecId=" + rsA.getInt("nmPrjId") + " and s.RecId=" + rsA.getInt("nmTaskId")
                    + " group by p.ProjectName,s.RecId order by p.ProjectName,s.SchId";
            	rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
            	rsR.absolute(1);
            	stReport += getReportFields(rsR, rsF, iMaxF) + "|";
  ...
  private String getReportFields(ResultSet rsR, ResultSet rsF, int iMaxF)
  {
    ...
    try
    {
      ...
      for (int i = 1; i <= rsR.getMetaData().getColumnCount(); i++) {
	      if ("nmProjectId".equals(rsR.getMetaData().getColumnName(i))) hPrjId = true;
	      if ("nmProject".equals(rsR.getMetaData().getColumnName(i))) hPrj = true;
      }
      String prjId = "";
      if (hPrjId)
      {
