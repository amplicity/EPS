...
public class EpsReport {
			...
			if (iMaxR <= 0) {
				...
				for (int iR = 1; iR <= iMaxR; iR++) {
					...
					stReturn += epsUd.ebEnt.ebUd.addOption2(
					    rsR.getString("stReportName"), rsR.getString("RecId"), "");
				}
				stReturn += epsUd.ebEnt.ebUd.addOption2("--Create New Custom Report--",
				    "-1", "");
		...
		try {
			...
			if (rs != null) {
				...
				stReturn += "<table border=0 bgcolor=white cellspacing=1 cellpadding=2>"
				    ...
				    + "<th "
				    + stClass
				    + " colspan=2>View Report Format</th><th "
				    + stClass
				    + ">&nbsp;</th></tr>";
				...
				for (int iR = 1; iR <= iMax; iR++) {
					rs.absolute(iR);
					stReturn += "<tr>";
					// stReturn += "<td " + stClass + ">" + rs.getString("stReportType") +
					// "</td>";
					stReturn += "<td " + stClass + ">" + rs.getString("stReportName")
					    + "</td>";
					stReturn += "<td " + stClass + ">" + rs.getString("dtRun") + "</td>";
					stReturn += "<td " + stClass
					    + "><a target=_blank href='./?stAction=reports&t=" + stT
					    + "&reportid=" + rs.getString("RecId")
					    + "&format=html'>HTML</a></td>";
					stReturn += "<td " + stClass
					    + "><a href='./?stAction=reports&customreport=" + stValue + "&t="
	...
	public String epsFormatSavedReports(String stReportId, EbEnterprise ebEnt) {
		...
		try {
			...
			if (stTemp != null && stTemp.equals("delete")) {
				ebEnt.dbDyn.ExecuteUpdate("delete from teb_reports where RecId="
				    + stReportId);
			...
			if (rs != null) {
				...
				/*
				 * //project name //get filtered projects String pIDs = ""; String
				 * prjName = ""; String[] prjArr = null; prjArr =
				 * rs.getString("prjFilter").split(","); if(prjArr != null &&
				 * prjArr.length > 0){ for(int i=0; i<prjArr.length; i++){
				 * if(!prjArr[i].equals("")) pIDs += "RecId = " + prjArr[i] + " or "; }
				 * if(!pIDs.equals("")){ pIDs = " where " + pIDs.substring(0,
				 * pIDs.length()-4); } } ResultSet rsPID =
				 * ebEnt.dbDyn.ExecuteSql("select ProjectName from projects" + pIDs);
				 * while(rsPID.next()){ prjName += rsPID.getString("ProjectName") +
				...
				// " Report &nbsp;&nbsp;&nbsp; Title: <b>" +
				// rs.getString("stReportName") + "</b> Run: " + rs.getString("dtRun");
				stPageTitle = "<b>" + rs.getString("stReportName")
				    + "</b> &nbsp;&nbsp;<i>Run: " + rs.getString("dtRun") + "</i>";
				...
				ResultSet rs2 = ebEnt.dbDyn
				    .ExecuteSql("select * from teb_reportcolumns rc,teb_fields f, teb_epsfields ef"
				        ...
				        + rs.getString("nmCustomReportId") + " order by rc.nmOrder");
				rs2.last();
				int i2 = rs2.getRow();
				for (int iL = 0; iL < aLines.length; iL++) {
					...
					if (iL == 0) {
						...
						for (int iF = 1; iF < aFields.length && iF <= i2; iF++) {
							...
							if (rs2.getString("stCustom").length() > 0
							    && !rs2.getString("stCustom").equals("null"))
								aTdClass[iF] += " style=\"" + rs2.getString("stCustom") + "\" ";
							if (aTdClass[iF].length() <= 0) {
								/* AS -- 19Oct2011 -- Issue # 55 */
								/*
								 * if (rs2.getInt("nmDataType") == 1 || rs2.getInt("nmDataType")
								 * == 5 || rs2.getInt("nmDataType") == 31) { if
								 * (rs2.getString("stHandler").length() <= 0) aTdClass[iF] =
								 * " align=right "; }
								 */
						 ...
						 * if(rs.getString("prjFilter") != ""){ stReturn +=
						 * "<tr><th colspan=" + (aFields.length - 1) +
						 * " align='left'>Project " + rs.getString("stReportName") +
						 * " Report for project " + prjName + "</th></tr>"; }
						 */
					}

					// add project name immediately before project info from project
					// filter #105
					// TODO: TanPD remove group line
					// if(!"89".equals(stT) && !aFields[aFields.length-1].equals(currPrj)
					// && iL > 0 && iL < (aLines.length-1)){
					// stReturn += "<tr><td colspan=" + (aFields.length - 1) + "><strong>"
					...
					stReturn += "<tr>";
					// ~Project Name^123~Synergy With Organization^1088~Synergy With Other
					// Projects^1089~Total Ranking Score^998|~EPS Office
					// Building~27~24~3370|~EPPORA Training Sessions~21~21~3179|~EPPORA
					// SQL Server Conversion Suite~18~18~2950|~EPPORA Source
					// Analysis~15~15~2749|~EPPORA Resource Modeling~12~12~2546|~EPPORA
					// Project Templates~9~9~2365|~EPPORA Project Manager Scheduling
					// Enhancer~24~27~2192|~EPPORA Primavera Conversion
					// Suite~6~6~2022|~EPPORA Performance Enhancer~3~3~1871|~EPPORA Oracle
					// Conversion Suite~6~6~1833|~EPPORA Online Training~9~9~1761|~EPPORA
					// Niku Workbench Conversion Suite~12~12~1655|~EPPORA MySQL Conversion
	...
	public String customReportDesigner(ResultSet rsTable) {
		...
		try {
			...
			ResultSet rsCr = epsUd.ebEnt.dbDyn
			...
			case 71: // Project Ranking
				stSql = "select * from teb_fields f left join teb_reportcolumns rc"
				    ...
				    + nmCustomReportId + " where  ( f.nmTabId="
				    + rsTable.getString("nmTableId") + " or "
			...
			default:
				stSql = "select * from teb_fields f left join teb_reportcolumns rc"
				    + " on f.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = "
				    + nmCustomReportId + " where f.nmTabId="
				    + rsTable.getString("nmTableId") + " and rc.nmFieldId is null "
			...
			if (giSubmitId > 0 && giSubmitId < 2000) {
				...
				epsUd.ebEnt.dbDyn
				    .ExecuteUpdate("update teb_reportcolumns set nmOrder = " + iNew
				        + " where nmFieldId = " + giSubmitId + " and nmCustomReportId="
				        + nmCustomReportId);
			}
			...
			if ((stSubmit != null && stSubmit.length() > 0)
				...
				for (iF = 1; iF <= iMaxF; iF++) {
					...
					String stShow = epsUd.ebEnt.ebUd.request.getParameter("show_"
					    + nmFieldId);
					if (stShow == null)
						stShow = "N";
					else
					...
					stSql = "update teb_reportcolumns set stShow = \"" + stShow + "\""
					    + ",stClass = \""
					    + epsUd.ebEnt.ebUd.request.getParameter("class_" + nmFieldId)
					    + "\"" + ",stCustom = \""
					    + epsUd.ebEnt.ebUd.request.getParameter("custom_" + nmFieldId)
					    + "\"" + ",stShort = \""
					    + epsUd.ebEnt.ebUd.request.getParameter("short_" + nmFieldId)
					    + "\"";
					if (epsUd.ebEnt.ebUd.request.getParameter("width_" + nmFieldId) != null
					    && epsUd.ebEnt.ebUd.request.getParameter("width_" + nmFieldId)
					        .length() > 0)
						stSql += ",nmWidth = "
						    + epsUd.ebEnt.ebUd.request.getParameter("width_" + nmFieldId);
					stSql += " where nmFieldId = " + nmFieldId + " and nmCustomReportId="
					    + nmCustomReportId;
				...
				if (stSubmit != null) {
					if (stSubmit.equals("Save")) {
						if (nmCustomReportId > 3) {
							stPopupError += epsUd.validateD22("stReportName", rsCr,
							    "stReportName", "teb_customreport", "RecId");
							if (stPopupError.length() <= 0) {
								...
								epsUd.ebEnt.dbDyn.ExecuteUpdate("update teb_customreport set "
								    ...
								    + ",stReportCss = "
								    + epsUd.ebEnt.dbDyn.fmtDbString(epsUd.ebEnt.ebUd.request
								        .getParameter("stReportCss")) + ",prjFilter = "
								    + epsUd.ebEnt.dbDyn.fmtDbString(prjFilter)
			...
			stReturn = "\n</form><form method=post name='form"
			    + rsTable.getInt("nmTableId") + "' id='form"
			    + rsTable.getInt("nmTableId") + "'>"
			...
			if (nmCustomReportId > 3) {
				stReturn += "<h2 style=\"font-size: 14pt;\">Report Content</h2>"
				    ...
				    + (isDefaultReport ? "readonly " : "") + "value=\""
				    + epsUd.ebEnt.ebUd.getMyDb("stReportName", rsCr, "stReportName")
				    + "\" size=64>";
			}
			stReturn += "<br/>&nbsp;" + "<font color=red>" + stPopupError
			...
			for (iF = 1; iF <= iMaxF; iF++) {
				...
				epsUd.ebEnt.dbDyn.ExecuteUpdate(stSql); // Just for safety sakes. Should
																								// be ok though.
				...
				stReturn += "\n<td align=center>"
				    + this.orderSelect("order_" + nmFieldId, iF, iMaxF,
				        rsTable.getInt("nmTableId"), nmFieldId) + "</td>";
				...
				stReturn += "<td align=center><input type=checkbox id=show_"
				    + nmFieldId + " name=show_" + nmFieldId + " value=" + nmFieldId
				    + " " + stChecked + "></td>";
				/* AS -- 19Oct2011 -- Issue # 64 */
				vecshow.add(nmFieldId);
				if (nmCustomReportId <= 3)
					stReturn += "<td align=left><input type=text name=width_" + nmFieldId
					    + " value=\""
					    + epsUd.ebEnt.ebUd.getMyDb("width_" + nmFieldId, rsF, "nmWidth")
					    + "\" size=3 style=\"text-align:right\" ></td>";
				// stReturn += "<td align=left>" + epsUd.ebEnt.ebUd.getClass(nmFieldId,
				// epsUd.ebEnt.ebUd.getMyDb("class_" + nmFieldId, rsF, "stClass")) +
				// "</td>";
				else
					stReturn += "<td align=left><input type=text name=custom_"
					    ...
					    + " value=\""
					    + epsUd.ebEnt.ebUd
					        .getMyDb("custom_" + nmFieldId, rsF, "stCustom")
					    + "\" size=40 ></td>";
				stReturn += "<td align=left><input type=text name=short_" + nmFieldId
				    + " value=\""
				    + epsUd.ebEnt.ebUd.getMyDb("short_" + nmFieldId, rsF, "stShort")
				    + "\" size=15 ></td>";
				stReturn += "</tr>";
			}
			...
			if (rsTable.getInt("nmTableId") == 76) { // project requirement submit
				...
				stReturn += "<tr><td align=center colspan=2>"
				    ...
				    + "<input type=submit name=savedata value='Cancel'>"
				    + "</td></tr></table>";
			} else if (rsTable.getInt("nmTableId") == 78) { // project requirement
																											// analysis report submit
				stReturn += "<tr><td align=center colspan=2>"
				    + "<input type=submit name=savedata value='Save' onclick='selectAllOptions(document.getElementById(\"fprojects_selected\"))'>&nbsp;&nbsp;&nbsp;"
				    + "<input type=submit name=savedata value='Cancel'>"
				    + "</td></tr></table>";
	...
	public String orderSelect(String stName, int iC, int iLength, int nmTableId,
	    int iField) {
		String stReturn = "<select name=\"" + stName + "\" id=\"" + stName
		    + "\" onChange=\"setSubmitId(" + iField + ");document.form" + nmTableId
		    + ".submit();\">";
		String stChecked = "";
	...
	private String runReport(ResultSet rsTable) {
		...
		if (nmCustomReportId > 0) {
			try {
				...
				ResultSet rsF = epsUd.ebEnt.dbDyn
				    .ExecuteSql("select rc.*,f.*,ef.stHandler"
				        ...
				        + " and rc.nmCustomReportId=" + nmCustomReportId
				        + " order by rc.nmOrder asc ");
				rsF.last();
				int iMaxF = rsF.getRow();
				if (iMaxF <= 0)
					return customReportDesigner(rsTable); // --------------> Must slect
																								// fields
				String stFilter = epsUd.ebEnt.dbDyn
				    .ExecuteSql1("select stFilter from teb_customreport where RecId="
				        + nmCustomReportId);
				...
				case 81: // Project Schedule
					 ...
					 * " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline" +
					 * " group by p.ProjectName,s.RecId order by p.ProjectName,s.SchId";
					 * rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql); rsR.last(); iMaxR =
					 * rsR.getRow(); for (int iR = 1; iR <= iMaxR; iR++) {
					 * rsR.absolute(iR); stReport += getReportFields(rsR, rsF, iMaxF) +
					 * "|"; }
					 */
					...
					if (prjFilter != null && prjFilter.length() > 0) {
						...
						if (prjArr != null && prjArr.length > 0) {
							...
							if (!pIDs.equals("")) {
								pIDs = "AND (" + pIDs.substring(0, pIDs.length() - 4) + ")";
								// pIDs = "(" + pIDs.substring(0, pIDs.length()-4) + ")";
							}
						}
					}
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSql = this.epsUd.makeUserSql(nmType, aV[1], aV[2], aV[3], aV[4],
						    aV[5]);
					} else {
						stSql = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full", "",
						    "beg"); // Do all
					}
					...
					for (int iR = 1; iR <= iMaxR; iR++) {
						...
						// stusrs += "nmUserId=" + rsR.getInt("nmUserId") + " OR ";
					}

					/*
					 * //create user and project filter for query if(stusrs.length() > 0
					 * && pIDs.length() > 0) filters = " WHERE (" + stusrs.substring(0,
					 * stusrs.length()-4) + ") AND " + pIDs; else if(stusrs.length() > 0
					 * && pIDs.length() <= 0) filters = " WHERE " + stusrs.substring(0,
					 * stusrs.length()-4); else if(stusrs.length() <= 0 && pIDs.length() >
					 * 0) filters = " WHERE " + pIDs;
					 * 
					 * //Look through allocated tasks an display based on filters #103
					 * stSql =
					 * "SELECT DISTINCT nmUserId, nmTaskId, nmPrjId FROM teb_allocateprj"
					 * + filters; ResultSet rsA =
					 * this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql); rsA.last(); int iMaxA =
					 * rsA.getRow(); for(int i=1; i<= iMaxA; i++){ rsA.absolute(i); stSql
					 * = "select p.ProjectName,s.* from Projects p, Schedule s" +
					 * " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline "
					 * + "and p.RecId=" + rsA.getInt("nmPrjId") + " and s.RecId=" +
					 * rsA.getInt("nmTaskId") +
					 * " group by p.ProjectName,s.RecId order by p.ProjectName,s.SchId";
					 * rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql); rsR.absolute(1);
					 * stReport += getReportFields(rsR, rsF, iMaxF) + "|"; }
					 */
						...
						if (!stlb.equals("")) {
							...
							if (!aFields[3].equals("")) {
								...
								stSql = "select count(*) from users u, teb_reflaborcategory rlc where rlc.nmRefType=42 and rlc.nmRefId=u.nmUserId and rlc.nmLaborCategoryId="
								    + aFields[0] + " and rlc.nmRefId IN (" + stUsers + ")";
				...
				case 69: // Productivity
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						String[] aV = stFilter.split("\\^", -1);
						int nmType = Integer.parseInt(aV[0]);
						stSqlUser1 = this.epsUd.makeUserSql(nmType, aV[1], aV[2], aV[3],
						    aV[4], aV[5]);
					} else {
						stSqlUser1 = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full",
						    "", "beg"); // Do all
					}
					stSqlUser1 = stSqlUser1.replace(
					    "u.*,xu.stEMail,xu.nmPriviledge,xu.RecId", "u.nmUserId");
				...
				case 55: // Criteria
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						String[] aV = stFilter.split("\\^", -1);
						int nmType = Integer.parseInt(aV[0]);
						// stSqlUser = this.epsUd.makeUserSql(nmType, aV[1], aV[2], aV[3],
						// aV[4], aV[5]);
						stDivList = aV[2];
				...
				case 56: // Division
					...
					stSql += "SELECT * FROM teb_division"
					    + (stDivList.length() > 0 ? " where nmDivision in (" + stDivList
				...
				case 53: // Cost Effectiveness
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSqlUser = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full", "",
						    "beg"); // Do all
					}
					...
					for (int iR = 1; iR <= iMaxR; iR++) {
						...
						for (int i = 1; i < fieldArr.length; i++) {
							...
							// Double vald = Double.parseDouble(fieldArr[i].substring(2)) *
			...
			return customReportDesigner(rsTable); // Not a valid id, go design it
																						// first.
	}
					...
					if ((rsF.getInt("nmFlags") & 0x10000000) != 0) {
						stTemp = this.epsUd.ebEnt.dbDyn
						    .ExecuteSql1("select (iValue * WeightImportance) as val"
						        ...
						        + rsP.getString("nmDivision") + " and c.CriteriaName = \""
						        + rsF.getString("stLabel") + "\"" + " and nmProjectId="
						        + rsP.getString("RecId") + " and nmBaseline=1 and"
						...
						if (this.epsUd.ebEnt.ebUd.getLoginId() == 1)
							this.stError += "<br>Field: " + rsF.getInt("nmForeignId") + " "
							    + rsF.getString("stDbFieldName") + " not yet mapped";
					}
					break;
				}
			}
	...
	private String getReportFields(ResultSet rsR, ResultSet rsF, int iMaxF) {
		...
		try {
			for (iF = 1; iF <= iMaxF; iF++) {
				rsF.absolute(iF);
				if (rsF.getInt("nmFieldId") != 765 && rsF.getInt("nmFieldId") != 766) { // these
																																								// are
																																								// not
																																								// in
																																								// db.
				...
				case 41: // LaborCateogries
					abReturn.append("~");
					abReturn
					    .append(this.epsUd.epsEf.makeLaborCategories(rsF, stValue, 2));
					break;
				case 48: // LaborCateogries
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeLaborCategories(rsF, stValue,
					    rsR));
					break;
				case 42: // Dependencies
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeDependencies(rsF, 2,
					    rsR.getString("nmProjectId"), rsR.getString("RecId")));
					break;
				case 45: // Successors
					abReturn.append("~");
				...
				case 56: // mapping task ids
					abReturn.append("~");
					abReturn
					    .append(this.epsUd.epsEf.makeProjectReqMap(rsF, stValue, rsR));
					break;
				...
				case 49:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeCostEffectiveness(rsF, stValue,
					    rsR));
					break;
				case 50:
					abReturn.append("~");
				...
				case 51:
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf
					    .makeEstimatedHours(rsF, stValue, rsR));
					break;
				case 52:
					abReturn.append("~");
				...
				default:
					...
					case 29:
						...
						abReturn.append(this.epsUd.ebEnt.dbDyn
						    .ExecuteSql1("select  ProjectName from Projects where RecId="
						        + rsR.getInt(rsF.getString("stDbFieldName"))));
						break;
					case 16: // Lc - for user;
						abReturn.append("~");
					...
					case 15: // nmPriviledge
						abReturn.append("~");
						abReturn.append(this.epsUd.epsEf.getPriviledgeTypes(rsR.getInt(rsF
				...
				if (rsF.getString("stHandler").contains("selectuser-29")) {
					ResultSet rs2 = this.epsUd.ebEnt.dbDyn
					    .ExecuteSql("select concat(FirstName,' ',LastName) as nm from Users where nmUserId in ( "
					        + stValue + " ) order by LastName,FirstName");
					rs2.last();
		...
		try {
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if (rs.getString("WeightImportance") != null
				    && rs.getString("iValue") != null && rs.getInt("iValue") > 0) {
					try {
		...
		try {
			...
			for (int iP = 1; iP <= iMaxP; iP++) {
				...
				EarnedValue = this.epsUd.ebEnt.dbDyn.ExecuteSql1n("SELECT sum(SchCost)"
				    ...
				    + rsP.getString("RecId") + " and (s.SchFlags & 0x1000) != 0");
				...
				int iFirstApprove = this.epsUd.ebEnt.dbDyn
				    .ExecuteSql1n("SELECT min(nmBaseline)"
				        ...
				        + rsP.getString("RecId") + " and stType='Approve'");
				int iIniticalBaseline = this.epsUd.ebEnt.dbDyn
				    .ExecuteSql1n("SELECT max(nmBaseline)"
				        + " FROM teb_baseline where nmProjectId="
				        + rsP.getString("RecId") + " and nmBaseline < " + iFirstApprove);
				...
				stSql = "INSERT INTO Budget_Report "
				    ...
				    + ",CPI,SPI,EarnedValue) values(" + rsP.getString("RecId") + ","
				    + DifferenceInitCurrCost + "," + CurrentEstimatedCost + ","
				    + DollarsExpendedtoDate + "," + InitialEstimatedCost + "," + CPI
				    + "," + SPI + "," + EarnedValue + ")";
		...
		try {
			dReturn = this.epsUd.ebEnt.dbDyn
			    .ExecuteSql1n("select sum(SchCost) from Schedule"
			        + " where nmProjectId=" + nmProjectId + " and nmBaseline="
			        + nmBaseline + " and SchLevel=0");
		} catch (Exception e) {
		...
		try {
			...
			if (iMaxC > 0) {
				for (int iC = 1; iC <= iMaxC; iC++) {
					rsC.absolute(iC);
					/*
					 * Cost Effective formula is: (Hourly Salary * Productivity Factor) /
					 * (Average Labor Category Salary)
					 */
					...
					if (rsC.getDouble("AverageHourlySalary") > 0)
						nmCostEffectiveness = rsC.getDouble("HourlyRate")
						    * nmProductiviyFactor / rsC.getDouble("AverageHourlySalary");
