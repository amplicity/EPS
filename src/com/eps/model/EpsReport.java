...
public class EpsReport {
	...
	public String doReport(ResultSet rsTable, EpsUserData epsUd) {
		...
		try {
			...
			ResultSet rsR = epsUd.ebEnt.dbDyn.ExecuteSql("SELECT * FROM teb_customreport where stReportType = '"
					+ rsTable.getString("nmTableId") + "' order by nmDefaultReport DESC,stReportName ");
			rsR.last();
			...
			if (iMaxR <= 0) {
				...
				rsR = epsUd.ebEnt.dbDyn.ExecuteSql("SELECT * FROM teb_customreport where stReportType = '" + rsTable.getString("nmTableId")
	...
	public String epsShowSavedReports(ResultSet rsTable) {
		...
		try {
			...
			if (rs != null) {
				...
				stReturn += "<table class=l1tableb>" + "<tr>" + "<th " + stClass + ">Report Title</th><th " + stClass + ">Run Time</th>"
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					stReturn += "<td " + stClass + "><a target=_blank href='./?stAction=reports&t=" + stT + "&reportid="
							+ rs.getString("RecId") + "&format=html'>HTML</a></td>";
					stReturn += "<td " + stClass + "><a href='./?stAction=reports&customreport=" + stValue + "&t=" + stT + "&reportid="
							+ rs.getString("RecId") + "&format=excel'>Excel</a></td>";
					stReturn += "<td " + stClass + "><a href='./?stAction=reports&customreport=" + stValue + "&t=" + stT + "&reportid="
							+ rs.getString("RecId")
		...
		try {
			...
			if (rs != null) {
				...
				ResultSet rs2 = ebEnt.dbDyn.ExecuteSql("select * from teb_reportcolumns rc,teb_fields f, teb_epsfields ef"
						+ " where rc.nmFieldId=f.nmForeignId and f.nmForeignId=ef.nmForeignId" + " and stShow='Y' and nmCustomReportId="
				...
				for (int iL = 0; iL < aLines.length; iL++) {
					...
					if (iL == 0) {
						...
						stReturn += "<tr><th colspan=" + (aFields.length - 1) + " align=center class=l1th>" + stPageTitle + "</th></tr>";
					...
					for (int iF = 1; iF < aFields.length && iF <= i2; iF++) {
						if (iL == 0) {
							...
							stReturn += "<td " + aTdClass[iF] + ">"
									+ (aFields[iF].replace("`", "<br>").equals("") ? "&nbsp;" : aFields[iF].replace("`", "<br>")) + "</td>";
						}
					}
					stReturn += "</tr>";
				}
	...
	public String customReportDesigner(ResultSet rsTable) {
		...
		try {
			...
			String stReportName = epsUd.ebEnt.ebUd.request.getParameter("stReportName");
			...
			do {
				if (nmCustomReportId <= 0) {
					...
					epsUd.ebEnt.dbDyn.ExecuteUpdate("replace into teb_customreport (RecId,stReportName,stReportType, prjFilter) values "
							...
							+ nmCustomReportId
							+ ","
							+ epsUd.ebEnt.dbDyn.fmtDbString((stReportName != null && stReportName.length() > 0 ? stReportName : "New "
									+ rsTable.getString("stTableName") + " - " + nmCustomReportId)) + ",\""
							+ rsTable.getString("nmTableId") + "\", \"\") ");
				}
				rsCr = epsUd.ebEnt.dbDyn.ExecuteSql("SELECT * FROM teb_customreport where RecId=" + nmCustomReportId);
				rsCr.absolute(1);
				isChecked = true;
				...
				if (isDefaultReport && nmCustomReportId > 0 && "Save".equals(stSubmit)
			...
			case 61: // Labor Category
				stSql = "select * from teb_fields f, teb_epsfields ef left join teb_reportcolumns rc"
						+ " on ef.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = " + nmCustomReportId
						+ " where f.nmForeignId=ef.nmForeignId and" + " ( (f.nmTabId in (14,610) and (f.nmFlags & 0x400) = 0)  )"
						+ " and rc.nmFieldId is null and f.nmForeignID not in ( 0 ) order by f.stLabel";
				break;
			...
			case 83: // Project Schedule Analysis
				stSql = "select * from teb_fields f, teb_epsfields ef left join teb_reportcolumns rc"
						+ " on ef.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = " + nmCustomReportId
						+ " where f.nmForeignId=ef.nmForeignId and" + " f.nmForeignId in (271,829,258,814,116,45,1008)"
						+ " and rc.nmFieldId is null and f.nmForeignID not in ( 0) order by f.stLabel";
				break;
			case 69: // Productivity
				stSql = "select * from teb_fields f, teb_epsfields ef left join teb_reportcolumns rc"
						+ " on ef.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = " + nmCustomReportId
						+ " where f.nmForeignId=ef.nmForeignId and" + " ( f.nmForeignId in (82,494,40,903,904,906,133) )"
						+ " and rc.nmFieldId is null and f.nmForeignID not in ( 0) order by f.stLabel";
				break;
			...
			case 71: // Project Ranking
				stSql = "select * from teb_fields f left join teb_reportcolumns rc"
						+ " on f.nmForeignId=rc.nmFieldId and rc.nmCustomReportId = " + nmCustomReportId + " where  ( f.nmTabId="
						+ rsTable.getString("nmTableId") + " or " + " (f.nmFlags & 0x10000000 ) != 0 ) and rc.nmFieldId is null "
						+ " order by f.stLabel";
				break;
			default:
				stSql = "select * from teb_fields f left join teb_reportcolumns rc"
			...
			for (iF = 1; iF <= iMaxF; iF++) {
				rsF.absolute(iF);
				epsUd.ebEnt.dbDyn.ExecuteUpdate("replace into teb_reportcolumns (nmCustomReportId,nmFieldId,nmWidth,nmOrder) " + "values("
						+ nmCustomReportId + ",\"" + rsF.getString("nmForeignId") + "\"," + rsF.getString("nmCols") + "," + iF + ") ");
			}
			if (giSubmitId > 0 && giSubmitId < 2000) {
				// Need to reorder.
				int iOrig = epsUd.ebEnt.dbDyn.ExecuteSql1n("select nmOrder from teb_reportcolumns " + "where nmCustomReportId="
						+ nmCustomReportId + " and nmFieldId=" + giSubmitId);
				...
				if (iNew < iOrig) { // 1 2 3 4 5 -> 3 1 2 4 5
					epsUd.ebEnt.dbDyn.ExecuteUpdate("update teb_reportcolumns set nmOrder = ( nmOrder + 1 )" + " where nmOrder >= " + iNew
							+ " and nmOrder <= " + iOrig + " and nmCustomReportId=" + nmCustomReportId);
				} else {
					epsUd.ebEnt.dbDyn.ExecuteUpdate("update teb_reportcolumns set nmOrder = ( nmOrder - 1 )" + " where nmOrder >= " + iOrig
							+ " and nmOrder <= " + iNew + " and nmCustomReportId=" + nmCustomReportId);
				}
				epsUd.ebEnt.dbDyn.ExecuteUpdate("update teb_reportcolumns set nmOrder = " + iNew + " where nmFieldId = " + giSubmitId
			...
			case 76: // project requirements omit project name
				rsF = epsUd.ebEnt.dbDyn.ExecuteSql("select rc.*,f.stLabel from teb_reportcolumns rc, teb_fields f "
						+ " where rc.nmFieldId=f.nmForeignId" + " and rc.nmCustomReportId=" + nmCustomReportId
			...
			if ((stSubmit != null && stSubmit.length() > 0) || (giSubmitId > 0 && giSubmitId < 2000)) {
				for (iF = 1; iF <= iMaxF; iF++) {
					...
					stSql = "update teb_reportcolumns set stShow = \"" + stShow + "\"" + ",stClass = \""
							+ epsUd.ebEnt.ebUd.request.getParameter("class_" + nmFieldId) + "\"" + ",stCustom = \""
							+ epsUd.ebEnt.ebUd.request.getParameter("custom_" + nmFieldId) + "\"" + ",stShort = \""
					...
					if (stSubmit.equals("Save")) {
						if (nmCustomReportId > 3) {
							...
							if (stPopupError.length() <= 0) {
								...
								epsUd.ebEnt.dbDyn
										.ExecuteUpdate("update teb_customreport set " + " stReportName = \"" + stReportName + "\""
												+ ",stFilter = " + epsUd.ebEnt.dbDyn.fmtDbString(stFilter) + ",stReportCss = "
												+ epsUd.ebEnt.dbDyn.fmtDbString(epsUd.ebEnt.ebUd.request.getParameter("stReportCss"))
												+ ",prjFilter = " + epsUd.ebEnt.dbDyn.fmtDbString(prjFilter) + " where RecId = "
												+ nmCustomReportId);
			...
			if (nmCustomReportId > 3) {
				stReturn += "<h2 style=\"font-size: 14pt;\">Report Content</h2>" + "Report Title: <input type=text name=stReportName "
						+ (!isCanEdit && isDefaultReport ? "readonly " : "") + "value=\""
						+ epsUd.ebEnt.ebUd.getMyDb("stReportName", rsCr, "stReportName") + "\" size=64>";
			}
			stReturn += "<br/>&nbsp;" + "<font color=red>" + stPopupError + "</font><br/><table border=1 style=\"background-color:white\">";
			...
			for (iF = 1; iF <= iMaxF; iF++) {
				...
				stSql = "update teb_reportcolumns set nmOrder= " + iF + " where nmCustomReportId=" + nmCustomReportId + " and nmFieldId=\""
						+ nmFieldId + "\"";
				...
				stReturn += "\n<td align=center>"
						+ this.orderSelect("order_" + nmFieldId, iF, iMaxF, rsTable.getInt("nmTableId"), nmFieldId) + "</td>";
				...
				stReturn += "<td align=center><input type=checkbox id=show_" + nmFieldId + " name=show_" + nmFieldId + " value="
						+ nmFieldId + " " + stChecked + "></td>";
				/* AS -- 19Oct2011 -- Issue # 64 */
				vecshow.add(nmFieldId);
				if (nmCustomReportId <= 3)
					stReturn += "<td align=left><input type=text name=width_" + nmFieldId + " value=\""
				...
				// "</td>";
					...
					stReturn += "<td align=left><input type=text name=custom_" + nmFieldId + " value=\""
							+ epsUd.ebEnt.ebUd.getMyDb("custom_" + nmFieldId, rsF, "stCustom") + "\" size=40 ></td>";
				stReturn += "<td align=left><input type=text name=short_" + nmFieldId + " value=\""
						+ epsUd.ebEnt.ebUd.getMyDb("short_" + nmFieldId, rsF, "stShort") + "\" size=15 ></td>";
				stReturn += "</tr>";
			}
			...
			if (rsTable.getInt("nmTableId") == 76) { // project requirement
				...
				stReturn += "<tr><td align=center colspan=2>"
						...
						+ "<input type=submit name=savedata value='Cancel'>" + "</td></tr></table>";
				...
				stReturn += "<tr><td align=center colspan=2>" + "<input type=submit name=savedata value='Save'>&nbsp;&nbsp;&nbsp;"
						+ "<input type=submit name=savedata value='Cancel'>" + "</td></tr></table>";
			}
	...
	public String orderSelect(String stName, int iC, int iLength, int nmTableId, int iField) {
		String stReturn = "<select name=\"" + stName + "\" id=\"" + stName + "\" onChange=\"setSubmitId(" + iField + ");document.form"
				+ nmTableId + ".submit();\">";
		String stChecked = "";
	...
	private String runReport(ResultSet rsTable) {
		...
		if (nmCustomReportId > 0) {
			try {
				...
				String stReportName = epsUd.ebEnt.dbDyn.ExecuteSql1("select stReportName from teb_customreport where RecId="
						+ nmCustomReportId);
				String[] prjArr = null;
				String pIDs = "";
				String stDivList = "";
				...
				case 81: // Project Schedule
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSql = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full", "", "beg"); // Do
																								// all
					}
				...
				case 78: // Project Requirements Analysis
					...
					stSql += "select p.ProjectName,r.*, '' as SuggestedMitigationStrategy from Requirements r, Projects p"
							...
							+ " and r.nmD50Flags != 0 and (ReqFlags & 0x10) != 0" + " order by p.ProjectName, r.ReqId;";
					rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					rsR.last();
					iMaxR = rsR.getRow();
					for (int iR = 1; iR <= iMaxR; iR++) {
				...
				case 83: // Project Schedule Analysis
					...
					stSql += "select p.ProjectName,s.*, '' as SuggestedMitigationStrategy from Schedule s, Projects p"
							+ " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline " + pIDs
							+ " and s.nmD53Flags != 0 and (SchFlags & 0x10) != 0" + " order by p.ProjectName, s.SchId";
					rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					rsR.last();
					iMaxR = rsR.getRow();
					for (int iR = 1; iR <= iMaxR; iR++) {
				...
				case 69: // Productivity
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSqlUser1 = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full", "", "beg"); // Do
																										// all
					}
				...
				case 55: // Criteria
					...
					stSql += "select c.RecId,c.nmDivision,c.nmFlags,c.CriteriaName,c.WeightImportance,IFNULL(d.stDivisionName,'') AS stDivisionName, IFNULL(GROUP_CONCAT(DISTINCT CONCAT(u.FirstName, ' ', u.LastName) SEPARATOR '<br>'),'') as Responsibilities"
							...
							+ " GROUP BY c.CriteriaName,d.nmDivision" + " order by c.CriteriaName,d.nmDivision";
					rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					rsR.last();
					iMaxR = rsR.getRow();
					for (int iR = 1; iR <= iMaxR; iR++) {
				...
				case 56: // Division
					...
					this.epsUd.epsEf.processUsersInDivision();
					stSql += "SELECT * FROM teb_division" + (stDivList.length() > 0 ? " where nmDivision in (" + stDivList + ")" : "")
				...
				case 57: // Inventory
					stSql = "select i.*,sum(ri.nmQty) as nmUsed from Inventory i"
							+ " left join teb_refinventory ri on i.RecId=ri.nmInventoryId" + " group by i.RecId order by InventoryName;";
					rsR = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
					rsR.last();
					iMaxR = rsR.getRow();
					for (int iR = 1; iR <= iMaxR; iR++) {
				...
				case 53: // Cost Effectiveness
					...
					if (stFilter != null && stFilter.length() > 7) { // 128^,3,,4,,6,^,3,,1,^full^^beg
						...
						stSqlUser = this.epsUd.makeUserSql(0xFFFF, ",0", ",0,", "full", "", "beg"); // Do
																									// all
					}
				...
				default:
					...
					if ((rsF.getInt("nmFlags") & 0x10000000) != 0) {
						stTemp = this.epsUd.ebEnt.dbDyn.ExecuteSql1("select (iValue * WeightImportance) as val"
								...
								+ " and c.CriteriaName = \"" + rsF.getString("stLabel") + "\"" + " and nmProjectId="
								+ rsP.getString("RecId") + " and nmBaseline=1 and" + " nmFieldId=" + rsF.getString("nmForeignId"));
						if (stTemp == null)
							stTemp = "";
						stReturn += fmtReportData(stTemp.trim(), rsF);
					} else {
						if (this.epsUd.ebEnt.ebUd.getLoginId() == 1)
							this.stError += "<br>Field: " + rsF.getInt("nmForeignId") + " " + rsF.getString("stDbFieldName")
									+ " not yet mapped";
					}
					break;
				}
		...
		try {
			for (iF = 1; iF <= iMaxF; iF++) {
				...
				case 48: // LaborCateogries
					abReturn.append("~");
					abReturn.append(this.epsUd.epsEf.makeLaborCategories(rsF, stValue, rsR));
					break;
				...
				default:
					...
					case 29:
						abReturn.append("~");
						abReturn.append(this.epsUd.ebEnt.dbDyn.ExecuteSql1("select  ProjectName from Projects where RecId="
					...
					case 765: // baseline cost difference
						...
						stSql = "select sum(SchCost) as costSum FROM Schedule where (SchFlags & 0x10 ) != 0 and nmProjectId="
								+ rsR.getString("RecId") + " and nmBaseline=" + rsR.getString("CurrentBaseline");
						...
						stSql = "select nmCost from teb_baseline where nmProjectId=" + rsR.getString("RecId")
					...
					case 766: // baseline effort difference
						...
						// look up saved baseline
						stSql = "select nmEffort from teb_baseline where nmProjectId=" + rsR.getString("RecId")
								+ " and stType='Approve' ORDER BY nmBaseline DESC";
						rs = this.epsUd.ebEnt.dbDyn.ExecuteSql(stSql);
						if (rs.first()) {
							rs.absolute(1);
			...
			if (!"".equals(prjId)) {
				abReturn.append("~" + this.epsUd.ebEnt.dbDyn.ExecuteSql1("SELECT ProjectName FROM projects WHERE RecId=" + prjId) + "~");
			}
		} catch (Exception e) {
			...
			if (stValue != null && stValue.trim().length() > 0) {
				if (rsF.getString("stHandler").contains("selectuser-29")) {
					ResultSet rs2 = this.epsUd.ebEnt.dbDyn
							.ExecuteSql("select concat(FirstName,' ',LastName) as nm from Users where nmUserId in ( " + stValue
	...
	public void doProjectRanking(EpsUserData epsUd) {
		try {
			...
			ResultSet rs = this.epsUd.ebEnt.dbDyn.ExecuteSql("select * from teb_project p, teb_fields f, Projects pr, Criteria c"
					...
					+ " and pr.RecId=p.nmProjectId" + " and (f.nmFlags & 0x10000000 ) != 0 and p.nmBaseline=pr.CurrentBaseline "
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if (iLast > 0 && iLast != rs.getInt("nmProjectId")) {
					this.epsUd.ebEnt.dbDyn.ExecuteUpdate("update Projects set" + " TotalRankingScore=" + dTotal + " where RecId=" + iLast);
					dTotal = 0;
				}
		...
		try {
			...
			for (int iP = 1; iP <= iMaxP; iP++) {
				...
				EarnedValue = this.epsUd.ebEnt.dbDyn.ExecuteSql1n("SELECT sum(SchCost)"
						+ " FROM Projects p, Schedule s where s.nmProjectId=p.RecId" + " and p.CurrentBaseline=s.nmBaseline and p.RecId="
				...
				SPI = 0;
				EarnedValue = 0;
				int iFirstApprove = this.epsUd.ebEnt.dbDyn.ExecuteSql1n("SELECT min(nmBaseline)" + " FROM teb_baseline where nmProjectId="
				...
				stSql = "INSERT INTO Budget_Report "
						...
						+ CurrentEstimatedCost + "," + DollarsExpendedtoDate + "," + InitialEstimatedCost + "," + CPI + "," + SPI + ","
						+ EarnedValue + ")";
	...
	public double getScheduleCost(int nmProjectId, int nmBaseline) {
		double dReturn = 0;
		try {
			dReturn = this.epsUd.ebEnt.dbDyn.ExecuteSql1n("select sum(SchCost) from Schedule" + " where nmProjectId=" + nmProjectId
					+ " and nmBaseline=" + nmBaseline + " and SchLevel=0");
		} catch (Exception e) {
	...
	public void getCostEffectiveness() {
		try {
			...
			String stSql = "SELECT count(*) cnt ,sum(nmAllocated) allocated,sum(nmActualApproved) approved,"
					...
					+ " and (s.SchFlags & 0x1000) != 0 " + " and ap.nmUserId=u.nmUserId and lc.nmLcId=nmLc"
			...
			if (iMaxC > 0) {
				for (int iC = 1; iC <= iMaxC; iC++) {
					...
					stSql = "update teb_reflaborcategory set" + " nmCostEffectiveness = " + nmCostEffectiveness
							+ ",nmProductiviyFactor = "
							+ nmProductiviyFactor
							// + ",nmTasksCompleted = " + nmTasksCompleted
							+ ",nmActualHours = " + rsC.getString("approved") + ",nmEstimatedHours = " + rsC.getString("allocated")
							+ " where nmLaborCategoryId = " + rsC.getString("nmLc") + " and nmRefType=42 and nmRefId = "
