...
public class EpsClient {
	...
	public String getEpsPage() {
		...
		try {
			...
			stReturn += "<script>"
					+ "$(document).ready(function() {$('#topNavItems .nav-home').append(\""
					+ stRecents
					+ "\");"
					+ "$('#topNavItems').menubar({autoExpand: true,position: {my: 'left top',at: 'left bottom-2'}});});"
					+ "</script>";
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String makeMenuBar() {
		...
		try {
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if ((rs.getInt("nmReportPriv") & nmPrivUser) != 0) {
					...
					stReport += "<li><a class='collapsed' href='"
							...
							+ stRun + "</ul></li>" + "<li><a href='"
							+ stViewLink + "'>View Saved Reports</a></li>"
							+ "</ul>" + "</li>";
				}
			...
			if (stReport.length() > 0)
				stReturn += "<li class='top-nav nav-reports'><a class='topnav' href='javascript:void(0);'>Reports</a><ul>"
						+ stReport + "</ul></li>";

			if (stProject.length() > 0)
				stReturn += "<li class='top-nav nav-projects'><a class='topnav' href='javascript:void(0);'>Project</a><ul>"
						+ stProject + "</ul></li>";

			if (stAdmin.length() > 0)
				stReturn += "<li class='top-nav nav-admin'><a class='topnav' href='javascript:void(0);'>Administration</a><ul>"
						+ stAdmin + "</ul></li>";
			...
			if ((this.ebEnt.ebUd.getLoginPersonFlags() & 0x800) != 0) { // Super
				...
				stReturn += "<li><a href='./?stAction=loadpage&m="
						+ URLEncoder.encode("End-of-Business Processing",
								"UTF-8")
						+ "&r="
						+ URLEncoder.encode("./?stAction=runeob&commtrace=i",
