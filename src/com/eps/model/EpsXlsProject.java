...
class EpsXlsProject // extends EpsUserData
{
	...
	public String xlsRequirementsEdit(String stChild) {
		...
		try {
			...
			String stLink = "?stAction=projects&t="
			    + this.ebEnt.ebUd.request.getParameter("t") + "&do=xls&pk=" + stPk
			...
			if (stAction != null && stAction.length() > 0 && stR != null
			    && stR.length() > 0) { // PROCESS ACTIONS
				int iRecIdDo = Integer.parseInt(stR);
				ResultSet rs1 = this.ebEnt.dbDyn
				    .ExecuteSql("select * from Requirements where RecId=" + iRecIdDo
				        + " and nmProjectId=" + stPk + " and nmBaseline="
				        + this.nmBaseline);
				rs1.absolute(1);
				if (stAction.equals("collapse")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags = ( ReqFlags | 2) where RecId= "
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags = ( ReqFlags | 1) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline
					        + " "
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags = (ReqFlags & ~0x3) where RecId= "
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags = (ReqFlags & ~0x3) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline
					        + " "
					...
					int iRecMax = this.ebEnt.dbDyn
					    .ExecuteSql1n("select max(RecId) from Requirements");
					iRecMax++;
					...
					try {
						...
						if (stWhat.equals("below")) {
							iEditId++;
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update Requirements set ReqId=(ReqId+1) where ReqId > "
							        + rs1.getString("ReqId")
							        + " and  nmProjectId="
							        + stPk
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("insert into Requirements (RecId,nmProjectId,ReqId,nmBaseline,ReqLevel) "
						        + "values("
						        + iRecMax
						        + ","
						        + stPk
						        ...
						        + iEditId
						        + ","
						        + nmBaseline + "," + rs1.getInt("ReqLevel") + ")");
					} catch (Exception e) {
						iEditId = 1;
						this.ebEnt.dbDyn
						    .ExecuteUpdate("insert into Requirements (RecId,nmProjectId,ReqId,nmBaseline,ReqLevel) "
					...
					this.epsUd.makeTask(6, this.epsUd.epsEf.getRequirementName(""
					    + iRecIdDo, stPk, nmBaseline)); // Inserted Requirement
					...
					if (stWhat.equals("children")) {
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("delete from Requirements where nmProjectId="
						        ...
						        + "and ReqId > " + rs1.getString("ReqId") + " and ReqId < "
						        + iReqIdEnd);
						iDel += (iReqIdEnd - rs1.getInt("ReqId") - 1);
					} else {
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Requirements set ReqLevel=(ReqLevel-1) where nmProjectId="
						        + stPk
						        + " and nmBaseline="
						        + this.nmBaseline
						        + " "
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqId=(ReqId-" + iDel
					        + ") where ReqId > " + rs1.getString("ReqId")
					        + " and  nmProjectId=" + stPk + " and nmBaseline="
					        + this.nmBaseline + " " + "and ReqId > "
					        + rs1.getString("ReqId"));
				} else if (stAction.equals("demote")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqLevel=(ReqLevel-1) where RecId="
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					...
					if (stB != null) {
						...
						try {
							...
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update Requirements set ReqFlags = ( ReqFlags | 1) "
							        + "where nmProjectId="
							        + stPk
							        + " and nmBaseline="
							        + this.nmBaseline + " and ReqLevel > " + iLevel);
						} catch (Exception e) {
						}
						setFlags(stChild);
					}
			...
			String s1 = "</form><form method=post name='form" + stChild
			    + "' id='form" + stChild + "' onsubmit='return myValidation(this)'>"
			    + "<input type=hidden name=from value='" + iFrom
			    + "'><table class=l1table><tr><td class=l1td colspan=" + iMaxFields
			    + " align=center><h2>" + this.rsProject.getString("ProjectName")
			    + "</h2></td></tr>";
			sbReturn.append(s1);
			...
			int iMaxRecords = this.ebEnt.dbDyn
			    .ExecuteSql1n("select count(*) from Requirements where nmProjectId="
			        + stPk + " and nmBaseline=" + this.nmBaseline
			        + " and (ReqFlags & 0x1) = 0");
			stSql = "select * from Requirements where nmProjectId=" + stPk
			    + " and nmBaseline=" + this.nmBaseline
			    + " and (ReqFlags & 0x1) =0 order by ReqId,RecId desc limit " + iFrom
			    + "," + iBlock;
			               ...
			               // we auto insert
			{
				...
				stSql = "select * from Requirements where nmProjectId=" + stPk
				    + " and nmBaseline=" + this.nmBaseline
				    + " and (ReqFlags & 0x1) != 1 order by ReqId,RecId desc limit "
				    + iFrom + "," + iBlock;
			...
			if (iMax > 0) {
				...
				for (int iR = 1; iR <= iMax; iR++) {
					                                                            ...
					                                                            // and PPM
					{
						...
						// iNextLevel + "," + iPrevLevel + "," + iNextShow + ",'" + stLink +
						// "' )\""
						s1 = "<tr  " + " onClick=" + "\"setActionsClick(1,"
						    + rs.getInt("RecId") + "," + rs.getInt("ReqId") + ","
						    + rs.getInt("ReqFlags") + "," + rs.getInt("ReqLevel") + ","
				...
				sbReturn.append(doNext(stChild, iMaxFields, 0, 0, iBlock, stGoBack,
				    stPk));
			}
	...
	public String xlsTestEdit(String stChild) {
		...
		try {
			...
			if (stAction != null && stAction.length() > 0 && stR != null
				...
				int iRecIdDo = Integer.parseInt(stR);
				ResultSet rs1 = this.ebEnt.dbDyn
				    .ExecuteSql("select * from Test where RecId=" + iRecIdDo
				        + " and  nmProjectId=" + stPk + " and nmBaseline="
				        + this.nmBaseline);
				rs1.absolute(1);
				if (stAction.equals("collapse")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstFlags=2 where RecId= "
					        + iRecIdDo + " and nmBaseline=" + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstFlags=1 where nmProjectId="
					        + stPk + " and nmBaseline=" + this.nmBaseline + " "
					        + "and TstId > " + rs1.getString("TstId") + " and TstId < "
					        + getEnd(stChild, stPk, rs1));
				} else if (stAction.equals("expand")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstFlags=0 where RecId= "
					        + iRecIdDo + " and nmBaseline=" + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstFlags=0 where nmProjectId="
					        + stPk + " and nmBaseline=" + this.nmBaseline + " "
					        + "and TstId > " + rs1.getString("TstId") + " and TstId < "
					        + getEnd(stChild, stPk, rs1));
				} else if (stAction.equals("insert")) {
					int iRecMax = this.ebEnt.dbDyn
					    .ExecuteSql1n("select max(RecId) from Test");
					iRecMax++;
					String stWhat = this.ebEnt.ebUd.request.getParameter("what");
					try {
						...
						if (stWhat.equals("below")) {
							iEditId++;
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update Test set TstId=(TstId+1) where TstId > "
							        + rs1.getString("TstId")
							        + " and  nmProjectId="
							        + stPk
							        + " and nmBaseline=" + this.nmBaseline + " ");
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("insert into Test (RecId,nmProjectId,TstId,nmBaseline,TstLevel) "
						        + "values("
						        + iRecMax
						        + ","
						        + stPk
					...
					this.ebEnt.ebUd.setRedirect(stLink + "&a=editfull&r=" + iRecMax
					    + "#row" + iEditId);
					return ""; // ----------------------------->
				} else if (stAction.equals("delete")) {
					this.ebEnt.dbDyn.ExecuteUpdate("delete from Test where RecId="
					    + iRecIdDo + " and  nmProjectId=" + stPk + " and nmBaseline="
					    + this.nmBaseline);
					String stWhat = this.ebEnt.ebUd.request.getParameter("what");
					int iDel = 1;
					if (stWhat.equals("children")) {
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("delete from Test where nmProjectId=" + stPk
						        + " and nmBaseline=" + this.nmBaseline + " "
						        + "and TstId > " + rs1.getString("TstId") + " and TstId < "
						        + iTstIdEnd);
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Test set TstLevel=(TstLevel-1) where nmProjectId="
						        + stPk
						        + " and nmBaseline="
						        + this.nmBaseline
						        + " "
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstId=(TstId-" + iDel
					        + ") where TstId > " + rs1.getString("TstId")
					        + " and  nmProjectId=" + stPk + " and nmBaseline="
					        + this.nmBaseline + " " + "and TstId > "
					        + rs1.getString("TstId"));
				} else if (stAction.equals("demote")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Test set TstLevel=(TstLevel-1) where RecId="
					        + iRecIdDo);
					String stWhat = this.ebEnt.ebUd.request.getParameter("what");
					if (stWhat.equals("children")) {
						this.ebEnt.dbDyn
					...
					if (stB != null) {
						...
						try {
							...
							this.ebEnt.dbDyn.ExecuteUpdate("update Test set TstFlags=1 "
							    + "where nmProjectId=" + stPk + " and nmBaseline="
			...
			if (stEdit != null && stEdit.length() > 0)
				...
				iEditId = -2;
			int iMaxRecords = this.ebEnt.dbDyn
			    .ExecuteSql1n("select count(*) from Test where nmProjectId=" + stPk
			        + " and nmBaseline=" + this.nmBaseline + " and TstFlags != 1");
			stSql = "select * from Test where nmProjectId=" + stPk
			    + " and nmBaseline=" + this.nmBaseline
			    + " and TstFlags != 1 order by TstId,RecId desc limit " + iFrom + ","
			    + iBlock;
			ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs.last();
			int iMax = rs.getRow();
			...
			if (iMax > 0) {
				...
				for (int iR = 1; iR <= iMax; iR++) {
					...
					if (iEditId <= 0) {
						...
						// iNextLevel + "," + iPrevLevel + "," + iNextShow + ",'" + stLink +
						// "' )\""
						s1 = "<tr  " + " onClick=" + "\"setActionsClick(3,"
						    + rs.getInt("RecId") + "," + rs.getInt("TstId") + ","
						    + rs.getInt("TstFlags") + "," + rs.getInt("TstLevel") + ","
						    + iNextLevel + "," + iPrevLevel + "," + iNextShow + ",'"
						    + stLink + "'," + iMaxFields + " )\"" + " >";
						sbReturn.append(s1);
					} else {
					...
					for (int iF = 1; iF <= this.iMaxFields; iF++) {
						...
						if (iF == 1)
							sbReturn
							    .append("<a name='row" + rs.getString("TstId") + "'></a>");
	...
	public String xlsSchedulesEdit(String stChild) {
		...
		try {
			...
			if (stAction != null && stAction.length() > 0 && stR != null
				...
				if (stAction.equals("collapse")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags | 2) where RecId= "
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags | 1) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline
					        + " "
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags & ~0x3) where RecId= "
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags & ~0x3) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline
					        + " "
					...
					int iRecMax = this.ebEnt.dbDyn
					    .ExecuteSql1n("select max(RecId) from Schedule");
					iRecMax++;
					String stWhat = this.ebEnt.ebUd.request.getParameter("what");
					try {
						...
						if (stWhat.equals("below")) {
							iEditId++;
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update Schedule set SchId=(SchId+1) where SchId > "
							        + rs1.getString("SchId")
							        + " and  nmProjectId="
							        + stPk
							        + " and nmBaseline=" + this.nmBaseline + " ");
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("insert into Schedule (RecId,nmProjectId,SchId,nmBaseline,SchLevel) "
						        + "values("
						        + iRecMax
						        + ","
						        + stPk
					...
					this.epsUd
					    .makeTask(16, this.epsUd.epsEf.getScheduleName("" + iRecIdDo,
					        stPk, nmBaseline)); // New Project Schedule
					this.ebEnt.ebUd.setRedirect(stLink + "&edit=" + iRecMax
					    + "&new=y#row" + iEditId);
					return ""; // ----------------------------->
				} else if (stAction.equals("send")) {
					ResultSet rset = this.ebEnt.dbDyn
					    .ExecuteSql("select SchEstimatedEffort, SchEfforttoDate from schedule where RecId="
					        + iRecIdDo
					        + " and  nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + this.nmBaseline);
					rset.absolute(1);
					if (rset.getInt("SchEfforttoDate") > rset
					    .getInt("SchEstimatedEffort")) {
						this.ebEnt.ebUd
						    .setPopupMessage("Expended hours may not be more than estimated hours. Please adjust estimated hours first.");
					} else {
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchMessage='Y', dtSchLastUpdate=now() where RecId="
						        + iRecIdDo
						        + " and  nmProjectId="
						        + stPk
						        + " and nmBaseline=" + this.nmBaseline);
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchLevel=(SchLevel-1) where nmProjectId="
						        + stPk
						        + " and nmBaseline="
						        + this.nmBaseline
						        + " "
						        ...
						        + " and SchId < "
						        + getEnd(stChild, stPk, rs1));
						this.ebEnt.dbDyn.ExecuteUpdate("update Schedule set SchId=(SchId-"
						    + iDel + ") where SchId > " + rs1.getString("SchId")
						    + " and  nmProjectId=" + stPk + " and nmBaseline="
						    + this.nmBaseline + " " + "and SchId > "
						    + rs1.getString("SchId"));
					}
				} else if (stAction.equals("delete")) {
					this.ebEnt.dbDyn.ExecuteUpdate("delete from Schedule where RecId="
					    + iRecIdDo + " and  nmProjectId=" + stPk + " and nmBaseline="
					    + this.nmBaseline);
					String stWhat = this.ebEnt.ebUd.request.getParameter("what");
					int iDel = 1;
					if (stWhat.equals("children")) {
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("delete from Schedule where nmProjectId=" + stPk
						        + " and nmBaseline=" + this.nmBaseline + " "
						        + "and SchId > " + rs1.getString("SchId") + " and SchId < "
						        + iSchIdEnd);
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchId=(SchId-" + iDel
					        + ") where SchId > " + rs1.getString("SchId")
					        + " and  nmProjectId=" + stPk + " and nmBaseline="
					        + this.nmBaseline + " " + "and SchId > "
					        + rs1.getString("SchId"));
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchLevel=(SchLevel-1) where RecId="
					        + iRecIdDo
					        + " and nmProjectId="
					        + stPk
					        + " and nmBaseline="
					...
					if (stB != null) {
						...
						try {
							int iLevel = Integer.parseInt(stB);
							this.ebEnt.dbDyn
							    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags | 1) "
							        + "where nmProjectId="
							        + stPk
							        + " and nmBaseline="
							        + this.nmBaseline + " and SchLevel > " + iLevel);
						} catch (Exception e) {
						}
						setFlags(stChild);
					}
			...
			String stHeader = xlsHeaders(stChild);
			stTemp = "</form><form method=post name='form" + stChild + "' id='form"
			    + stChild + "' onsubmit='return myValidation(this)'>"
			    + "<input type=hidden name=from value='" + iFrom
			    + "'><table class=l1table><tr><td class=l1td colspan=" + iMaxFields
			    + " align=center><h2>" + this.rsProject.getString("ProjectName")
			    + "</h2></td></tr>";
			...
			if (stEdit != null && stEdit.length() > 0)
				...
				iEditId = -2;
			int iMaxRecords = this.ebEnt.dbDyn
			    .ExecuteSql1n("select count(*) from Schedule where nmProjectId="
			        + stPk + " and nmBaseline=" + this.nmBaseline
			        + " and (SchFlags & 0x1) != 1");
			...
			stSql = "select * from Schedule where nmProjectId=" + stPk
			    + " and nmBaseline=" + this.nmBaseline
			    + " and (SchFlags & 0x1) != 1 order by SchId,RecId desc limit "
			    + iFrom + "," + iBlock;
			ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs.last();
			int iMax = rs.getRow();
			               ...
			               // we auto insert
			{
				iFrom = 0; // reset to 0
				rs.close();
				stSql = "select * from Schedule where SchMessage IS NULL and nmProjectId="
				    + stPk
				    + " and nmBaseline="
				    + this.nmBaseline
				    + " and (SchFlags & 0x1) != 1 order by SchId,RecId desc limit "
			...
			if (iMax > 0) {
				...
				for (int iR = 1; iR <= iMax; iR++) {
					                                                            ...
					                                                            // and PPM
					{
						...
						// iNextLevel + "," + iPrevLevel + "," + iNextShow + ",'" + stLink +
						// "' )\""
						sbReturn.append("<tr  " + " onClick=" + "\"setActionsClick(1,"
						    + rs.getInt("RecId") + "," + rs.getInt("SchId") + ","
						    + rs.getInt("SchFlags") + "," + rs.getInt("SchLevel") + ","
						    + iNextLevel + "," + iPrevLevel + "," + iNextShow + ",'"
						    + stLink + "'," + iMaxFields + " )\"" + " >");
					} else {
						sbReturn.append("<tr>");
					...
					for (int iF = 1; iF <= this.iMaxFields; iF++) {
						...
						if (rsFields.getInt("nmForeignId") == 825
						    && this.ebEnt.ebUd.request.getParameter("edit") != null) {
							// don't append labor categories if inline edit but store labor
							...
							if (stValue == null)
								stValue = "";
							if (this.rsFields.getInt("nmDataType") == 5
							    && stValue.length() > 0) {
								sbReturn.append(this.epsUd.rsMyDiv.getString("stMoneySymbol")
								    + " "); // inline fields
							}
							if (rsFields.getInt("nmForeignId") == 813
							    && iEditId == rs.getInt("RecId")
							    && (rsFields.getInt("nmFlags") & 0x1) != 0) {
								...
								if (!stValue.equals("")
								...
								} else if (stValue.equals("")
								    && rsFields.getString("stDbFieldName").equals(
								        "SchLaborCategories")) {
									stValue = makeListValue(rsFields, "");
									sbReturn.append(stValue.replace("\n", "<BR>"));
								} else if (rsFields.getString("stDbFieldName").equals(
								    "SchDependencies")) {
									// stValue = makeListValue(rsFields, stValue+"");
									String stdSql = "select * from teb_link where nmProjectId="
									    + this.stPk + " and nmBaseline=" + this.nmBaseline
									    + " and nmLinkFlags=2 and nmToId=" + rs.getInt("RecId")
									...
									sbReturn.append(stValue.replace("\n", "<BR>"));
								} else if (rsFields.getString("stDbFieldName")
								    .equals("SchDone")) {
									if (stValue.equals("Y")) {
								...
								// The low-level (LL) indicator is not being set for a task that
								// is low level when you display a schedule task's attributes.
								// #110
					...
					if (iEditId == rs.getInt("RecId")
						...
						sbReturn
						    .append("<tr><td "
						        + tbstyle
						        + "><input type=submit name=savedataxls value='Save' onClick=\"return setSubmitId(9970);\"></td>"
						        + "<td "
						        + tbstyle
						...
						sbReturn
						    .append("<table class='l1table'><tr class=d1>"
						...
						if (iRecMax > 0) { // add existing labor category rows
							for (iRa = 0; iRa < iRecMax; iRa++) {
								...
								stReturn += "<td " + stRowspan + "><select name=lc_" + iRa
								    + " id=lc_" + iRa + ">";
								ResultSet rsLcAll = this.ebEnt.dbDyn
								...
								stReturn += "</select></td>";
								stReturn += "<td " + stRowspan + "><select name=nr_" + iRa
								    + " id=nr_" + iRa + ">";
								...
								stReturn += "<td "
								    ...
								    + iRa + " id=est_" + iRa + " value=\"" + aFields[2]
								    + "\"></td>";
								stReturn += "<td "
								    + stRowspan
								    + ">"
								    + this.epsUd.epsEf.getMulitUsers(stChild, 9001, "_must_"
								        + iRa, aFields[3], "lc_" + iRa) + "</td>";
								stReturn += "<td "
								    + stRowspan
								    + ">"
								    + this.epsUd.epsEf.getMulitUsers(stChild, 9002, "_most_"
								        + iRa, aFields[4], "lc_" + iRa) + "</td>";
								stReturn += "<td "
								    + stRowspan
								    + ">"
								    + this.epsUd.epsEf.getMulitUsers(stChild, 9003, "_least_"
								        + iRa, aFields[5], "lc_" + iRa) + "</td>";
								stReturn += "<td "
								    + stRowspan
								    + ">"
								    + this.epsUd.epsEf.getMulitUsers(stChild, 9004, "_not_"
								        + iRa, aFields[6], "lc_" + iRa) + "</td>";
								stReturn += "</tr>";
							}
						}
						...
						stReturn += "<td " + stRowspan
						    + " align='left'>Add New:<br><select name=lc_" + iRa
						    + " id=lc_" + iRa + ">";
						ResultSet rsLcAll = this.ebEnt.dbDyn
						    .ExecuteSql("SELECT * FROM LaborCategory order by LaborCategory");
						rsLcAll.last();
						...
						// populate dropdown
						stReturn += this.ebEnt.ebUd.addOption2(
						    "-- Select Labor Category --", "0", "");
						for (int iLc = 1; iLc <= iMaxLc; iLc++) {
							rsLcAll.absolute(iLc);
							stReturn += this.ebEnt.ebUd.addOption2(
							    rsLcAll.getString("LaborCategory"),
							    rsLcAll.getString("nmLcId"), "");
						}
						stReturn += "</select></td>";
						stReturn += "<td " + stRowspan + "><select name=nr_" + iRa
						    + " id=nr_" + iRa + ">";
						...
						stReturn += "<td " + stRowspan
						    + "><input type=text size=5 style='text-align:right' name=est_"
						    + iRa + " id=est_" + iRa + " value=\"\"></td>";
						stReturn += "<td "
						    + stRowspan
						    + ">"
						    + this.epsUd.epsEf.getMulitUsers(stChild, 9001, "_must_" + iRa,
						        "", "lc_" + iRa) + "</td>";
						stReturn += "<td "
						    + stRowspan
						    + ">"
						    + this.epsUd.epsEf.getMulitUsers(stChild, 9002, "_most_" + iRa,
						        "", "lc_" + iRa) + "</td>";
						stReturn += "<td "
						    + stRowspan
						    + ">"
						    + this.epsUd.epsEf.getMulitUsers(stChild, 9003,
						        "_least_" + iRa, "", "lc_" + iRa) + "</td>";
						stReturn += "<td "
						    + stRowspan
						    + ">"
						    + this.epsUd.epsEf.getMulitUsers(stChild, 9004, "_not_" + iRa,
						        "", "lc_" + iRa) + "</td>";
						stReturn += "</tr>";

						// close the labor category table
						stReturn += "</table><input type=hidden name=imax id=imax value='"
						    + (iRecMax + 1) + "'>";
	...
	public void setFlags(String stChild) {
		...
		try {
			...
			{
				stSql = "select RecId,ReqId,ReqFlags,ReqLevel from Requirements where nmProjectId="
				    + this.rsProject.getString("RecId")
				    + " and nmBaseline="
				    + this.nmBaseline + " and (ReqFlags & 0x1) = 1";
				stOrder = " order by ReqId";
			} else if (stChild.equals("21")) // Requirements
			{
				stSql = "select RecId,SchId,SchFlags,SchLevel from Schedule where nmProjectId="
				    + this.rsProject.getString("RecId")
				    + " and nmBaseline="
				    + this.nmBaseline + " and (SchFlags & 0x1) = 1 ";
				stOrder = " order by SchId";
			} else {
				stSql = "select RecId,TstId,TstFlags,TstLevel from Test where nmProjectId="
				    + this.rsProject.getString("RecId")
				    + " and nmBaseline="
				    + this.nmBaseline + " and (TstFlags &0x1) = 1";
			...
			for (int iR = 1; iR <= iMax; iR++) {
				...
				if ((iLastId + 1) != rs1.getInt(2)) { // Out of sequence ... set parent
					...
					{
						if (rs1.getInt(2) > 2)
							iFlags2above = this.ebEnt.dbDyn
							    .ExecuteSql1n("select ReqFlags from Requirements where nmProjectId="
							        + this.rsProject.getString("RecId")
							        + " and nmBaseline="
							        + this.nmBaseline + " and ReqId=" + (rs1.getInt(2) - 2));
						if (iFlags2above != 0)
							iNewFlags = 4; // Record ABOVE is hidden
						else
							iNewFlags = 2;
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Requirements set ReqFlags =( ReqFlags | "
						        + iNewFlags
						        + ") where nmProjectId="
						        + this.rsProject.getString("RecId")
						        + " and nmBaseline="
						        + this.nmBaseline + " and ReqId=" + (rs1.getInt(2) - 1));
					} else if (stChild.equals("21")) // Schedule
					{
						if (rs1.getInt(2) > 2)
							iFlags2above = this.ebEnt.dbDyn
							    .ExecuteSql1n("select SchFlags from Schedule where nmProjectId="
							        + this.rsProject.getString("RecId")
							        + " and nmBaseline="
							        + this.nmBaseline + " and SchId=" + (rs1.getInt(2) - 2));
						if (iFlags2above != 0)
							iNewFlags = 4; // Record ABOVE is hidden
						else
							iNewFlags = 2;
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchFlags= (SchFlags | "
						        + iNewFlags + ") where nmProjectId="
						        + this.rsProject.getString("RecId") + " and nmBaseline="
						        + this.nmBaseline + " and SchId=" + (rs1.getInt(2) - 1));
					} else // Test
					{
						if (rs1.getInt(2) > 2)
							iFlags2above = this.ebEnt.dbDyn
							    .ExecuteSql1n("select TstFlags from Test where nmProjectId="
							        + this.rsProject.getString("RecId") + " and nmBaseline="
							        + this.nmBaseline + " and TstId=" + (rs1.getInt(2) - 2));
						if (iFlags2above != 0)
							iNewFlags = 4; // Record ABOVE is hidden
						else
							iNewFlags = 2;
						this.ebEnt.dbDyn.ExecuteUpdate("update Test set TstFlags="
						    + iNewFlags + " where nmProjectId="
						    + this.rsProject.getString("RecId") + " and nmBaseline="
						    + this.nmBaseline + " and ReqId=" + (rs1.getInt(2) - 1));
					}
	...
	public String getCustomize(String stChild, String stLink, String stPk) {
		...
		try {
			...
			sbReturn
			    .append("&r=1&a=custom\" onsubmit='return myValidation(this)' method='post'><table border=1>");
			sbReturn
			    .append("<tr><td align=right>Show levels:</td><td align=left><select name=b id=b  onChange=\"document.form4.submit();\" >");
			int iLevelMax = 0;
			if (stChild.equals("19"))
				iLevelMax = this.ebEnt.dbDyn
				    .ExecuteSql1n("select max(ReqLevel) from Requirements where nmProjectId="
				        + stPk + " and nmBaseline=" + this.nmBaseline + " ");
			else
				iLevelMax = this.ebEnt.dbDyn
				    .ExecuteSql1n("select max(SchLevel) from Schedule where nmProjectId="
				        + stPk + " and nmBaseline=" + this.nmBaseline + " ");
			sbReturn.append(this.ebEnt.ebUd.addOption("-- Level --", "", ""));
			sbReturn.append(this.ebEnt.ebUd
			    .addOption("All", "" + (iLevelMax + 1), ""));
	...
	private String doNext(String stChild, int iCol, int iMaxRecords, int iFrom,
	    int iBlock, String stGoBack, String stPk) {
		StringBuilder sbReturn = new StringBuilder(5000);
	...
	private int getEnd(String stChild, String stPk, ResultSet rs1) {
		...
		try {
			if (stChild.equals("19")) {
				iReturn = this.ebEnt.dbDyn
				    .ExecuteSql1n("select min(ReqId) from Requirements where nmProjectId="
				        + stPk
				        + " and nmBaseline="
				        + this.nmBaseline
				        + " and ReqId > "
				...
				if (iReturn <= 0)
					iReturn = this.ebEnt.dbDyn
					    .ExecuteSql1n("select (max(ReqId)+1) from Requirements where nmProjectId="
					        + stPk + " and nmBaseline=" + this.nmBaseline);
			} else if (stChild.equals("21")) {
				iReturn = this.ebEnt.dbDyn
				    .ExecuteSql1n("select min(SchId) from Schedule where nmProjectId="
				        + stPk + " and nmBaseline=" + this.nmBaseline + " and SchId > "
				        + rs1.getString("SchId") + " and SchLevel <= "
				        + rs1.getString("SchLevel"));
				if (iReturn <= 0)
					iReturn = this.ebEnt.dbDyn
					    .ExecuteSql1n("select (max(SchId)+1) from Schedule where nmProjectId="
					        + stPk + " and nmBaseline=" + this.nmBaseline);
			} else {
				iReturn = this.ebEnt.dbDyn
				    .ExecuteSql1n("select min(TstId) from Test where nmProjectId="
				        + stPk + " and nmBaseline=" + this.nmBaseline + " and TstId > "
				        + rs1.getString("TstId") + " and TstLevel <= "
				        + rs1.getString("TstLevel"));
				if (iReturn <= 0)
					iReturn = this.ebEnt.dbDyn
					    .ExecuteSql1n("select (max(TstId)+1) from Test where nmProjectId="
					        + stPk + " and nmBaseline=" + this.nmBaseline);
			}
		} catch (Exception e) {
		...
		try {
			...
			case 4:
				s1 = "<textarea name=f" + rsFields.getString("nmForeignId") + " id=f"
				    + rsFields.getString("nmForeignId") + " rows=4 " + stStyle + ">"
				    + stValue + "</textarea>";
				sbReturn.append(s1);
				break;
			default:
				if ((rsFields.getString("nmForeignId").equals("231") && Integer
				    .parseInt(this.ebEnt.ebUd.request.getParameter("child")) == 19)
				    || (rsFields.getString("nmForeignId").equals("271") && Integer
				        .parseInt(this.ebEnt.ebUd.request.getParameter("child")) == 21)) {
					// drop down for req level 0-x+1
					s1 = "<select " + stStyle + " type=text name=f"
					...
					s1 += "<option value=\"" + stValue + "\" selected='selected'>"
					    + stValue + "</option>";
					if (stNew != null && "y".equals(stNew)) {
						s1 += "<option value=\"" + (Integer.parseInt(stValue) + 1) + "\">"
					...
					if (stValue.equals("Y"))
						s1 = "<input type='checkbox' checked='checked' name=f"
						    + rsFields.getString("nmForeignId") + " id=f"
						    + rsFields.getString("nmForeignId") + ">";
					else
						s1 = "<input type='checkbox' name=f"
						    + rsFields.getString("nmForeignId") + " id=f"
						    + rsFields.getString("nmForeignId") + ">";
					sbReturn.append(s1);
				} else {
					s1 = "<input '" + stStyle + "' maxsize="
					    + rsFields.getString("nmMaxBytes") + " type=text name=f"
					    + rsFields.getString("nmForeignId") + " id=f"
				...
				/*
				 * if(rsFields.getString("nmForeignId").equals("271")){ //add hidden
				 * value for current level s1 =
				                                                                              ...
				                                                                              // non-schedule
				{
					...
					if (stTemp == null || stTemp.length() <= 0) {
						sbReturn.append("&nbsp;");
						sbReturn
						    .append("<input type=submit name=savedataxls value='Cancel' onClick=\"return setSubmitId(8888);\">");
					}
	...
	private void saveInline(String stChild, String stPk) {
		...
		try {
			...
			for (int iF = 1; iF <= this.iMaxFields; iF++) {
				                                                               ...
				                                                               // checkbox
				{
					...
					if (stTemp != null && stTemp.length() > 0
						...
						stValue = this.epsUd.validateD22(rsTable, this.rsFields,
						    this.ebEnt.ebUd.request.getParameter("edit"), stValue, stPk,
						    nmBaseline);
					} else if (stTemp != null && stTemp.length() > 0
					    && stTemp.toLowerCase().equals("d50")) {
						stValue = this.epsUd.validateD50(rsTable, this.rsFields,
						    this.ebEnt.ebUd.request.getParameter("edit"), stValue, stPk,
						    nmBaseline);
					} else if (stTemp != null && stTemp.length() > 0
					    && stTemp.toLowerCase().equals("d53")) {
						stValue = this.epsUd.validateD53(rsTable, this.rsFields,
						    this.ebEnt.ebUd.request.getParameter("edit"), stValue, stPk,
						    nmBaseline);
					} else if (this.rsFields.getString("stDbFieldName").equals(
					    "SchEstimatedEffort")
					    && stChild.equals("21")) {
						stValue = this.epsUd.validateEffort(this.rsFields,
						    this.ebEnt.ebUd.request.getParameter("edit"), stValue, stPk,
						    nmBaseline);
					}
					if (iCount > 0)
						stSql += ",";
					stSql += this.rsFields.getString("stDbFieldName") + "="
					    + this.ebEnt.dbDyn.fmtDbString(stValue);
					iCount++;

					if (this.rsFields.getString("stDbFieldName").equals("ReqLevel")
					    && stChild.equals("19")) {
			...
			if (stChild.equals("21")
			    && this.ebEnt.ebUd.request.getParameter("f" + doneID) != null) {
				if (iCount > 0)
					stSql += ",";
			...
			if (this.ebEnt.ebUd.request.getParameter("lc_0") != null
				...
				String lcfields = "";
				for (int iR = 0; iR < Integer.parseInt(this.ebEnt.ebUd.request
				    .getParameter("imax")); iR++) {
					if (iR == 0)
						stSql += ", SchLaborCategories=";
					else if (!this.ebEnt.ebUd.request.getParameter("lc_" + iR)
					...
					if (!this.ebEnt.ebUd.request.getParameter("lc_" + iR).equals("0")) { // skip
						...
						lcfields += this.ebEnt.ebUd.request.getParameter("lc_" + iR) + "~"
						    + this.ebEnt.ebUd.request.getParameter("nr_" + iR) + "~"
						    + stEffort + "~"
			...
			if (stChild.equals("21")) {
				stSql += ", SchEstimatedEffort="
				    + (new DecimalFormat("#0.0").format(nmEffort));

				// popup messages for larger 40 hours
				String sql = "SELECT * FROM schedule where nmProjectId=" + stPk
				    + " and nmBaseline=" + nmBaseline + " and RecId="
				    + this.ebEnt.ebUd.request.getParameter("edit");
				String lowlvl = this.ebEnt.dbDyn.ExecuteSql1(sql);
				if (lowlvl != null && Integer.parseInt(lowlvl) > 0
				    && nmEffort > epsUd.rsMyDiv.getDouble("MaximumTaskHours")) {
					this.ebEnt.ebUd.setPopupMessage("Estimate (" + nmEffort
			...
			if (Integer.parseInt(this.ebEnt.ebUd.request.getParameter("child")) == 21
				...
				String dfields = "";
				for (int d = 1; d <= Integer.parseInt(this.ebEnt.ebUd.request
				    .getParameter("dmax")); d++) {
					if (d == 1) {
					...
					if (this.ebEnt.ebUd.request.getParameter("did_" + d) != null
						...
						dfields += this.ebEnt.ebUd.request.getParameter("did_" + d)
						    + this.ebEnt.ebUd.request.getParameter("type_" + d) + "+"
						...
						linkMap(
						    ...
						    nmBaseline,
						    stPk,
						    Integer.parseInt(this.ebEnt.ebUd.request.getParameter("did_"
						        + d)),
						    stPk,
						    Integer.parseInt(this.ebEnt.ebUd.request.getParameter("edit")),
						    Double.parseDouble(this.ebEnt.ebUd.request.getParameter("lag_"
						        + d)), this.ebEnt.ebUd.request.getParameter("type_" + d),
						    0, 0);
						...
						linkMap(
						    ...
						    Integer.parseInt(this.ebEnt.ebUd.request.getParameter("did_"
						        + d)), stPk,
						    Integer.parseInt(this.ebEnt.ebUd.request.getParameter("edit")),
						    0, this.ebEnt.ebUd.request.getParameter("type_" + d), 0, 0);
					}
				}
				stSql += this.ebEnt.dbDyn.fmtDbString(dfields);
			}

			// calc 1 level below if this level is not 0. order by desc and find first
			...
			if (Integer.parseInt(this.ebEnt.ebUd.request.getParameter("child")) == 19) {
				...
				if (rLvl > 0) {
					...
					int rId = this.ebEnt.dbDyn
					    .ExecuteSql1n("SELECT ReqId FROM requirements WHERE RecId = "
					        + this.ebEnt.ebUd.request.getParameter("edit"));
					stSql += ", ReqParentRecId="
					    + this.ebEnt.dbDyn.fmtDbString(this.ebEnt.dbDyn
					        .ExecuteSql1n("SELECT RecId FROM requirements WHERE ReqId < "
					            + rId + " AND ReqLevel=" + (rLvl - 1)
					            + " AND nmProjectId=" + stPk + " AND nmBaseLine="
					            + nmBaseline + " ORDER BY ReqId DESC")
					        + "");
				}
			}
	...
	private String editFull(String stChild, String stPk) {
		...
		try {
			...
			if (stSave != null || stCancel != null || iDelete >= 0) { // DO SAVEDATA()
				...
				if (stParent == null)
					stParent = "";
				String stLink = "?stAction=projects&t="
				    + this.ebEnt.ebUd.request.getParameter("t") + "&do=xls&pk="
				    + this.ebEnt.ebUd.request.getParameter("pk") + "&parent="
				    + stParent + "&child=" + stChild;
				if (stSave != null && stSave.length() > 0) {
					if (!stSave.toLowerCase().contains("and")) {
						...
						case 9994: // Other Resources
							...
							if (stTemp != null && stTemp.length() > 0) {
								...
								for (int i = 0; i < iMax; i++) {
									if (i == iDelete)
										continue;
									String stCst = this.ebEnt.ebUd.request.getParameter("cst_"
									    + i);
									if (stCst != null && stCst.length() > 0 && !stCst.equals("0")) {
										if (SchOtherResources.length() > 0)
											SchOtherResources += "|";
										SchOtherResources += this.ebEnt.ebUd.request
										    .getParameter("oth_" + i) + "~";
										SchOtherResources += this.ebEnt.ebUd.request
										    .getParameter("cst_" + i) + "~";
									}
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update Schedule set SchOtherResources = "
								        + this.ebEnt.dbDyn.fmtDbString(SchOtherResources)
								        + " where nmProjectId=" + stPrj + " and nmBaseline="
								        + iBaseline + " and RecId="
						...
						case 9993: // Inventory
							...
							if (stTemp != null && stTemp.length() > 0) {
								...
								String SchInventory = "";
								for (int i = 0; i < iMax; i++) {
									String stQty = this.ebEnt.ebUd.request.getParameter("qty_"
									    + i);
									if (i == iDelete) { // Add QTY
										if (stQty != null && stQty.length() > 0
										    && !stQty.equals("0"))
											this.ebEnt.dbDyn.ExecuteUpdate("update Inventory set"
											    + " DateLastUsed = now(), ProjectLastUsing = "
											    + stPrj + " where RecId ="
											    + this.ebEnt.ebUd.request.getParameter("inv_" + i));
										this.ebEnt.dbDyn
										    .ExecuteUpdate("delete from teb_refinventory where nmProjectId="
										        + stPrj
										        + " and nmInventoryId="
										        + this.ebEnt.ebUd.request.getParameter("inv_" + i)
										        + " and nmTaskId=" + stPk);
									...
									if (stQty != null && stQty.length() > 0 && !stQty.equals("0")) {
										...
										try {
											int iMissing = 0;
											iQty = Integer.parseInt(this.ebEnt.ebUd.request
											    .getParameter("qty_" + i));
											if (iQty > 0) {
												ResultSet rsAvail = this.ebEnt.dbDyn
												    .ExecuteSql("select Quantity,sum(ri.nmQty) as used,sum(ri.nmMissing) missing"
												        ...
												        + " )"
												        + " where i.RecId="
												        + this.ebEnt.ebUd.request.getParameter("inv_"
												            + i));
												rsAvail.absolute(1);
												int iAvail = rsAvail.getInt("Quantity")
												    - rsAvail.getInt("used")
												    + rsAvail.getInt("missing");
												if (iAvail < iQty) {
													this.ebEnt.ebUd.setPopupMessage("Not enough ("
													    + iAvail + ") resources available. "
													    + "You requested (" + iQty + ")");
													iMissing = iQty - iAvail;
												}
												this.ebEnt.dbDyn
												    .ExecuteUpdate("replace into teb_refinventory"
										...
										if (SchInventory.length() > 0)
											SchInventory += "|";
										SchInventory += this.ebEnt.ebUd.request.getParameter("inv_"
										    + i)
										    + "~";
										SchInventory += iQty + "~";
									}
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update Schedule set SchInventory = "
						...
						case 9992: // Successors
							...
							String stSql = "delete from teb_link where nmProjectId=" + stPrj
							    + " and nmBaseline=" + iBaseline + " and nmLinkFlags=2 "
							...
							if (stTemp != null && stTemp.length() > 0) {
								...
								for (int i = 1; i <= iMax; i++) {
									if (i != iDelete) {
										String stToProject = this.ebEnt.ebUd.request
										    .getParameter("prj_" + i);
										String stToId = this.ebEnt.ebUd.request.getParameter("id_"
										    + i);
										if (stToProject != null && stToProject.length() > 0
										    && stToId != null && stToId.length() > 0) {
											ResultSet rs1 = this.ebEnt.dbDyn
											    .ExecuteSql("select * from Projects p left join Schedule s"
											        + " on s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline and s.RecId="
											        + stToId + " where  p.RecId=" + stToProject);
											rs1.absolute(1);
											if (rs1.getString("SchFlags") == null
											    || (rs1.getInt("SchFlags") & 0x10) == 0) {
												if (rs1.getString("SchFlags") == null)
													this.ebEnt.ebUd.setPopupMessage("Project: "
													    + rs1.getString("ProjectName") + " Task ID: "
													    + stToId + " not found");
												else
													this.ebEnt.ebUd.setPopupMessage("Project: "
													    + rs1.getString("ProjectName") + " Task ID: "
													    + stToId + " is not a low level schedule");
											} else {
												int iTo1 = 0;
												if (stToId != null && stToId.length() > 0)
													iTo1 = Integer.parseInt(stToId);
												String stLag = this.ebEnt.ebUd.request
												    .getParameter("lag_" + i);
												double dLag = 0;
												                         ...
												                         // to FS
												{
													...
													stSuc += iTo1;
													linkMap(2, stPrj, iBaseline, stPrj, iFrom,
													    stToProject, iTo1, dLag, stType, 0, 0);
												}
											}
										} else {
											if (i != iMax) // Only last one may have missing ID
												this.ebEnt.ebUd
												    .setPopupMessage("Invalid Project and/or ID " + i);
										}
									}
								}
								if (iForeign > 0)
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update Schedule set SchSuccessors = "
						...
						case 9991: // Dependents
							...
							stR = this.ebEnt.ebUd.request.getParameter("r");
							String stDep = "";
							stSql = "delete from teb_link where nmProjectId=" + stPrj
							    + " and nmBaseline=" + iBaseline + " and nmLinkFlags=2 "
							...
							if (stTemp != null && stTemp.length() > 0) {
								...
								for (int i = 1; i <= iMax; i++) {
									if (i != iDelete) {
										String stFromProject = this.ebEnt.ebUd.request
										    .getParameter("prj_" + i);
										String stFromId = this.ebEnt.ebUd.request
										    .getParameter("id_" + i);
										if (stFromProject != null && stFromProject.length() > 0
										    && stFromId != null && stFromId.length() > 0) {
											ResultSet rs1 = this.ebEnt.dbDyn
											    .ExecuteSql("select * from Projects p left join Schedule s"
											        + " on s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline and s.RecId="
											...
											if (rs1.getString("SchFlags") == null
												...
												if (rs1.getString("SchFlags") == null)
													...
													this.ebEnt.ebUd.setPopupMessage("Project: "
													    + rs1.getString("ProjectName") + " Task ID: "
													    + stFromId + " is not a low level schedule");
											} else {
												int iFrom = 0;
												                         ...
												                         // to FS
												{
													...
													linkMap(2, stPrj, iBaseline, stPrj, iTo,
													    stFromProject, iFrom, dLag, stType, 0, 0);
												} else {
													if (stDep.length() > 0)
														stDep += ", ";
													if (!stFromProject.equals(stPrj))
														stDep += stFromProject + ".";
													stDep += iFrom;
													linkMap(2, stPrj, iBaseline, stFromProject, iFrom,
													    stPrj, iTo, dLag, stType, 0, 0);
												}
											}
										} else {
											if (i != iMax) // Only last one may have missing ID
								...
								for (int i = 0; i < iMax; i++) {
									...
									if (stLc != null && !stLc.equals("0")) {
										if (SchLaborCategories.length() > 0)
											SchLaborCategories += "|";
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("lc_" + i) + "~";
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("nr_" + i) + "~";
										stTemp = this.ebEnt.ebUd.request.getParameter("est_" + i);
										double d = Double.parseDouble(stTemp);
										int iNr = Integer.parseInt(this.ebEnt.ebUd.request
										    .getParameter("nr_" + i));
										double d2 = d;
										if (iNr > 0)
											d2 = d / iNr;
										...
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("f9001_must_" + i) + "~";
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("f9002_most_" + i) + "~";
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("f9003_least_" + i) + "~";
										SchLaborCategories += this.ebEnt.ebUd.request
										    .getParameter("f9004_not_" + i) + "~";
									}
								}
								// if (dEffort > dMax)
								// {
								// this.ebEnt.ebUd.setPopupMessage("Estimate (" +
								// df.format(dEffort) + ") exceeds maximum task hours (" +
						...
						if (giSubmitId >= 9980 && giSubmitId <= 9990) {
							this.ebEnt.ebUd.setRedirect(stLink + "&from=" + stFrom
							    + "&a=editfull&r="
			...
			if (giSubmitId != 8888) // 9970-9979 is Save and Exit;
			{
				...
				if ((giSubmitId <= 0 && stTemp != null && stTemp.length() > 0)
				    || (giSubmitId >= 9980 && giSubmitId <= 9990)) {
					return this.editMake(stChild, stPk);
				}
			}

			String s1 = "<center><table><tr><td valign=middle><h2>"
			    + this.rsProject.getString("ProjectName")
			    + "</h2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
			sbReturn.append(s1);
			if (stChild.equals("21")) {
				sbReturn.append("<td align=left><b>");
				sbReturn.append(this.epsUd.epsEf.fullSchTitle(stPk, this.stPk,
				    this.nmBaseline));
				sbReturn.append("</b><br>&nbsp;</td>");
			} else if (stChild.equals("19")) {
				sbReturn.append("<td align=left><b>");
				sbReturn.append(this.epsUd.epsEf.fullReqTitle(stPk, this.stPk,
				    this.nmBaseline));
				sbReturn.append("</b><br>&nbsp;</td>");
			}
	...
	public String xlsAnalyze() {
		...
		if (this.ebEnt.ebUd.request.getParameter("pk") != null) {
			...
			sbReturn
			    .append("<tr><th>Prj</th><th>Module</th><th>Verify</th><th colspan=2>Comment/Output</th><th>Rslt</th><th>#db</th><th>Time</th></tr>");
		}
		try {
			this.iMaxPath = 0;
		...
		this.ebEnt.dbDyn.ExecuteUpdate("update teb_baseline set nmAnalyzeStatus="
		    + this.iAnalyzeStatus + ",dtLastAnalyze=now()," + "stAnalyzeReport="
		    + this.ebEnt.dbDyn.fmtDbString(sbReturn.toString())
		    + " where nmProjectId=" + stPk + " and nmBaseline=" + nmBaseline);
		if (this.ebEnt.ebUd.request.getParameter("pk") != null) {
			int nmPPM = ebEnt.dbDyn
			    .ExecuteSql1n("select ProjectPortfolioManagerAssignment from Projects where RecId="
			        + stPk);
			String s1 = "";
			sbReturn.append("</table><br>&nbsp;<br><center>");
			if (stChild.length() <= 0 && this.iAnalyzeStatus == 0
			    && ebEnt.ebUd.getLoginId() == nmPPM) {
				s1 = "<input type=button onClick=\"parent.location='" + stGoBack
				    + "&pk=" + stPk
				    + "&do=approve'\" value='Approve'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
				sbReturn.append(s1);
			}
			s1 = "<input type=button onClick=\"parent.location='" + stGoBack + "&pk="
			    + stPk + "&do=edit'\" value='Go Back to Projects'>";
			sbReturn.append(s1);
			if (stChild.length() <= 0 && this.iAnalyzeStatus == 0
			    && ebEnt.ebUd.getLoginId() == nmPPM) {
	...
	private String analyzeRequirements(String stChild) {
		...
		try {
			...
			// ReqFlags 0x1 = Hidden, 0x2=Show next item hidden, 0x4= isParent, 0x8 is
			// first child, 0x10 is Lowest
			// 1) reset all flags
			this.ebEnt.dbDyn
			    .ExecuteUpdate("update Requirements set ReqFlags=0 where nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);

			// 2) set parent/child flags
			stSql = "SELECT * FROM Requirements where nmProjectId=" + stPk
			    + " and nmBaseline=" + nmBaseline + " order by ReqId;";
			ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs.last();
			String stRecIdList = "";
			String stRecIdList4 = "";
			...
			int iReq = rs.getRow();
			if (iReq >= 1) {
				sbReturn.append("<td>Found</td><td valign=top align=right>" + iReq
				    + "</td>");
				int iLastLevel = 0;
				int ReqFlags = 0;
				iCount = 0;
				for (int iR = 1; iR <= iReq; iR++) {
					...
					if (iLastLevel < rs.getInt("Reqlevel")) {
						 ...
						 * (stRecIdList4.length() > 0) stRecIdList4 += ","; stRecIdList4 +=
						 * rs.getInt("RecId"); }
						 */
					if ((iAnalyzeStatus & 0x2) != 0 || iR != rs.getInt("ReqId")) {
						iCount++;
						iAnalyzeStatus |= 0x2; // out of seq
						this.ebEnt.dbDyn.ExecuteUpdate("update Requirements set ReqId="
						    + iR + " where nmProjectId=" + stPk + " and nmBaseline="
						    + nmBaseline + " and RecId=" + rs.getInt("RecId"));
					}
					if (rs.getInt("Reqlevel") > 0) {
						if (aRecId[rs.getInt("Reqlevel") - 1] != rs
						    .getInt("ReqParentRecId")) {
				...
				if (stRecIdList.length() > 0)
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags=(ReqFlags | 0x8 ) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + nmBaseline
					        + " and RecId in ("
				...
				if (stRecIdList4.length() > 0)
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements set ReqFlags=(ReqFlags | 0x4) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + nmBaseline
					        + " and RecId in ("
				...
				stSql = "SELECT * FROM Requirements where nmProjectId=" + stPk
				    + " and nmBaseline=" + nmBaseline + " order by ReqId desc";
				...
				this.ebEnt.dbDyn
				    .ExecuteUpdate("update Requirements set ReqFlags = ( ReqFlags | 0x10 ) where nmProjectId="
				        + stPk
				        + " and nmBaseline="
				        + nmBaseline
				        + " and RecId in ("
				...
				this.ebEnt.dbDyn.ExecuteUpdate("update Projects set nmReq=" + iReq
				    + " where RecId=" + stPk);
			} else {
				sbReturn
				    .append("<td colspan=2 style='color:red;'>No requirements found</td><td style='color:red;'>fail</td>");
				iAnalyzeStatus |= 0x1;
				this.epsUd.makeMessage(stProjectName, ppm + "," + pm + "," + ba + ","
				    + sp, "Requirement Warning", "No requirements found",
	...
	private String analyzeSchedules(String stChild) {
		...
		try {
			...
			sbReturn.append("<td>ID's and Levels</td>");
			/*
			 * 1) reset all flags except permanent 0x1 Hide This item 0x2 CP=Critical
			 * Path 0x4 is parent 0x8 is first child 0x10 is lowest level / leaf leve
			 * 0x20 F=Fixed Date 0x40 L=Late Task from Original Schedule 0x80
			 * PLA=Permanent Labor Assignment 0x100 FD = Foreign Dependency 0x1000
			 * Completed 0x2000 D=Deliverable 0x4000 M=Milestone 0x8000 Reserved
			 * Permanent Flag
			 */
			this.ebEnt.dbDyn
			    .ExecuteUpdate("update Schedule set SchFlags= (SchFlags & ~0x0FFF) where nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);

			// 2) set parent/child flags
			stSql = "SELECT * FROM Schedule where nmProjectId=" + stPk
			    + " and nmBaseline=" + nmBaseline + " order by SchId;";
			ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs.last();
			String stRecIdList = "";
			String stRecIdList4 = "";
			int iSch = rs.getRow();
			if (iSch >= 1) {
				sbReturn.append("<td>Found</td><td valign=top align=right>" + iSch
				    + "</td>");
				int iLastLevel = 0;
				int iLastId = 0;
				int SchFlags = 0;
				...
				for (int iR = 1; iR <= iSch; iR++) {
					...
					if ((iAnalyzeStatus & 0x8) != 0 || iR != rs.getInt("SchId")) {
						...
						this.ebEnt.dbDyn.ExecuteUpdate("update Schedule set SchId=" + iR
						    + " where nmProjectId=" + stPk + " and nmBaseline="
						    + nmBaseline + " and RecId=" + rs.getInt("RecId"));
					}
					if (rs.getInt("Schlevel") > 0) {
						if (aRecId[rs.getInt("Schlevel") - 1] != rs
						    .getInt("SchParentRecId")) {
					...
					if (total > 0) {
						...
						for (int i = 1; i <= total; i++) {
							...
							msgContent += "<tr><td>" + rs.getString("SchId") + "</td>"
							    + "<td>" + rs.getString("SchTitle") + "</td>" + "<td>"
							    + rsInventory.getString("InventoryName") + "</td>" + "<td>"
						...
						epsUd.makeMessage(
						    prjname,
						    ppm + "," + pm + "," + sp + ","
						        + epsUd.getAllUsers(epsUd.getPriviledge("ad")),
				...
				if (stRecIdList.length() > 0)
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags=(SchFlags | 0x8) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + nmBaseline
					        + " and RecId in ("
				...
				if (stRecIdList4.length() > 0)
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Schedule set SchFlags=(SchFlags | 0x4) where nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + nmBaseline
					        + " and RecId in ("
				...
				for (int iR = 1; iR <= iSch; iR++) {
					...
					if (iR == 1 || (SchFlags == 0x10)) {
						...
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchEstimatedEffort="
						        + aHours[rs.getInt("Schlevel")] + ",SchCost="
						        + aCost[rs.getInt("Schlevel")] + " where nmProjectId="
				...
				sbReturn
				    .append("<td colspan=2 style='color:red;'>No Schedules found</td><td style='color:red;'>fail</td>");
				iAnalyzeStatus |= 0x4;
				this.epsUd.makeMessage(prjname, ppm + "," + pm + "," + ba + "," + sp,
				    "Schedule Warning", "No Schedules found", dateEnd);
			}
			// 4) Process Low-leves, Rollups, check LC
			// in Sch -> must get from Schecule
			this.ebEnt.dbDyn.ExecuteUpdate("update Projects set nmSch=" + iSch
			    + " where RecId=" + stPk);
			...
			stSql = "SELECT * FROM Schedule where nmProjectId=" + stPk
			    + " and nmBaseline=" + nmBaseline + " order by SchId desc";
			ResultSet rs1 = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs1.last();
			iSch = rs1.getRow();
			int iLastLevel = -1;
			...
			for (int iR = 1; iR <= iSch; iR++) {
				...
				if (iLastLevel == rs1.getInt("Schlevel")
					...
					this.ebEnt.dbDyn
					    .ExecuteUpdate("UPDATE Schedule SET nmEffort40Flag=0 WHERE nmProjectId="
					        + stPk
					        + " and nmBaseline="
					        + nmBaseline
					        + " and RecID="
			...
			if (effortFlag > 0) {
				sbReturn.append("<td style='color:red;'>Low Level Tasks Exceeding "
				    + epsUd.rsMyDiv.getDouble("MaximumTaskHours")
				    + " Hours</td><td valign=top align=right>" + effortFlag
				    + "</td><td style='color:red;'>fail</td>");
			} else {
				sbReturn.append("<td>Complete</td><td align='right'>" + iSch
				    + "</td><td style='color:green;'>ok</td>");
			}
			sbReturn.append(this.getElapsed());
		} catch (Exception e) {
		...
		try {
			...
			if (stAction != null && stAction.equals("LOAD")) {
					...
					for (int iL = 0; iL < aLines.length; iL++) {
						if (aLines[iL].trim().startsWith("::")) {
							...
							if (aLines[iL].toLowerCase().trim().startsWith("::div")) {
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("truncate table Cost_Effectiveness_Report");
								this.ebEnt.dbDyn
								...
								iTable = 3;
								this.ebEnt.dbDyn
								    .ExecuteUpdate("delete from Users where nmUserId > 10 ");
								this.ebEnt.dbEnterprise
								    .ExecuteUpdate("delete from X25User where RecId > 10 ");
								this.ebEnt.dbEnterprise
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("delete from teb_link where nmLinkFlags=1");
								iL++; // go past field headers
								iTable = 5;
								...
								this.ebEnt.dbDyn
								    .ExecuteUpdate("truncate table teb_refinventory");
								iL++; // go past field headers
								iTable = 10;
								...
								iHoliday = iTable = 9;
								this.ebEnt.dbDyn
								    .ExecuteUpdate("update teb_division set stHolidays='' ");
								this.ebEnt.dbDyn.ExecuteUpdate("truncate table teb_allocate");
								this.ebEnt.dbDyn
								    .ExecuteUpdate("truncate table teb_allocateprj");
							...
							case 3:
								stSql = "replace into X25User (RecId,stEMail,stPassword,nmPriviledge) values("
								    + aFields[2]
								    + ","
								    + this.ebEnt.dbEnterprise.fmtDbString(aFields[13])
								    + ",old_password("
								    + this.ebEnt.dbEnterprise.fmtDbString(aFields[23])
								    + "),"
								...
								for (int iV = 0; iV < aV.length; iV++) {
									int nmLaborCategoryId = this.ebEnt.dbDyn
									    .ExecuteSql1n("select nmLcId from LaborCategory where "
									        + "LaborCategory = "
									        + this.ebEnt.dbDyn.fmtDbString(aV[iV].trim()));
									if (nmLaborCategoryId > 0) {
										stSql = "replace INTO teb_reflaborcategory "
										    ...
										    + aFields[16] + "," + aFields[17] + "," + aFields[18]
										    + ")";
										this.ebEnt.dbDyn.ExecuteUpdate(stSql);
									} else
										stError += "<br>USERS ERROR: ID: " + aFields[2]
								...
								int iPrimary = 1;
								for (int iV = 0; iV < aV.length; iV++) {
									int nmDivision = this.ebEnt.dbDyn
									    .ExecuteSql1n("select nmDivision from teb_division where "
									        + "stDivisionName = "
									        + this.ebEnt.dbDyn.fmtDbString(aV[iV].trim()));
									if (nmDivision > 0) {
								...
								if (aS != null && aS.length > 1)
									iSupervisor = this.ebEnt.dbDyn
									    .ExecuteSql1n("SELECT max(nmUserId) FROM Users where FirstName = \""
									        + aS[0].trim()
									        + "\" and LastName=\""
									        + aS[1].trim()
									        + "\" ");
								stSql = "INSERT INTO Users"
								    ...
								    + ","
								    + this.ebEnt.dbDyn.fmtDbString(this.ebEnt.ebUd
								        .fmtDateToDb(aFields[10]))
								    + ","
								    ...
								    + ","
								    + this.ebEnt.dbDyn.fmtDbString(aFields[7].replace("(", "-")
								        .replace(")", ""))
								    + ","
							...
							case 4: // ::projects
								...
								if (aV.length > 1)
									stSql = "select max(nmUserId) from Users where FirstName=\""
									    + aV[0].trim() + "\" and LastName=\"" + aV[1].trim()
									    + "\" ";
								else
									stSql = "select max(nmUserId) from Users where FirstName='' and LastName=\""
									    + aV[0].trim() + "\" ";
								...
								int[] aiFields = { 1026, 932, 1023, 1028, 923, 1031, 1027, 895,
								    811, 1068, 1069, 1070, 1071, 1072, 1073, 1074, 1075, 1076,
								    1077, 1078, 1079, 1080, 1081, 1082, 1083, 1084, 1085, 1086,
								    1087, 1088, 1089, 1090 };
								// CPI Tasks Task Done Est. Hours Hours Exp. Prj Start Est.
								...
								for (int i = 0; i < aiFields.length; i++) {
									...
									try {
										dValue = Double.parseDouble(this.ebEnt.dbDyn
										    .makeDecimal(stValue));
										iValue = Integer.parseInt(this.ebEnt.dbDyn
										    .makeDecimal(stValue));
									} catch (Exception e) {
										iValue = 0;
										dValue = 0;
									...
									stSql = "INSERT INTO teb_project "
									    ...
									    + aFields[0] + ",1," + aiFields[i] + "," + iValue + ","
									    + dValue + "," + this.ebEnt.dbDyn.fmtDbString(stValue)
							...
							case 5:
								...
								if (aFields.length >= 7 && aFields[5].length() > 0
								    && aFields[6].length() > 0) {
									...
									for (int iM = 0; iM < aMap.length; iM++) {
										try {
											...
											linkMap(1, aFields[0], nmBaseline, aFields[0], iReqId,
											    aFields[0], iSchId, dPercent, stComment, iAmortize,
											    iRemainder);
										} catch (Exception e) {
											stError += "<br>Error in mapping: ReqId " + aFields[1]
											    + " [" + aFields[5] + "] [" + aFields[6] + "]";
										}
									}
								}
									...
									try {
										...
										for (int i = 0; i < aDep.length; i++) {
											...
											if (aDep[i].trim().toLowerCase().contains("fs")) {
												stComment = "fs";
												aDep[i] = aDep[i].trim().toLowerCase()
												    .replace("fs", "");
											...
											if (aDep[i].trim().contains("+")) {
												...
												dLag = Double.parseDouble(aV2[1].trim()
											...
											if (stComment.equals("sf")) // we reverse START TO FINISH
												...
												linkMap(2, aFields[0], nmBaseline, aFields[0], iDep,
												    aFields[0], Integer.parseInt(aFields[1]), dLag,
												    stComment, 0, 0);
										}
									} catch (Exception e) {
										stError += "<BR> Error with Dependencies Schedule: "
										    + aFields[1] + " Dep: " + aFields[8].trim();
									}
								}
								String stLc = "";
								if (aFields[10].trim().length() > 0) {
									...
									for (int iV = 0; iV < aV.length; iV++) {
										...
										if (nmLaborCategoryId > 0) {
											...
											double nmHours = 0;
											try {
												nmHours = Double.parseDouble(this.epsUd
												    .makeHours(aFields[5])) / aV.length;
											} catch (Exception e) {
												stError += "<br>ProjectId " + aFields[0] + " Task ID: "
												    + aFields[1] + " Labor Category invalid EFFORT: "
												    + aV[iV].trim();
											}
											stLc += nmLaborCategoryId + "~1~" + nmHours + "~~~~";
										} else
											stError += "<br>ProjectId " + aFields[0] + " Task ID: "
											    + aFields[1] + " Labor Category not found: "
											    + aV[iV].trim();
									}
								}
							...
							case 8:
								...
								stSql = "INSERT INTO test (RecId, nmProjectId, nmBaseLine, ReqMap, TstType, TstMethod,"
								    ...
								    + this.ebEnt.dbDyn.fmtDbString(aFields[4])
								    + ","
								    + this.ebEnt.dbDyn.fmtDbString(aFields[5])
								    + ","
								    + aFields[3]
								    + ","
								    + this.ebEnt.dbDyn.fmtDbString(aFields[2])
	          ...
	          processAllRequirementCost();
				...
				sbReturn
				    .append("</form><form method=post><br>Load  Values (copy/paste from Excel)<br><textarea name=ta rows=10 cols=100>");
				sbReturn
				    .append("</textarea><br /><input type=submit name=submit value=LOAD></form>");
			}
		} catch (Exception e) {
	...
	public void linkMap(int iFlags, String stPrjId, int iBaseline,
	    String stReqPrj, int iReqRecId, String stSchPrj, int iSchRecId,
	    double dPercent, String stComment, int iAmortize, int iRemainder) {
		try {
			String stValue = this.ebEnt.dbDyn
			    .ExecuteSql1("select nmPercent from teb_link where nmProjectId="
			        + stPrjId + " and nmBaseline=" + iBaseline + " and nmLinkFlags="
			        + iFlags + " and nmFromProject=" + stReqPrj + " and nmFromId="
			        + iReqRecId + " and nmToProject=" + stSchPrj + " and nmToId="
			        + iSchRecId);
			if (stValue != null && stValue.length() > 0) {
				this.ebEnt.dbDyn.ExecuteUpdate("update teb_link set nmPercent="
				    + dPercent + ",nmAmortize=" + iAmortize + ",nmRemainder="
				    + iRemainder + ",stComment=\"" + stComment
				    + "\" where nmProjectId=" + stPrjId + " and nmBaseline="
				    + iBaseline + " and nmLinkFlags=" + iFlags + " and nmFromProject="
				    + stReqPrj + " and nmFromId=" + iReqRecId + " and nmToProject="
				    + stSchPrj + " and nmToId=" + iSchRecId);
			} else {
				this.ebEnt.dbDyn
			...
			this.stError += "<br>ERROR linkMap iReqRecId " + iReqRecId
			    + " iSchRecId " + iSchRecId + " :" + e;
		}
		return;
	}
	...
	public String analyzeLink() {
		String stHead = "<tr><td align=right>" + this.stPk
		    + "</td><td>Map/Link</td>";
		StringBuilder sbReturn = new StringBuilder(5000);
		...
		try {
			...
			if (this.ebEnt.ebUd.request.getParameter("stAction").equals("runeob")) {
					...
					if (iLastLevel == rs.getInt("ReqLevel")) {
						...
						linkHit = this.ebEnt.dbDyn
						    .ExecuteSql1n("SELECT COUNT(*) as c FROM teb_link WHERE nmProjectId="
						        ...
						        + " and nmBaseline="
						        + this.nmBaseline
						        + " and nmLinkFlags=1 and nmFromId=" + rs.getInt("RecId"));
						if (linkHit <= 0) {
							this.epsUd.makeMessage(
							    prjname,
							    ppm + "," + pm + "," + ba + "," + sp,
							    "Requirement Mapping",
							    "Please Assign a Task to Requirement("
							        + rs.getString("ReqTitle") + ")", dateEnd);
						}
					}
					iLastLevel = rs.getInt("ReqLevel");
				}
				...
				rs = this.ebEnt.dbDyn
				    .ExecuteSql("SELECT * FROM schedule WHERE nmProjectId=" + this.stPk
				        + " and nmBaseline=" + this.nmBaseline + " ORDER BY RecId DESC");
				rs.last();
				iSch = rs.getRow();
				iLastLevel = -1;
				for (int i = 1; i <= iSch; i++) {
					...
					if (iLastLevel == rs.getInt("SchLevel")) {
						...
						linkHit = this.ebEnt.dbDyn
						    .ExecuteSql1n("SELECT COUNT(*) as c FROM teb_link WHERE nmProjectId="
						        ...
						        + " and nmBaseline="
						        + this.nmBaseline
						        + " and nmLinkFlags=1 and nmToId=" + rs.getInt("RecId"));
						if (linkHit <= 0) {
							this.epsUd.makeMessage(
							    prjname,
							    ppm + "," + pm + "," + ba + "," + sp,
			...
			this.ebEnt.dbDyn
			    .ExecuteUpdate("update teb_link set nmPercent=0 where nmProjectId="
			        + this.stPk + " and nmBaseline=" + this.nmBaseline
			        + " and nmLinkFlags=1 and "
			...
			rs = this.ebEnt.dbDyn
			    .ExecuteSql("select "
			        ...
			        + "from teb_link where nmProjectId=" + this.stPk
			        + " and nmBaseline=" + this.nmBaseline
			...
			for (int iL = 1; iL <= iMax; iL++) {
				...
				if (rs.getDouble("nmPercent") < 100 && rs.getInt("nmRemainder") > 0) {
					double nmRemainder = (100.0 - rs.getDouble("nmPercent"))
					    / rs.getInt("nmRemainder");
					this.ebEnt.dbDyn.ExecuteUpdate("update teb_link set nmPercent="
					    + nmRemainder + " " + "where nmProjectId=" + this.stPk
					    + " and nmBaseline=" + this.nmBaseline + " and nmLinkFlags=1 "
					    + "and nmToId=" + rs.getString("nmToId")
					    + " and nmRemainder != 0");
				}

				if (rs.getDouble("nmPercent") < 100
				    && this.ebEnt.dbDyn
				        .ExecuteSql1n("select count(*) from Schedule where"
				            + " nmProjectId=" + this.stPk + " and nmBaseline="
				            + this.nmBaseline + " and RecId=" + rs.getString("nmToId")
				            + " and (SchFlags & 0x8000) != 0 ") > 0) {
					// If < 100 % and SchFlags == Amortize ... Distribute to all
					this.ebEnt.dbDyn
					    .ExecuteUpdate("update Requirements r left join teb_link l"
					        + " on l.nmProjectId=r.nmProjectId and l.nmBaseline=r.nmBaseline and l.nmLinkFlags=1 and l.nmFromId=r.RecId"
					        + " and l.nmToId=" + rs.getInt("nmToId")
					        + " set l.nmLinkFlags=5"
					        + " where (r.ReqFlags & 0x10) != 0 and (l.nmAmortize="
					        + rs.getInt("nmToId") + " or l.nmAmortize is null )");

					ResultSet rs2 = this.ebEnt.dbDyn
					    .ExecuteSql("select r.RecId from Requirements r left join teb_link l"
					        + " on l.nmProjectId=r.nmProjectId and l.nmBaseline=r.nmBaseline and l.nmLinkFlags=1 and l.nmFromId=r.RecId"
					        + " and l.nmToId="
					        + rs.getInt("nmToId")
					        + " where (r.ReqFlags & 0x10) != 0"
					        + " and (l.nmAmortize="
					        + rs.getInt("nmToId") + " or l.nmAmortize is null )");
					rs2.last();
					...
					if (iMax2 > 0) {
						...
						for (int i2 = 1; i2 <= iMax2; i2++) {
							rs2.absolute(i2);
							linkMap(1, stPk, nmBaseline, stPk, rs2.getInt("RecId"), stPk,
							    rs.getInt("nmToId"), nmRemainder, "", rs.getInt("nmToId"), 0);
						}
					}
					this.ebEnt.dbDyn
			...
			if (iMax > 0)
				...
				sbReturn
				    .append("<td style='color:red;'>Requirement Mapping</td><td align=right>"
				        + iMax + "</td><td style='color:red;'>fail</td>");
				this.epsUd.makeMessage(prjname, ppm + "," + pm + "," + ba + "," + sp,
				    "Requirement Mapping", "Please Assign Tasks to Requirements",
				    dateEnd);
			}
			sbReturn.append(this.getElapsed());
			rs1.close();
			...
			if (iMax > 0) {
				...
				if (this.ebEnt.ebUd.request.getParameter("stAction").equals("runeob")) {
					this.epsUd.makeMessage(prjname, ppm + "," + pm + "," + ba + "," + sp,
					    "Schedule Mapping", "Schedules Mapped &lt; 100%", dateEnd);
				}

				sbReturn
				    .append(stHead
				        + "<td>Requirements to Schedules</td><td>Schedules Mapped &lt; 100% <br>");
				for (int i = 1; i <= iMax; i++) {
					rs1.absolute(i);
					sbReturn.append("<a href='" + makeScheduleLink(rs1.getInt("nmToId"))
					    + "'>" + rs1.getString("nmToId") + "</a>&nbsp;");
				}
	...
	private String makeScheduleLink(int SchRecId) {
		...
		int SchId = 0;
		try {
			SchId = this.ebEnt.dbDyn
			    .ExecuteSql1n("select SchId from Schedule where nmProjectId=" + stPk
			        + " and nmBaseline=" + this.nmBaseline + " and RecId=" + SchRecId);
			iFrom = (SchId / this.epsUd.rsMyDiv.getInt("MaxRecords"))
			    * this.epsUd.rsMyDiv.getInt("MaxRecords");
		} catch (Exception e) {
		}
		return "./?stAction=projects&t=12&do=xls&pk=" + this.stPk
		    + "&parent=&child=21&from=" + iFrom + "#row" + SchId;
	}
	...
	public String doMap(String stChild, String stR) {
		...
		String stLink = "?stAction=projects&t="
		    + this.ebEnt.ebUd.request.getParameter("t") + "&do=xls&pk=" + stPk
		    + "&parent=" + stTemp + "&child=" + stChild + "&a=map&r=" + stR;
		int iFrom = 0;
		...
		try {
			...
			if (stChild.equals("21")) {
				...
				if (stFrom != null && stFrom.length() > 0
					...
					stFromRecId = ebEnt.dbDyn
					    .ExecuteSql1("SELECT RecId FROM requirements WHERE nmProjectId="
					        + stPk + " AND nmBaseline=" + this.nmBaseline + " AND ReqId="
					        + stFrom);
				stTo = ebEnt.dbDyn
				    .ExecuteSql1("SELECT SchId FROM schedule WHERE RecId=" + stR);
			} else {
				stTo = this.ebEnt.ebUd.request.getParameter("rid");
				stFromRecId = stR;
				if (stTo != null && stTo.length() > 0 && Integer.parseInt(stTo) > 0)
					stToRecId = ebEnt.dbDyn
					    .ExecuteSql1("SELECT RecId FROM schedule WHERE nmProjectId="
					        + stPk + " AND nmBaseline=" + this.nmBaseline + " AND SchId="
					        + stTo);
				stFrom = ebEnt.dbDyn
				    .ExecuteSql1("SELECT ReqId FROM requirements WHERE RecId=" + stR);
			}
			if (stDo2 != null && stDo2.equals("del")) {
				this.ebEnt.dbDyn
				    .ExecuteUpdate("delete l.* from teb_link l, Projects p"
				        + " where l.nmProjectId=p.RecId and l.nmBaseline=p.CurrentBaseline and l.nmLinkFlags=1"
				        + " and l.nmProjectId=" + stPk + " and l.nmFromId="
				        + stFromRecId + " and l.nmToId=" + stToRecId);
				analyzeLink();
				...
				if (stTemp != null && stTemp.equals("Save")) {
					...
					if (stFrom == null || stFrom.length() <= 0) {
						...
						try {
							...
							iCount = this.ebEnt.dbDyn
							    .ExecuteSql1n("SELECT count(*) FROM Requirements r, Projects p"
							        + " where r.nmProjectId=p.RecId and r.nmBaseline=p.CurrentBaseline"
							        + " and (ReqFlags & 0x10 ) != 0 and r.nmProjectId="
							        + stPk + " and r.RecId=" + stFromRecId);
							if (iCount <= 0)
								this.ebEnt.ebUd
								    .setPopupMessage("Invalid low level Requirement ID: "
								        + stFromRecId);
						} catch (Exception e) {
							this.ebEnt.ebUd
							    .setPopupMessage("Invalid low level Requirement ID: "
					...
					if (stTo == null || stTo.length() <= 0) {
						...
						try {
							...
							iCount = this.ebEnt.dbDyn
							    .ExecuteSql1n("SELECT count(*) FROM Schedule s, Projects p"
							        + " where s.nmProjectId=p.RecId and s.nmBaseline=p.CurrentBaseline"
							        + " and (SchFlags & 0x10 ) != 0 and s.nmProjectId="
							        + stPk + " and s.RecId=" + stToRecId);
							if (iCount <= 0)
								this.ebEnt.ebUd.setPopupMessage("Invalid low level Task Id: "
								    + stToRecId);
						} catch (Exception e) {
							this.ebEnt.ebUd.setPopupMessage("Invalid low level Task Id: "
							    + stToRecId);
					...
					if (stTemp == null || stTemp.length() <= 0) {
						if (stChild.equals("19") && stToOrig.equals("-1")) {
							...
							linkMap(1, stPk, nmBaseline, stPk, iFromId, stPk, iToId,
							    dPercent, "", 0, iRemainder);
							...
							if (stChild.equals("21")) {
								stSql = "update teb_link l, Projects p"
								    ...
								    + " where l.nmProjectId=p.RecId and l.nmBaseline=p.CurrentBaseline and l.nmLinkFlags=1"
								    + " and l.nmProjectId=" + stPk + " and l.nmFromId="
								    + stFromOrig + " and l.nmToId=" + stToOrig;
			...
			if (stDo2 != null && stDo2.equals("edit")) {
				if (stFromRecId.equals("-1")) {
					...
					stToOrig = stToRecId;
					ResultSet rsValue = this.ebEnt.dbDyn
					    .ExecuteSql("select * from teb_link l, Projects p"
					        + " where l.nmProjectId=p.RecId and l.nmBaseline=p.CurrentBaseline and l.nmLinkFlags=1"
					        + " and l.nmProjectId=" + stPk + " and l.nmFromId="
					        + stFromRecId + " and l.nmToId=" + stToRecId);
					rsValue.last();
				...
				sbReturn
				    .append("<tr><td>Requirement ID:</td><td><input type=text name=stFrom style='text-align:right;width:50px;' value=\"");
				...
				if (stToRecId == null || "".equals(stToRecId) || "-1".equals(stToRecId)) {
					...
					sbReturn.append(this.ebEnt.ebUd.addOption2("Select Task id", "-1",
					    stToRecId));
					ResultSet taskIds = this.ebEnt.dbDyn
					    .ExecuteSql("SELECT * FROM Schedule s where (SchFlags & 0x10 ) != 0 and s.nmProjectId="
					        + stPk);
					taskIds.last();
					...
					for (int i = 1; i <= maxIds; i++) {
						...
						sbReturn.append(this.ebEnt.ebUd.addOption2(
						    Integer.toString(taskIds.getInt("SchId")) + ": "
						        + taskIds.getString("SchTitle"),
						    Integer.toString(taskIds.getInt("SchId")), stToRecId));
					}
					sbReturn.append("</select>");
				} else {
					sbReturn
					    .append("<input type=text style='text-align:right;width:50px;' name=stTo value=\"");
					sbReturn.append(stTo);
					sbReturn.append("\">");
				}

				sbReturn
				    .append("</td></tr><tr><td>Map:<td><input type=test style='text-align:right;width:50px;' name=nmPercent value=\"");
				sbReturn.append(nmPercent);
				sbReturn.append("\">% <select name=nmRemainder>");
				sbReturn.append(this.ebEnt.ebUd.addOption2("Map by Percent", "0",
				    nmRemainder));
				sbReturn.append(this.ebEnt.ebUd.addOption2("Remainder", "1",
				    nmRemainder));
				sbReturn.append("</select></td></tr>");
				sbReturn
				    .append("<tr><td colspan=2 align=center><input type=submit name=Save value='Save'> &nbsp;");
				...
				sbReturn
				    .append("<input type=submit name=Save value='Cancel'></td></tr>");
			} else if (stChild.equals("19")) {
				// Requirements Map
				stSql = "select * from Schedule s, teb_link l, Requirements r where l.nmLinkFlags=1 and "
				    ...
				    + " and s.nmProjectId="
				    + stPk
				    + " and s.nmBaseline=" + this.nmBaseline + " order by r.ReqId";
				ResultSet rs = this.ebEnt.dbDyn.ExecuteSql(stSql);
				rs.last();
				int iMax = rs.getRow();
				if (iMax > 0) {
					rs.absolute(1);
					sbReturn
					    .append("<tr><td align=right valign=bottom><b>Requirement</b> ID: ");
					sbReturn.append(rs.getString("ReqId"));
					...
					sbReturn
					    .append("<tr><th class=l1th>Action</th><th class=l1th>ID</th><th class=l1th>Level</th><th class=l1th>Task Title</th><th class=l1th colspan=2>Map</th></tr>");
					int iLastSchId = -1;
					for (int iR = 1; iR <= iMax; iR++) {
						rs.absolute(iR);
						sbReturn.append("<tr>");
						sbReturn.append("<td bgcolor=white align=center valign=bottom>");
						sbReturn.append("<a title='Edit' href='" + stLink
						    + "&do2=edit&rid=" + rs.getString("SchId")
						    + "'><img src='./common/b_edit.png'></a>");
						sbReturn
						    .append("<a title='Delete' href='"
						        + stLink
						        + "&do2=del&rid="
						        + rs.getString("SchId")
						...
						sbReturn.append("<td bgcolor=white align=right valign=bottom>"
						    + rs.getString("SchId") + "</td>");
						sbReturn.append("<td bgcolor=white align=right valign=bottom>"
						    + rs.getString("SchLevel") + "</td>");
						sbReturn.append("<td bgcolor=white align=left valign=bottom>");
						if ((iLastSchId + 1) != rs.getInt("SchId")) // Out of sync
							sbReturn.append(this.epsUd.epsEf.fullSchTitle(rs, stPk,
							    nmBaseline));
						else {
						...
						sbReturn.append("</td>");
						sbReturn.append("<td bgcolor=white align=right valign=bottom>"
						    + df.format(rs.getDouble("nmPercent")) + "</td>");
						...
						if (rs.getInt("nmAmortize") > 0)
							sbReturn
							    .append("&nbsp;Amortize(" + rs.getInt("nmAmortize") + ")");
					...
					if (stChild.equals("21")) {
						sbReturn.append("<tr>");
						sbReturn.append("<td bgcolor=white align=right colspan=4>Total ("
						    + iCount + "):</td>");
						sbReturn.append("<td bgcolor=white align=right><b>"
						    + df.format(nmTotalPercent) + "</b></td>");
						nmTotalPercent += rs.getDouble("nmPercent");
						iCount++;
				...
				sbReturn
				    .append("<tr><td bgcolor=white align=center colspan=6><input type=button onClick =\"parent.location='");
				sbReturn.append(stGoBack);
				sbReturn.append("&pk=");
				sbReturn.append(stPk);
				sbReturn.append("&do=edit'\" value='Go Back to Projects'>");
				sbReturn
				    .append("&nbsp;&nbsp;<input type=button onClick=\"parent.location='");
				sbReturn.append(stLink);
				sbReturn
				    .append("&do2=edit&rid=-1'\" value='Insert New Map'></td></tr>");
			} else if (stChild.equals("21")) {
				if (nmAmortize != null && nmAmortize.length() > 0) {
					/*
					 * this.ebEnt.dbDyn.ExecuteUpdate("update teb_link l, Requirements r"
					 * + " set l.nmAmortize=" + nmAmortize + " where  r.ReqId=l.nmFromId"
					 * + " and r.RecId=" + stR + " and r.nmProjectId=" + stPk +
					 * " and r.nmBaseline=" + this.nmBaseline);
					 */
					if (nmAmortize.equals("1")) // YES
					{
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchFlags = ( SchFlags | 0x8000 )"
						        + " where nmProjectId="
						        + stPk
						        + " and nmBaseline="
						        + this.nmBaseline + " and RecId=" + stR);
						// this.ebEnt.dbDyn.ExecuteUpdate("update teb_link set nmLinkFlags=5 where (nmLinkFlags&0x1) != 0"
						// + " and nmProjectId="+stPk+" and nmBaseline=" +
						// this.nmBaseline+" and nmToId="+ stR+" and nmAmortize=" + stR);
					} else { // No
						this.ebEnt.dbDyn
						    .ExecuteUpdate("update Schedule set SchFlags = ( SchFlags & ~0x8000 )"
						        + " where nmProjectId="
				...
				if (iMax > 0) {
					rs.absolute(1);
					sbReturn
					    .append("<tr><td align=right valign=bottom><b>Schedule</b> ID: "
					        + rs.getString("SchId"));
					sbReturn.append(" Level:" + rs.getString("SchLevel") + "");
					sbReturn
					    .append(" Title: </td><td align=left><b>"
					...
					for (int iR = 1; iR <= iMax; iR++) {
						rs.absolute(iR);
						if ((rs.getInt("SchFlags") & 0x8000) != 0
						    && rs.getInt("nmToId") == rs.getInt("nmAmortize"))
							continue;
						sbReturn.append("<tr>");
						sbReturn.append("<td bgcolor=white align=center valign=bottom>");
						sbReturn.append("<a title='Edit' href='" + stLink
						    + "&do2=edit&rid=" + rs.getString("ReqId")
						    + "'><img src='./common/b_edit.png'></a>");
						sbReturn
						    .append("<a title='Delete' href='"
						        + stLink
						...
						sbReturn.append("<td bgcolor=white align=right valign=bottom>"
						    + rs.getString("ReqId") + "</td>");
						sbReturn.append("<td bgcolor=white align=right valign=bottom>"
						    + rs.getString("ReqLevel") + "</td>");
						sbReturn.append("<td bgcolor=white align=left valign=bottom>");
						if ((iLastReqId + 1) != rs.getInt("ReqId")) // Out of sync
							sbReturn.append(this.epsUd.epsEf.fullReqTitle(rs, stPk,
							    nmBaseline));
						else {
					...
					sbReturn.append("<tr><td bgcolor=white align=right colspan=4>Total ("
					    + iCount + "):</td>");
					sbReturn.append("<td bgcolor=white align=right><b>"
					    + df.format(nmTotalPercent) + "</b></td>");
					sbReturn.append("<td bgcolor=white align=left>%</td>");
					...
					sbReturn
					    .append("<tr><td bgcolor=white valign=bottom colspan=4 align=right><b>Amortize remainder of task over rest (");
					sbReturn
					    .append(df.format(this.ebEnt.dbDyn
					        .ExecuteSql1n("select count(*) from Requirements r where r.nmProjectId="
					            + stPk
					            + " and r.nmBaseline="
					            + nmBaseline
					            + " and (r.ReqFlags & 0x10) != 0;")
					        - iCount));
					sbReturn
					    .append(") of Requirements:</b></td><td align=right bgcolor=yellow>");
					...
					if (iRemainder > 0) {
						...
						sbReturn
						    .append("<select name=nmAmortize id=nmAmortize onChange=\"document.form");
						sbReturn.append(stChild);
						...
						sbReturn
						    .append(this.ebEnt.ebUd.addOption2("Yes", "1", "" + iValue));
						sbReturn.append("</select>");
					}
					sbReturn.append("</td><tr>");
				}
	...
	public String processCriticalPath() {
		...
		int iMaxSchedule = 0;
		String stHead = "<tr><td align=right valign=top>" + this.stPk
		    + "</td><th valign=top>Critical Path</th>";
		StringBuilder sbReturn = new StringBuilder(5000);
		try {
			// Reset CP flags
			this.ebEnt.dbDyn
			...
			String stWeekend = this.epsUd.rsMyDiv.getString("stWorkDays")
			    .toLowerCase();
			...
			if (iMaxSchedule > 0) {
				...
				// for (int iR = 1; iR <= iMaxSchedule && this.stError.length() <= 0;
				// iR++)
				for (int iR = 1; iR <= iMaxSchedule; iR++) {
					rsAll.absolute(iR);
					if (aCp[rsAll.getInt("RecId")] == null) {
						aCp[rsAll.getInt("RecId")] = new EpsCriticalPath();
						aCp[rsAll.getInt("RecId")].setEpsCriticalPath(iMaxRecId, epsUd,
						    rsProject, rsAll, stHolidays, aWeekend);
				...
				sbReturn.append(stHead);
				sbReturn
				    .append("<td valign=top>by End Date</td><td valign=top colspan=3>");
				sbReturn.append("<table border=5>");

				// First, process Critical path to end, but need to stop at each Task
				// with dependenciies
				...
				if (stStartDate != null && stStartDate.length() > 8) {
					...
					if (EpsStatic.daysBetween(null, dtProjectStart) < 0) {
						stError += "<br>ERROR: Invalid start date for project: "
						    + this.rsProject.getString("ProjectName") + " " + stStartDate;
				...
				for (int iC = 0; iC < this.iMaxPath; iC++) {
					...
					for (int iS = 0; iS < iMaxRecId; iS++) {
						if (this.aaPath[iC][iS] > 0) {
							...
							if (dtEnd2 != null)
								iWorkDays = aCp[this.aaPath[iC][iS]].workDaysBetween(dtEnd2,
							...
							sbReturn.append(aCp[this.aaPath[iC][iS]].getStartEnd()
							    + "/<font color=blue>" + aCp[this.aaPath[iC][iS]].iWeekend
							    + "</font>/<font color=red>"
				...
				sbReturn
				    .append("<table><tr><td class=small><u><b>Legend:</b> (fields from left to right)</u>");
				sbReturn.append("<br>Task Id");
				sbReturn.append("<br>Task Title");
				sbReturn
				    .append("<br>{ Effort in Hours / Start Date: strating Hour / End Date: Ending Hour }");
				...
				sbReturn
				    .append("<br><span STYLE='background-color: #aaaaaa'>[Difference in days from Line above <b>if not 1 day</b>. Shows in gray background]</span>");
				...
				this.ebEnt.dbDyn
				    .ExecuteUpdate("update Schedule set SchFlags = (SchFlags | 0x200) "
				        + "where RecId in (" + stCp + ") and nmProjectId=" + stPk
				        + " and nmBaseline=" + this.nmBaseline);
				for (int iC = 0; iC < aCp.length; iC++) {
					if (aCp[iC] != null) {
						stSql = "update Schedule set SchSlack= " + aCp[iC].iSlack
						    ...
						    + "  where RecId = " + iC + " and nmProjectId=" + stPk
						    + " and nmBaseline=" + this.nmBaseline;
						this.ebEnt.dbDyn.ExecuteUpdate(stSql);
					}
				}
			...
			for (int iD = 0; iD < iMaxRecId; iD++) {
				if (aCp[iI].aDependency[iD] > 0) {
					...
					if ((aCp[aCp[iI].aDependency[iD]].iFLags & 0x8000) == 0) {
						// Need to back Track to first Finished task and forward track to
						// here
							...
							for (; iChain > 0; iChain--) {
								...
								if (aCp[aChain[iChain - 1]] != null) {
									if (aCp[iI].aDependency[iD] == aChain[iChain - 1]) {
										...
										dtEnd2 = calcStartEnd(aChain[iChain - 1], dtEnd2,
										    aChain[iChain]); // Not our dependent
									}
								}
								if (iLoop++ > iMaxRecId) {
	...
	public void calcUpdateBaseline() {
		try {
			int nmReqt = this.ebEnt.dbDyn
			    .ExecuteSql1n("SELECT count(*) FROM Requirements where nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);
			int nmSch = this.ebEnt.dbDyn
			    .ExecuteSql1n("SELECT count(*) FROM Schedule where nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);
			double nmEffort = this.ebEnt.dbDyn
			    .ExecuteSql1n("SELECT sum(SchEstimatedEffort) FROM Schedule where (SchFlags & 0x10 ) != 0 and nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);
			double nmCost = this.ebEnt.dbDyn
			    .ExecuteSql1n("SELECT sum(SchCost) FROM Schedule where (SchFlags & 0x10 ) != 0 and nmProjectId="
			        + stPk + " and nmBaseline=" + nmBaseline);
			this.ebEnt.dbDyn.ExecuteUpdate("update teb_baseline set " + "nmReqt="
			    + nmReqt + ",nmSch=" + nmSch + ",nmEffort=" + nmEffort + ",nmCost="
			    + nmCost + " " + "where nmProjectId=" + stPk + " and nmBaseline="
			    + nmBaseline);
		} catch (Exception e) {
			this.stError += "<br>ERROR followPath " + e;
		}
	...
	public String xlsBaseline(String stChild) {
		...
		try {
			...
			iMaxFields = 12;
			if (stAction != null && stAction.length() > 0 && stFrom != null
			    && stFrom.length() > 0) { // PROCESS ACTIONS
				if (stAction.equals("delete")) {
					this.ebEnt.dbDyn
					    .ExecuteUpdate("delete from teb_baseline where nmProjectId="
					        + stPk + " and nmBaseline=" + stFrom);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("delete from Schedule where nmProjectId=" + stPk
					        + " and nmBaseline=" + stFrom);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("delete from Requirements where nmProjectId="
					        + stPk + " and nmBaseline=" + stFrom);
					this.ebEnt.dbDyn.ExecuteUpdate("delete from Test where nmProjectId="
					    + stPk + " and nmBaseline=" + stFrom);
					this.ebEnt.dbDyn
					    .ExecuteUpdate("delete from teb_link where nmProjectId=" + stPk
					        + " and nmBaseline=" + stFrom);
					this.ebEnt.dbDyn
					...
					int nmMaxBaseline = this.ebEnt.dbDyn
					    .ExecuteSql1n("SELECT max(nmBaseline) FROM teb_baseline where nmProjectId ="
					        + stPk);
					nmMaxBaseline++;
					...
					this.ebEnt.dbDyn.ExecuteUpdate("update Projects set CurrentBaseline="
					    + nmMaxBaseline + stAdditional + " where RecId=" + stPk
					    + " and CurrentBaseline=" + nmBaseline);
					copyRows("teb_baseline", "nmProjectId", stPk, "nmBaseline", stFrom,
					    "" + nmMaxBaseline);
					this.ebEnt.dbDyn.ExecuteUpdate("update teb_baseline set stType="
					    + this.ebEnt.dbDyn.fmtDbString(stType) + ",nmUserEntered="
					    + this.ebEnt.ebUd.getLoginId() + " where nmProjectId=" + stPk
					    + " and nmBaseline=" + nmMaxBaseline);
					copyRows("Schedule", "nmProjectId", stPk, "nmBaseline", stFrom, ""
					    + nmMaxBaseline);
					copyRows("Requirements", "nmProjectId", stPk, "nmBaseline", stFrom,
					    "" + nmMaxBaseline);
					copyRows("Test", "nmProjectId", stPk, "nmBaseline", stFrom, ""
					    + nmMaxBaseline);
					copyRows("teb_link", "nmProjectId", stPk, "nmBaseline", stFrom, ""
					    + nmMaxBaseline);
					copyRows("teb_project", "nmProjectId", stPk, "nmBaseline", stFrom, ""
					    + nmMaxBaseline);
					nmBaseline = nmMaxBaseline;
				}
			}
			sbReturn.append("</form><form method=post name='form" + stChild
			    + "' id='form" + stChild + "' onsubmit='return myValidation(this)'>"
			    + "<table class=l1tablenarrow><tr><td class=l1td colspan="
			    + iMaxFields + " align=center>" + "<h2>"
			    + this.rsProject.getString("ProjectName") + "</h2></td></tr>");
			xlsAnalyze(); // must analyze it
			stSql = "SELECT * FROM teb_baseline where nmProjectId =" + stPk
			    + " order by nmBaseline desc";
			...
			for (int iB = 1; iB <= iMaxB; iB++) {
				...
				if (rsB.getInt("nmBaseline") != nmBaseline)
					stReturn1 += "<a title='Delete' href="
					    + "'"
					    + stLink
					    + "&submit=delete&nmFrom="
					    + rsB.getString("nmBaseline")
					    + "'>"
					    + "<img src='./common/b_drop.png' "
					    + "onClick=\"return myConfirm('Are you sure you want to delete this item?', "
					    + rsB.getString("nmBaseline") + "," + iMaxB + " )\"></a></td>";
				stReturn1 += "<td class=l1td align=right>";
				stReturn1 += rsB.getString("nmBaseline") + "</td>";
				stReturn1 += "<td class=l1td align=right>" + rsB.getString("stType")
				    + "</td>";
				stReturn1 += "<td class=l1td align=right>"
				    + rsB.getInt("nmFromBaseline") + "</td>";
				stReturn1 += "<td class=l1td>"
				    + this.epsUd.getUserName(rsB.getInt("nmUserEntered")) + "</td>";
				stReturn1 += "<td class=l1td>"
				    + this.ebEnt.ebUd.fmtDateFromDb(rsB.getString("dtEntered"))
				    + "</td>";
				stReturn1 += "<td class=l1td>";
				...
				stReturn1 += "</td>";
				stReturn1 += "<td class=l1td>"
				    + this.ebEnt.ebUd.fmtDateTimeFromDb(rsB.getString("dtLastAnalyze"))
				    + "</td>";
				stReturn1 += "<td class=l1td align=right>" + rsB.getString("nmReqt")
				    + "</td>";
				stReturn1 += "<td class=l1td align=right>" + rsB.getString("nmSch")
				    + "</td>";
			...
			if (stAction == null || stFrom == null) {
				...
				if ((this.ebEnt.ebUd.getLoginPersonFlags() & 0x20) != 0) // PPM only
					iAllowed = 1;
				stType += this.ebEnt.ebUd
				    .addOption4("Approve", "Approve", "", iAllowed);
				stType += this.ebEnt.ebUd.addOption4("Completed", "Completed", "",
				    iAllowed);
				stType += this.ebEnt.ebUd.addOption4("Suspended", "Suspended", "",
				    iAllowed);
				stType += "</select>";
				sbReturn
				    .append("<tr><td class=l1td colspan="
				        + iMaxFields
				        + " align=center><br><table bgcolor=yellow><tr>"
				        + "<td>Create new baseline based on </td><td>"
				        + stSelect
				        + "</td><td>"
				        + stType
				        + "</td><td> "
		...
		sbReturn.append("</table>");
		sbReturn.append("<input type=button onClick=\"parent.location='" + stGoBack
		    + "&pk=" + stPk + "&do=edit'\" value='Go Back to Projects'>");
		return sbReturn.toString();
	}

	public void copyRows(String stTable, String stPk, String stPkValue,
	    String stPk2, String stPk2Value, String stPk2New) {
		try {
			...
			stSql += " (" + stFields + ") select " + stSelectFrom + " from "
			    + stTable + " t2 where t2." + stPk + " = " + stPkValue + " and t2."
			    + stPk2 + " = " + stPk2Value;
			this.ebEnt.dbDyn.ExecuteUpdate(stSql);
		} catch (Exception e) {
	...
	private String editMake(String stChild, String stR) {
		...
		try {
			...
			String s1 = "</form><form method=post name='form" + stChild
			    ...
			    + "<center><table><tr><td valign=middle><h2>"
			    + this.rsProject.getString("ProjectName")
			    + "</h2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
			sbReturn.append(s1);
			sbReturn.append("<td align=left><b>");
			...
			case 9994:
			case 9974:
				rsField = this.ebEnt.dbDyn
				    .ExecuteSql("select * from teb_fields where nmDataType=44");
				rsField.absolute(1);
				stValue = this.ebEnt.dbDyn
				    .ExecuteSql1("select SchOtherResources from Schedule where nmProjectId="
				        + stPk
				        + " "
				        + "and nmBaseline="
				        + nmBaseline
				        + " and SchId="
				        + stR);
				sbReturn.append(this.epsUd.epsEf
				    .makeOtherResources(rsField, stValue, 1));
				break;
			case 9993:
			case 9973:
				rsField = this.ebEnt.dbDyn
				    .ExecuteSql("select * from teb_fields where nmDataType=43");
				rsField.absolute(1);
				stValue = this.ebEnt.dbDyn
				    .ExecuteSql1("select SchInventory from Schedule where nmProjectId="
				        + stPk + " " + "and nmBaseline=" + nmBaseline + " and SchId="
				        + stR);
			...
			case 9972:
				rsField = this.ebEnt.dbDyn
				    .ExecuteSql("select * from teb_fields where nmDataType=45");
				rsField.absolute(1);
				stValue = "";
				sbReturn
				    .append(this.epsUd.epsEf.makeSuccessors(rsField, 1, null, null));
				break;
			case 9991:
			case 9971:
				rsField = this.ebEnt.dbDyn
				    .ExecuteSql("select * from teb_fields where nmDataType=42");
				rsField.absolute(1);
				stValue = "";
				sbReturn.append(this.epsUd.epsEf.makeDependencies(rsField, 1, null,
				    null));
				break;
			case 9990:
			case 9970:
				rsField = this.ebEnt.dbDyn
				    .ExecuteSql("select * from teb_fields where nmDataType=41");
				rsField.absolute(1);
				stValue = this.ebEnt.dbDyn
				    .ExecuteSql1("select SchLaborCategories from Schedule where nmProjectId="
				        ...
				        + " and RecId="
				        + stR);
				sbReturn.append(this.epsUd.epsEf.makeLaborCategories(rsField, stValue,
				    1));
				break;
			default:
	...
	public void processRequirementCost(int reqID) throws SQLException {
		...
		this.ebEnt.dbDyn.ExecuteUpdate("update requirements set ReqCost='"
		    ...
		    + reqID + " and nmBaseline=" + this.nmBaseline);
		int nmParentId = this.ebEnt.dbDyn
	...
	public void processScheduleRequirementCost(int scheduleID)
		...
		ResultSet stResult = this.ebEnt.dbDyn
		    .ExecuteSql("select nmFromId from teb_link where nmLinkFlags=1 and nmProjectId="
		        + stPk
		        + " and nmBaseline="
		        + this.nmBaseline
		        + " and nmToId="
		...
		try {
			...
			while (stResult.next()) {
				...
				if (lowlvl) {
					...
					while (rResult.next()) {
						stCost = this.ebEnt.dbDyn
						    .ExecuteSql1("select SchCost from schedule where nmProjectId="
						        + projID + " and nmBaseline=" + bsline + " and RecId="
						...
						if (rResult.getString("l.nmRemainder").equals("1"))
							...
							rCost += Double.parseDouble(stCost)
							    * (Double.parseDouble(rResult.getString("l.nmPercent")) * 0.01);
					}
					this.ebEnt.dbDyn.ExecuteUpdate("update requirements set ReqCost='"
					    + df.format(rCost) + "' where nmProjectId=" + projID
					...
					rCost = Double.parseDouble(stCost == null || "".equals(stCost) ? "0"
					    : stCost);
					this.ebEnt.dbDyn.ExecuteUpdate("update requirements set ReqCost='"
					    + df.format(rCost) + "' where nmProjectId=" + projID
					    + " and RecId=" + reqID + " and nmBaseline=" + bsline);
	...
	private String makeDependenciesTB(ResultSet rs, String stChild) {
		...
		try {
			if (this.stPk != null && this.stPk.length() > 0 && stChild != null
			    && stChild.length() > 0) {
				stPrj = this.stPk;
				stChild = "21";
			...
			for (int k = 1; k <= (iRecMax + 1); k++) {
				...
				if (k < (iRecMax + 1)) {
					...
					// stReturn += "<input type=image name=del id=del value=" + k +
				...
				/*
				 * for (int iLc = 1; iLc <= iMaxPrj; iLc++) { rsPrj.absolute(iLc);
				 * stReturn +=
				 * this.ebEnt.ebUd.addOption2(rsPrj.getString("ProjectName"),
				 * rsPrj.getString("RecId"), stFromProject); }
				 */
				stReturn += "<td align=right>" + newtxt + "<input type=text name=did_"
				    + k + " id=did_" + k + " value=\"" + stFromId
				    + "\" size=5 style='text-align:right'></td>";
				stReturn += "<td>"
				    + this.epsUd.epsEf.ConstraintList("type_" + k, stType, "")
				    + "</td>";
				stReturn += "<td align=right><input type=text name=lag_" + k
				    + " id=lag_" + k + " value=\"" + stLag
	...
	private void analyzeLowLevelTasks() {
		try {
			String stSql = "SELECT * FROM Schedule where nmProjectId=" + stPk
			    + " and nmBaseline=" + nmBaseline + " order by SchId desc";
			ResultSet rs1 = this.ebEnt.dbDyn.ExecuteSql(stSql);
			rs1.last();
			int iSch = rs1.getRow();
			...
			for (int iR = 1; iR <= iSch; iR++) {
				...
				if (rs1.getInt("lowlvl") != SchFlags)
					this.ebEnt.dbDyn.ExecuteUpdate("update Schedule set lowlvl = "
					    + SchFlags + " where nmProjectId=" + stPk + " and nmBaseline="
