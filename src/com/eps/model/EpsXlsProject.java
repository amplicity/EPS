...
class EpsXlsProject //extends EpsUserData
{
            ...
            if (rsFields.getInt("nmForeignId") == 825 && this.ebEnt.ebUd.request.getParameter("edit") != null){
            	...
            	laborcat = stValue;
                 ...
                 else
                 {
                   ...
                   //The low-level (LL) indicator is not being set for a task that is low level when you display a schedule task's attributes. #110
                   else if(rsFields.getString("stDbFieldName").equals("SchLevel") ){
                      
                         if (rs.getInt("lowlvl") == 1 && rs.getInt("SchLevel") == Integer.parseInt(stValue)){ // same level as before, therefore we are LAST
                        	 sbReturn.append("(LL) ");
                         }
  ...
  public String analyzeLink()
  {
    ...
    try
    {
      ...
      ResultSet rs = this.ebEnt.dbDyn.ExecuteSql("select ProjectName, ProjectManagerAssignment, ProjectPortfolioManagerAssignment, BusinessAnalystAssignment, Sponsor FROM projects where RecId=" + this.stPk);
      rs.absolute(1);
    ...
    //Verify low level requirements and tasks are mapped #19
      if(this.ebEnt.ebUd.request.getParameter("stAction").equals("runeob")){
	      ...
	      rs = this.ebEnt.dbDyn.ExecuteSql("SELECT * FROM requirements WHERE nmProjectId=" + this.stPk + " and nmBaseline=" + this.nmBaseline + " ORDER BY RecId DESC");
	      rs.last();
	      int iSch = rs.getRow();
	      int iLastLevel = -1;
	      for (int i = 1; i <= iSch; i++)
	      ...
	      rs = this.ebEnt.dbDyn.ExecuteSql("SELECT * FROM schedule WHERE nmProjectId=" + this.stPk + " and nmBaseline=" + this.nmBaseline + " ORDER BY RecId DESC");
	      rs.last();
	      iSch = rs.getRow();
	      iLastLevel = -1;
	      for (int i = 1; i <= iSch; i++)
  ...
  private void analyzeLowLevelTasks(){
	  try{
	      ...
	      {
	        rs1.absolute(iR);
	        if (iR == 1 || iLastLevel == rs1.getInt("Schlevel")) // same level as before, therefore we are LAST
	          SchFlags = 1;
	        else if (iLastLevel > rs1.getInt("Schlevel"))
	            SchFlags = 0;
	        else if (iLastLevel < rs1.getInt("Schlevel"))
	        ...
	        if(rs1.getInt("lowlvl") != SchFlags)
		        this.ebEnt.dbDyn.ExecuteUpdate("update Schedule set lowlvl = " + SchFlags
		            + " where nmProjectId=" + stPk + " and nmBaseline=" + nmBaseline + " and RecId =" + rs1.getInt("RecId"));
	        iLastLevel = rs1.getInt("Schlevel");
	      }
